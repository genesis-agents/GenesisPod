...
RAG（Retrieval-Augmented Generation）模块是 DeepDive Engine 的知识库管理和检索系统，为 AI Ask、AI Teams 等应用提供知识检索增强能力。
...
```bash
# 1. 创建知识库
curl -X POST https://api.deepdive.com/api/v1/rag/knowledge-bases \
  -d '{"name": "技术文档库"}'

# 2. 上传文档
curl -X POST https://api.deepdive.com/api/v1/rag/knowledge-bases/KB_ID/documents/upload \
  -F "file=@document.pdf"
...
```bash
curl -X POST https://api.deepdive.com/api/v1/rag/query \
  -d '{
    "query": "如何配置数据库？",
    "knowledgeBaseIds": ["kb-xxx"],
    "options": {
