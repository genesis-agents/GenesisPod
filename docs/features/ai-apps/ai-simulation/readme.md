...
AI Simulation 是 DeepDive Engine 的商业博弈模拟模块，通过模拟多方 Agent 在竞争环境中的决策和互动，帮助用户推演战略方案、评估风险和预测结果。
...
```bash
# 定义竞争场景
curl -X POST https://api.deepdive.com/api/v1/ai-simulation/scenarios \
  -d '{
    "name": "外卖大战",
...
```bash
curl -X POST https://api.deepdive.com/api/v1/ai-simulation/scenarios/SCENARIO_ID/runs \
  -d '{"rounds": 5}'
...
```bash
# 在推演过程中注入事件
curl -X POST https://api.deepdive.com/api/v1/ai-simulation/runs/RUN_ID/intervene \
  -d '{
    "message": "政府出台补贴政策",
