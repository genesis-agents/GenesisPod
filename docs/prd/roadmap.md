...
**目标**：让 Genesis 拥有"执行能力 agent"——能 Read 代码 / Edit 文件 / Bash 跑命令 / git 操作 / npm 验证，作为内部基础原语服务工程师 sub-agent 场景与未来自演进内核（v2.0）。**保守 scope**：不开 ai-app/code 用户产品；不暴露 Open API；4 模式权限锁 PR-only Apply。
