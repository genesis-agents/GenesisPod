# DeepDive Engine v2.0 产品需求文档 (PRD)
...
**DeepDive Engine** 是一款企业级 AI 驱动视觉内容生成引擎，采用 **文本模型 + 图像模型 + HTML + SVG** 的组合路径，融合：
...
DeepDive Engine 是 DeepDive 平台的核心图像生成模块，与以下模块协同：
...
├─────────────────────────────────────────────────────────────┤
│  ┌─────────────┐  ┌─────────────┐  ┌─────────────────────┐  │
│  │ AI Research │  │ AI Office   │  │ DeepDive Engine     │  │
│  │ (资源研究)   │  │ (文档生成)  │  │ (视觉内容生成) ◀───│  │
│  └──────┬──────┘  └──────┬──────┘  └──────────┬──────────┘  │
│         │                │                     │             │
│         └────────────────┴─────────────────────┘             │
...
| 功能维度     | Napkin AI | Canva Magic | Nano Banana Pro | DeepDive 现状 | **DeepDive v2.0** |
| ------------ | :-------: | :---------: | :-------------: | :-----------: | :---------------: |
| **输入理解** |
| 文本内容分析 |    ✅     |     ⚠️      |       ⚠️        |      ✅       |        ✅+        |
| URL 智能抓取 |    ❌     |     ❌      |       ❌        |      ✅       |        ✅         |
...
| 维度         | Napkin   | Canva      | Nano Banana | **DeepDive Engine**   |
| ------------ | -------- | ---------- | ----------- | --------------------- |
...
export interface GenerateInfographicInput {
  ...
  urls?: string[];
  files?: FileInput[];
  resourceId?: string; // DeepDive 资源 ID
