...
DeepDive Engine 是一个企业级 AI 驱动的深度研究和内容管理平台。目前平台已集成 Notion 作为外部知识管理工具，用户可以在 Profile 页面连接 Notion 工作区，并在 Library 中访问同步的 Notion 页面。
...
- **实时协作编辑**：不支持在 DeepDive 内直接编辑 Google Drive 文档（Phase 2）
...
```
+--------+     +----------------+     +--------------+     +----------------+
| User   |     | DeepDive       |     | Google OAuth |     | DeepDive API   |
+--------+     +----------------+     +--------------+     +----------------+
    |                 |                      |                     |
    | Click Connect   |                      |                     |
    |---------------->|                      |                     |
...
用户可以将 Google Drive 中的文件导入到 DeepDive Library 作为资源。
...
|  Destination Folder:                                              |
|  +-------------------------------------------------------------+  |
|  | My Drive / DeepDive Exports / [Select Folder]               |  |
|  +-------------------------------------------------------------+  |
|                                                                   |
|  Export Options:                                                  |
|  +-------------------------------------------------------------+  |
