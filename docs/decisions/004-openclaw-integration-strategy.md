...
Genesis.ai has mature deep content production capabilities (research, writing, team collaboration, office automation) but limited distribution reach (only WeChat MP and Xiaohongshu). The integration aims to extend Genesis's content distribution capabilities while maintaining its core identity.

## Decision

### Product Positioning: AI Content Hub

Genesis positions as an **AI-powered deep content production and distribution platform**:

```
Input (any source) → Genesis Production Engine → Distribution (any platform)
```
...
OpenClaw serves as a **distribution channel partner**, not a feature module. It extends Genesis's output reach from 2 platforms to 8+ through MCP protocol, without absorbing OpenClaw's codebase.

### Integration Model: MCP-Based Channel Extension

OpenClaw instances connect to Genesis as remote MCP Servers. Genesis's `MCPManager` (already supporting HTTP/SSE transport) connects to OpenClaw to invoke messaging capabilities. This is the same pattern used for any MCP Server, not an OpenClaw-specific integration.

```
Genesis MCPManager ──HTTP/SSE──→ OpenClaw MCP Server ──→ WhatsApp/TG/Discord/Slack
                                                    ──→ Signal/Teams/iMessage
```

### What We Do NOT Do

1. **Do NOT embed OpenClaw code** into Genesis
2. **Do NOT build a parallel messaging layer** - OpenClaw handles platform-specific protocols
3. **Do NOT depend on OpenClaw** - it's an optional channel, Genesis works without it
4. **Do NOT expose Genesis internals** - communication is strictly via MCP protocol
...
| --------------------------------------- | ------------------------------------------------------------------------------------------------------------------------ |
| Distribution Channel Only (Model A)     | Underutilizes OpenClaw ecosystem; Genesis's social module already does this via Playwright                               |
| Backend Engine (Model B)                | Genesis becomes "headless", brand hidden behind OpenClaw                                                                 |
| A2A Federation (Model C)                | Elegant but unclear practical collaboration scenarios; Moltbook is experimental                                          |
| **AI Content Hub (Model D-simplified)** | **Selected**: Amplifies Genesis's core strength (deep content production), OpenClaw fills the last-mile distribution gap |

### Why MCP Protocol (not custom integration)

- Genesis's `MCPManager` already supports stdio/HTTP/SSE transport (`mcp-client-factory.ts`)
- Genesis's MCP Server already exposes Research/Ask/Teams tools (`mcp-server/`)
...
- **Positive**: Optional integration - Genesis operates independently without OpenClaw
...
- [002 Genesis as MCP Server](./002-genesis-as-mcp-server.md)
