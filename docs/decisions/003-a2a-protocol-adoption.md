...
1. **A2ATeamMemberAdapter** implements existing `ITeamMember` interface
   ...
   - External agents are always members, never leaders (Raven retains orchestration control)

2. **Agent Cards** for Raven agents expose them to external A2A platforms
...
- **Positive**: Raven agents discoverable by external platforms
