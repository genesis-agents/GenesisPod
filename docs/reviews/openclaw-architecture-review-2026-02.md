...
export class MCPToolBridgeService implements OnModuleInit {
  ...
  async callTool(
    ...
    if (name.startsWith("tool_"))
      return this.executeTool(name.slice(5), args, context);
    if (name.startsWith("skill_"))
      return this.executeSkill(name.slice(6), args, context);
    if (name.startsWith("agent_"))
      return this.executeAgent(name.slice(6), args, context);
...
Genesis.ai 的核心引擎能力（Facade、Registry、Orchestration、Guardrails、Billing）远超 OpenClaw 这类轻量级 Agent 框架。但在**对外能力开放**的架构上，存在明显的"内强外弱"问题:
