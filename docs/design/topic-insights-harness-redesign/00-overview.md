...
### 延后项（Enhancement / Advanced Tier）

- 真 LLM 接入（`BaseAgentRunner.executeImpl`）— 需 AiChatService + prompt 工程 + retry 策略
- ST-06-COGLOOP / ST-08-QGATE / ST-09-EVAL / ST-10-FACT / ST-12-LATEX / ST-13-PERSIST / ST-14-CLEANUP 7 stages
- 11 个 Enhancement/Advanced agents
- Real DB persistence（stage.persist 当前 no-op）
- WebSocket event broadcast（harness pipeline → ResearchEventEmitter）
- PR-0.4 真 baseline 替换 mock fixtures + Claude Opus-4.7 judge
