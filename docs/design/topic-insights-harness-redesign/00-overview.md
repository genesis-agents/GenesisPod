...
- PR-0.4: 真 prod baseline 录制 2-5 mission + Claude Opus-4.7 真 judge 启用
  （AG-13-RE 真 LLM 路径需要 CLI 内加载 `NestApplicationContext`；框架已就绪）
- AG-16-MA runtime 集成（mid-mission budget/quality 调整）
- AG-17-LDP IntentGateway 集成（意图分发到 harness / legacy）
- ST-02-RESEARCH 维度并行（当前顺序跑）
- Golden runner `--mode=harness` 从 StageResults 拿真产出（当前浅拷贝 baseline）
