...
**评价**：基础设施已到位，消费方现在 *可以* 从 facade 获取这些内容，但大多数消费方**尚未迁移**。
...
import type { AiCallerFn, EstablishedFact } from "ai-engine/orchestration/services";
```
...
export { UserIntent, ContextStrategy } from "../../../../ai-engine/orchestration/services";
```
...
export { StateCategory, ExecutionStateStats } from "../orchestration/state-machine"; // V4
...
*本报告所有数据均通过 grep 全量扫描 + 关键文件逐行阅读验证。*
