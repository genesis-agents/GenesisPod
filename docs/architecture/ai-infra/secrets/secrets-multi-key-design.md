...
## 七'、P1 落地状态（2026-05-06）

| 子步骤                                       | commit      | 状态                                                                 |
| -------------------------------------------- | ----------- | -------------------------------------------------------------------- |
| schema + migration + design doc              | `b22b9d549` | ✅ 已落                                                              |
| service + admin controller + spec (12 tests) | `f88518fc1` | ✅ 已落                                                              |
| BYOK endpoint 补齐                           | —           | ⏸ 移到 P2 与 MultiKeyTable 一起做（UserApiKey schema PR-2 已 ready） |

**实施差异**：原计划独立 BYOK endpoint 在 P1 完成；评估后挪到 P2，避免后端 endpoint 与前端 UI 错峰上线导致 endpoint 无消费方。

**P2 起步前的 follow-up**：

1. 业务侧 SecretsService.getValue 替换为 SecretKeysService.getSecretKey（dual-track 还在）
2. KeyHealthStore 抽象统一（DistributableKey 现用 + SecretKey 即将用）
3. P3 切换完后再删 `secrets.encrypted_value` / `iv` / `key_version` 列
