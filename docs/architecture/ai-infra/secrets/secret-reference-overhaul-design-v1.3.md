...
> 日期：2026-05-07
> 状态：**第 4 轮草案**（吸收 v1.2 第 3 轮 SRE NOT-APPROVED + DB/安全 残余 5 项）
> 前序：v1.0 / v1.1 / v1.2

## 0. 第 3 轮评审 fix-list 索引

- 架构师：**APPROVED** ✅（无需进一步修订）
- DB NEW-DB-1/2/3：S6 state 落点 / ownerToolId 索引 / 2 个 enum migration
- 安全 N6/N7：token rotation 规程 / byokOverride 审计值最小化
- 运维 NOT-APPROVED：phantom method `cancelMissionGracefully` / Phase 0 串行算术不通 / 4 个 medium / 2 个 low
...
- 紧急快速失效：admin 调 `POST /admin/ai/tools/:toolId/flush-flag-cache` → 各 pod 立即 invalidate（通过现有 EventEmitter 跨 pod？目前是单 pod 路径，验证；否则降级为"立即对当前 pod 生效"）
...
| Sec-6 | 全部 admin endpoint `@AdminGuard` + spec test                                                                                                                  | S6 + NS3           |
...
POST-DEPLOY (5 min watch):
  ...
  - **smoke test 自动化 `[NO4 manual smoke test]`**：
       Railway post-deploy hook 调 PR-S0c e2e harness 的子集 (1 search + 1 extraction)
       → 5min 内成功必备；失败触发 PagerDuty alert
       → 不依赖人 on-call 跑命令
```
...
| Phase   | 名义                               | 实际预期（含并行优化）           |
| ------- | ---------------------------------- | -------------------------------- |
| Phase 0 | 1.5w prep1 + 1.5w {S0a,b,c,d 并行} | **3w**（v1.2 估 4w，并行后修正） |
| Phase 1 | 1w                                 | 2w                               |
| Phase 2 | 3w                                 | 5w                               |
| Phase 3 | 1w                                 | 2w                               |
| Phase 4 | 4w                                 | 4w                               |
...
- v1.1：吸收第 1 轮 26 条
- v1.2：吸收第 2 轮 18 条
- v1.3 (2026-05-07)：吸收第 3 轮（架构师 APPROVED；DB 3 新；安全 2 新；运维 NOT-APPROVED 的 phantom method + 6 项）
