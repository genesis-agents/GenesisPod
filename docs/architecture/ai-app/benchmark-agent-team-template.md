...
Stage 0 mechanical guard suite(`backend/.eslintrc.js` + `tools/ci/check-harness-namespace.sh`)对新 team 的覆盖**不全自动**:

**自动覆盖**(`tools/ci/check-harness-namespace.sh` 通用规则):
...
**需手动加**(`tools/ci/check-harness-namespace.sh` 当前 hardcode `agent-playground` / `AGENT_PLAYGROUND_` / `PLAYGROUND_`):
...
**验收**:跑 `bash tools/ci/check-harness-namespace.sh` 应 EXIT=0(strict mode 7/7 + 新 team 添加的 gate)。
...
| 2   | grep gate 7/7 green                                                                   | `bash tools/ci/check-harness-namespace.sh` → exit 0             |
