...
| 6   | **WS 不查 Redis blocklist**                           | 路 1 §3               | 被禁用户旧 socket 直到 token 过期前都能 emit join + 收事件                                             |
| 7   | **没有 80% budget soft warning event**                | 路 5 §10.5            | 80% 只 logger.warn 不 emit 业务事件，前端无法提示用户"快超预算了"                                      |
