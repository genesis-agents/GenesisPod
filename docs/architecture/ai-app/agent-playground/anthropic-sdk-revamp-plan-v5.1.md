...
|           | A.5 s3-researcher-collect hook                                    | ⏳ in progress | —                       | 最复杂 stage（fanOut + perItemPipeline）   |
|           | A.6 s4-leader-assess hook                                         | ⏳ pending     | —                       |                                            |
|           | A.7 s5-reconciler hook                                            | ⏳ pending     | —                       |                                            |
|           | A.8 s6-analyst hook                                               | ⏳ pending     | —                       |                                            |
|           | A.9 s7-writer-outline hook                                        | ⏳ pending     | —                       |                                            |
|           | A.10 s8-writer hook                                               | ⏳ pending     | —                       |                                            |
|           | A.11 s8b/s9/s9b hook (review×3)                                   | ⏳ pending     | —                       |                                            |
|           | A.12 s10-leader-foreword-signoff hook                             | ⏳ pending     | —                       |                                            |
|           | A.13 s11-persist + s12-self-evolution hook                        | ⏳ pending     | —                       | **此项落地 = pipeline-v1 首次完整可跑**    |
|           | A.14 mission_runs.metadata.runtime_version 列 + 双轨产物对比工具  | ⏳ pending     | —                       | R2-B 观察基础                              |
...
| R2-A pipeline-v1 首次完整可跑 (R2-A.13) | ⏳ pending    | 5-6 个 session |
...
- 设 `PLAYGROUND_RUNTIME=pipeline-v1` 或加白名单 → 走新轨，**目前会在 s3-researcher-collect 处 NotYetWired fail**（s1+s2 已通）
- 主干现有 30+ 个 R0.5/R1/R3-A/R2-A0/R2-A.0~A.4 commit **可安全 push**，对线上无影响
