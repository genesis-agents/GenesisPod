...
> **harness/engine 内核不得承载"可换 backend"横切实现**——telemetry exporter / sandbox runtime / cache backend / vector store / embedding provider 等真正存在多种竞争实现的能力，必须以 plugin 形态存在于 `src/plugins/`，通过 plugin-core 提供的 HookBus 注入 harness/engine。
>
> **修正（2026-05-04）**：v5 原计划将 26 项归 plugin 是过度抽象。仅满足"判别口诀第 1 条（多种合理实现可替换）"才是真 plugin，第 2 条"跨 ai-app 共用"不充分（凡平台公共 service 都跨 app 用，比如 timeout / validation / circuit-breaker / rate-limit）。这些是核心系统 primitive，归 ai-engine / ai-harness / ai-infra middleware 或 service，**不**走 HookBus。

判别口诀（修正版）：

- ✅ "如果一个能力**有多种合理 backend 实现可以替换**（vm2 vs isolated-vm / pgvector vs qdrant / OpenTelemetry vs Datadog vs Sentry），它就是 plugin 候选"
- ✅ "如果一个能力**会跟着发布 SDK 时被用户替换**（用户接 自己的 LLM key 池 / 自己的向量库），它必须是 plugin"
- ❌ ~~"所有 ai-app 都用得上但没有 ai-app 单独拥有它（rate-limit / circuit-breaker / pii-moderation），它就是 plugin 候选"~~ —— **此条作废**：核心 primitive 跨 app 共用是常态，不等于 plugin。timeout / validation / circuit-breaker / rate-limit 即是反例（一个标准状态机/算法），归核心 service 不归 plugin。
...
|           | E-W1-a tool-augment 域 plugin 迁出（已撤销）                      | ⏪ reverted    | (rollback PR)           | tool-timeout / tool-validation-zod 不属于"可换 backend"，回归 ai-engine 核心 middleware；rate-limit scaffold 同步删除                                                           |
|           | E 剩余 plugin **重分类**                                          | ⏳ pending     | —                       | 严格"是否真的可换 backend"过滤；预计仅 ~10 真 plugin（embedding-_ / vector-_ / sandbox-_ / cache-_ / telemetry-\* / llm-multi-key），其余回归 engine/harness/infra 核心 service |
...
#### 主干安全状态（截至 2026-05-04 R2-A.13 + 默认切换 commit `93ef5be03`）

- ★ **默认已切到 pipeline-v1**（2026-05-04 全 14 stage wired 完毕后）
- 紧急回滚 → Railway env 设 `PLAYGROUND_RUNTIME=legacy` 立刻切回旧轨
- legacy `TeamMission` 实现仍保留（R2-C 才删），任意时刻可回滚
- 主干现有 R0.5/R1/R3-A/R2-A0/R2-A.0~A.13 + 默认切换 commit 推到 prod，**真实流量将走新轨**
