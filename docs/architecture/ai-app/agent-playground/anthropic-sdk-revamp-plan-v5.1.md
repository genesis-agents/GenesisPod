...
|           | A.6 s4-leader-assess hook                                         | ⏳ in progress | —                       | assess primitive runRole + parseDecision   |
...
- 设 `PLAYGROUND_RUNTIME=pipeline-v1` 或加白名单 → 走新轨，**目前会在 s4-leader-assess 处 NotYetWired fail**（s1+s2+s3 已通）
- 主干现有 30+ 个 R0.5/R1/R3-A/R2-A0/R2-A.0~A.5 commit **可安全 push**，对线上无影响
