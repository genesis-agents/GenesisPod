...
**最后更新**：2026-05-08
**版本**：v1.2.1（4/4 路集体评审共识达成版本）
**状态**：✅ **APPROVED-FOR-IMPLEMENTATION**（R3 + R4 跨轮整合后 architect/reviewer/security/tester 4/4 通过）
**对应代码区域**：`backend/src/modules/ai-app/library/`、`backend/src/modules/ai-engine/skills/runtime/`、`frontend/app/library/wiki/`
**外部参考**：
...
```
L3 ai-app/library                              ★ 主要改动
  ├─ wiki/                                     ★ 新增子模块
  │   ├─ wiki-page.service.ts                  CRUD + body 解析 + export tarball
  │   ├─ wiki-page.controller.ts               REST
  │   ├─ wiki-ingest.service.ts                ingest LLM 编排（产 diff）
  │   ├─ wiki-diff.service.ts                  diff apply / revert
  │   ├─ wiki-query.service.ts                 query 路由（长 context vs RAG-pick）
  │   ├─ wiki-lint.service.ts                  lint 5 类规则编排
  │   ├─ wiki-revision.service.ts              快照写入（apply/edit/revert 三处调用）
  │   ├─ link-parser.ts                        markdown remark AST 抽 [[slug]]
  │   ├─ slug-normalize.ts                     纯函数：title → kebab-case slug
  │   ├─ skills/                               LLM prompt skills（domain="library"）
  │   │   ├─ wiki-ingest.skill.md
  │   │   ├─ wiki-stale-check.skill.md
  │   │   └─ wiki-contradiction.skill.md
  │   ├─ dto/
...
L2 ai-engine                                   仅作消费
  └─ facade/                                   已有：EmbeddingService / SkillRegistry / AiChatService /
                                                       sanitizeMarkdownBody / parseJsonFence /
                                                       PromptSkillBridge / wrapExternalContent
                                                **本期不改 facade**
...
> **依赖方向**：wiki 子模块 `imports: [PrismaModule, AiEngineModule]`，沿用 NotesModule / RAGModule pattern。**不在 facade 新增任何 export**——架构师 R1 已确认所需基元已全部 export。
...
Branch B (RAG 选页):
  Step 1:   embed(question) → 检索 WikiPageEmbedding (resolution=ONELINER) top-K (K=15)
            （只 embed oneLiner 比 body 噪声更低）
...
```
ORPHAN          (纯 SQL):    WikiPage 没有 inbound WikiPageLink + category != SOURCE
MISSING_XREF    (纯 SQL):    WikiPageLink.toSlug 不存在于 WikiPage
STALE           (LLM):       扫每页 sources → 比对 KnowledgeBaseDocument.rawContent 当前 hash 与 quote 字段
                              若 doc 改动显著（hash 变 + LLM 判 quote 上下文偏移）→ STALE
CONTRADICTION   (LLM):       同 entity / 同 concept 类页两两比；抽样上限：min(50, pairs) 对/天，
                              优先选 updatedAt < 7d 内的页
DATA_GAP        (LLM):       全 wiki 跑一次"哪些 entity 提到 ≥3 次但没自己的页"
```
...
| -------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ----- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **P0 schema + ADR**        | 10 张新表（含 `WikiKnowledgeBaseConfig`，从 P2 提到 P0）+ `KnowledgeBase.wikiEnabled` + `ExportSourceType.WIKI` + `ExportFormat.TARBALL` + 手写 SQL migration（IF NOT EXISTS 幂等 + `wiki_diffs.affected_slugs` GIN partial index + 5 处 SetNull onDelete）+ `docs/architecture/decisions/ADR-XXX-wiki-vs-graph-coexistence.md` 落盘 | 1 天  | `npm test --testPathPattern=wiki/__tests__/schema` 全绿（CRUD + uniq + FK cascade + SetNull 行为）<br>migration 在 Railway 跑两次第二次不报错                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| **P1 ingest + edit + log** | 全部 wiki service + DTO + controller + skills/wiki-ingest.skill.md + WikiPageRevision 写入 + invariant lint + diff apply 事务                                                                                                                                                                                                        | 4 天  | `npm test --testPathPattern=wiki` 全绿，至少：<br>- `link-parser.spec.ts` 10 条边界用例（见 §10）<br>- `slug-normalize.spec.ts`<br>- `wiki-page.service.spec.ts`（CRUD + slug uniq + **PATCH edit 路径写 WikiPageRevision** + **revert 跨页 revisionId 返回 403**）<br>- `wiki-diff.service.spec.ts`（事务：第 N+1 项失败前 N 不提交；CONFLICTED 路径；revert 写 revision；**WikiDiffItemsSchema zod parse 失败 400**；**affectedSlugs 实时重算（DB 字段被改空仍能挡冲突）**；**SELECT FOR UPDATE 串行化两并发 apply 测试**）<br>- `wiki-ingest.service.spec.ts`（mock ChatFacade，验证 wrapExternalContent 调用含显式 maxLength + diff 持久化 + **baselineHash 计算确定性：相同 index 两次 hash 相等**）<br>- `wiki.controller.spec.ts`（diffId IDOR：跨 KB diff 返回 403；**wikiEnabled=false 时 POST/PATCH/DELETE 返回 400**） |
| **P2 query + lint**        | wiki-query 双分支 + WikiPageEmbedding 写入（oneLiner+body）+ 5 类 lint + cron（读 P0 已建的 WikiKnowledgeBaseConfig）                                                                                                                                                                                                                | 3 天  | `npm test --testPathPattern=wiki` 全绿，至少：<br>- `wiki-query.service.spec.ts`：阈值边界 pageCount=200/201、Branch B 不加载非 top-K 页、BM25 排序生效<br>- `wiki-lint.service.spec.ts`：5 类各 ≥1 fixture，ORPHAN/MISSING_XREF 不调 LLM，CONTRADICTION 抽样上限 50<br>- cron 与手动并发 spec：第二次返回最近 finding 不重跑<br>- 把首个真实 KB query 结果存到 `eval-baseline.md`（不作为 Phase gate）                                                                                                                                                                                                                                                                                                                                                                                                                           |
