...
**最后更新**：2026-05-09
**版本**：v1.3（按 CLAUDE.md 能力归属原则把 4 项通用能力上提到 ai-engine）
**状态**：待 R5 评审（架构归属变更，需 architect 重新过；其余 3 路次要核对）
**对应代码区域**：`backend/src/modules/ai-app/library/wiki/`、`backend/src/modules/ai-engine/content/markdown/`、`backend/src/modules/ai-engine/rag/`、`backend/src/modules/ai-engine/knowledge/consistency/`（新建）、`frontend/app/library/wiki/`
**外部参考**：
...
L3 ai-app/library                              ★ wiki 业务专属
  ├─ wiki/                                     ★ 新增子模块（瘦身后）
  │   ├─ wiki-page.service.ts                  CRUD + body 解析（调 engine link-parser）+ export tarball
...
L2 ai-engine                                   ★ 通用能力新增（v1.3 上提项）
  ...
  ├─ rag/                                      （已有 embedding/vector/pipeline/abstractions）
  │   ├─ multi-resolution-search.service.ts    ★ 新增：(entityType, entityId, resolution) 三元组多分辨率检索
  │   │                                          抽象出"同一实体多分辨率 embed + 加权检索"通用能力
  │   ├─ abstractions/multi-resolution.interface.ts  ★ 新增 IMultiResolutionSearch + token
  │   └─ __tests__/                            阈值边界 / Branch B 不加载非 top-K
  ├─ knowledge/                                （已有 evidence/extraction/synthesis/world-building 等）
  │   └─ consistency/                          ★ 新建子目录
  │       ├─ contradiction-checker.service.ts  对一组 markdown 文档跑 LLM 找互相矛盾对
  │       ├─ stale-detector.service.ts         对每条文档的 source quote vs 当前 raw hash 跑 LLM 判陈旧
  │       ├─ data-gap-detector.service.ts      跑 LLM 找"提到 ≥3 次但无独立页"的 entity
  │       ├─ abstractions/consistency.interface.ts  IConsistencyChecker（统一抽象）+ token
  │       ├─ consistency.module.ts
  │       └─ __tests__/                        5 类各 ≥1 fixture（mock ChatFacade）
  └─ facade/                                   ★ 新增 4 个 export：
                                                   - parseWikiLinks (function)
                                                   - normalizeSlug (function)
                                                   - MULTI_RESOLUTION_SEARCH_SERVICE_TOKEN
                                                   - CONSISTENCY_CHECKER_SERVICE_TOKEN
                                                  现有 export 不变。
...
| **(v1.3 新增) `ai-engine/content/markdown/wiki-link-parser`**                | wiki body `[[slug]]` 解析消费方；writing/research/office 跨引用解析也复用                                                                           |
| **(v1.3 新增) `ai-engine/content/markdown/slug-normalize`**                  | wiki title→slug 规范化消费方；任何文档锚点生成复用                                                                                                  |
| **(v1.3 新增) `ai-engine/rag/multi-resolution-search`**                      | wiki query Branch B 消费方；任何"同一 entity 多分辨率 embed + 加权检索"场景复用                                                                     |
| **(v1.3 新建) `ai-engine/knowledge/consistency`**                            | wiki-lint 三类 (CONTRADICTION/STALE/DATA_GAP) 调用方；research/writing 跨段落一致性检测复用                                                         |
...
Branch B (RAG 选页):
  Step 1:   调 engine `MultiResolutionSearchService.search({
              entityType: 'wiki_page',
              kbId,
              question,
              resolution: 'ONELINER',  // 只 embed oneLiner 比 body 噪声远低
              topK: 15,
            })` → top-K pageId 列表
            （v1.3：通用多分辨率检索能力上提，wiki 仅是消费方；engine 内部用 EmbeddingService + WikiPageEmbedding 表查询）
...
### 5.3 Lint（v1.3 把 LLM 三类上提到 engine）
...
下三类 wiki-lint.service 把数据装好后调 engine 通用 primitives：
...
STALE           (LLM):  调 engine `StaleDetectorService.detect({
                        ...
                        })` → 返回每条 entry 是否 stale + 偏移度
                        wiki-lint 把 stale=true 的 entry 写入 WikiLintFinding (type=STALE)

CONTRADICTION   (LLM):  调 engine `ContradictionCheckerService.check({
                          documents: pagesGroupedByCategory,                // 按 category 分组
                          samplingLimit: config.cronLintDailyBudgetCalls,   // 抽样上限传入
                          preferRecent: { sinceHours: 168 },                // 7d 内变动页优先
                          taskProfile: { creativity: 'deterministic' },
                        })` → 返回矛盾对列表
                        wiki-lint 写入 WikiLintFinding (type=CONTRADICTION, detail={pageA, pageB, reason})

DATA_GAP        (LLM):  调 engine `DataGapDetectorService.detect({
                          documents: pages,
                          minMentions: 3,                                   // 阈值
...
> **能力归属**：三个 engine consistency primitive 都接受通用 markdown documents 数组，不依赖任何 wiki-specific schema；research 报告 / writing 长文以同样接口调用即可。
...
| -------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **P0a engine 通用能力**    | (v1.3 新) `ai-engine/content/markdown/{wiki-link-parser, slug-normalize}.util.ts` + `ai-engine/rag/multi-resolution-search.service.ts` + `ai-engine/knowledge/consistency/{contradiction-checker, stale-detector, data-gap-detector}.service.ts` + `consistency.module.ts` + 4 个 facade export                                      | 1.5 天 | `npm test --testPathPattern='ai-engine/(content/markdown\|rag/multi-resolution\|knowledge/consistency)'` 全绿；`verify:arch` 全绿（无穿透）                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
...
| **P1 ingest + edit + log** | 全部 wiki service + DTO + controller + skills/wiki-ingest.skill.md + WikiPageRevision 写入 + invariant lint + diff apply 事务（消费 P0a 的 engine 能力）                                                                                                                                                                             | 4 天   | engine 侧（P0a 已建）：<br>- `wiki-link-parser.util.spec.ts` 10 条边界用例（见 §10）<br>- `slug-normalize.util.spec.ts`<br>wiki app 侧：<br>- `wiki-page.service.spec.ts`（CRUD + slug uniq + **PATCH edit 路径写 WikiPageRevision** + **revert 跨页 revisionId 返回 403**；调 engine `parseWikiLinks` mock）<br>- `wiki-diff.service.spec.ts`（事务：第 N+1 项失败前 N 不提交；CONFLICTED 路径；revert 写 revision；**WikiDiffItemsSchema zod parse 失败 400**；**affectedSlugs 实时重算（DB 字段被改空仍能挡冲突）**；**SELECT FOR UPDATE 串行化两并发 apply 测试**）<br>- `wiki-ingest.service.spec.ts`（mock ChatFacade，验证 wrapExternalContent 调用含显式 maxLength + diff 持久化 + **baselineHash 计算确定性：相同 index 两次 hash 相等**）<br>- `wiki.controller.spec.ts`（diffId IDOR：跨 KB diff 返回 403；**wikiEnabled=false 时 POST/PATCH/DELETE 返回 400**） |
| **P2 query + lint**        | wiki-query 双分支（消费 engine MultiResolutionSearchService）+ WikiPageEmbedding 写入 + ORPHAN/MISSING_XREF wiki 自做 + CONTRADICTION/STALE/DATA_GAP 调 engine ConsistencyChecker + cron（读 P0 已建的 WikiKnowledgeBaseConfig）                                                                                                     | 3 天   | engine 侧（P0a 已建，P2 补集成测）：<br>- `multi-resolution-search.service.spec.ts`：`entityType='wiki_page'` 时阈值边界 + 排序<br>- `contradiction-checker.service.spec.ts` / `stale-detector.service.spec.ts` / `data-gap-detector.service.spec.ts` 各 ≥1 fixture（mock ChatFacade）<br>wiki app 侧：<br>- `wiki-query.service.spec.ts`：阈值边界 pageCount=200/201、Branch B mock engine search 验证 `entityType='wiki_page'` 调用、BM25 排序（Branch A）生效<br>- `wiki-lint.service.spec.ts`：ORPHAN/MISSING_XREF 纯 SQL 不调 LLM；STALE/CONTRADICTION/DATA_GAP 调 engine 服务 mock 验证传参（含 samplingLimit / preferRecent / minMentions）<br>- cron 与手动并发 spec：第二次返回最近 finding 不重跑<br>- 把首个真实 KB query 结果存到 `eval-baseline.md`（不作为 Phase gate）                                                                                       |
...
- **能力归属判断**（CLAUDE.md `.claude/rules/ai-engine.md`，**v1.3 新增对齐**）："如果明天做一个完全不同的 AI App，这个能力还能复用吗？" 答 YES → AI Engine。v1.3 上提 4 项到 engine（link-parser / slug-normalize / multi-resolution-search / consistency 三类），留 wiki/ 6 项均答 NO（依赖 wiki-specific schema）
...
- R5 评审：v1.3 架构归属上提 4 项到 engine（link-parser / slug-normalize / multi-resolution-search / consistency 三类），重点由 architect 复核归属合理性 + 其余 3 路次要核对
