...
DeepDive Engine 是一个 AI 驱动的知识发现平台，通过 4 个专项 Agent（Slides、Docs、Designer、Developer）为用户提供内容生成服务。当前工具系统仅覆盖 12 种工具类型，无法满足完整的知识工作流需求。
...
interface HumanApprovalInput {
  ...
  options?: {
    choices?: Array<{         // 选项（type=choose 时）
      id: string;
      label: string;
      description?: string;
    }>;
