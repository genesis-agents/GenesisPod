...
# URLs
FRONTEND_URL=https://deepdive.app
CORS_ORIGIN=https://deepdive.app
...
GOOGLE_CALLBACK_URL=https://api.deepdive.app/api/v1/auth/google/callback
...
# URLs
FRONTEND_URL=https://staging.deepdive.app
CORS_ORIGIN=https://staging.deepdive.app
...
GOOGLE_CALLBACK_URL=https://staging-api.deepdive.app/api/v1/auth/google/callback
...
# URLs
FRONTEND_URL=https://dev.deepdive.app
CORS_ORIGIN=https://dev.deepdive.app
...
GOOGLE_CALLBACK_URL=https://dev-api.deepdive.app/api/v1/auth/google/callback
...
```bash
# API URLs (构建时注入)
NEXT_PUBLIC_API_URL=https://api.deepdive.app
NEXT_PUBLIC_AI_URL=https://ai.deepdive.app
...
```bash
NEXT_PUBLIC_API_URL=https://staging-api.deepdive.app
NEXT_PUBLIC_AI_URL=https://staging-ai.deepdive.app
...
```bash
NEXT_PUBLIC_API_URL=https://dev-api.deepdive.app
NEXT_PUBLIC_AI_URL=https://dev-ai.deepdive.app
```
...
| 类型  | 名称                 | 目标                               | 环境        |
| ----- | -------------------- | ---------------------------------- | ----------- |
| CNAME | `@` / `deepdive.app` | Railway Frontend Production Domain | Production  |
...
- [ ] 访问 `https://staging.deepdive.app` 页面正常加载
- [ ] 访问 `https://staging-api.deepdive.app/api/v1/health` 返回健康状态
- [ ] Google OAuth 登录流程正常
- [ ] AI 功能可用（调用 LiteLLM）
- [ ] 数据库连接正常
- [ ] 从 `develop` 分支推送后自动部署
...
- [ ] 访问 `https://dev.deepdive.app` 页面正常加载
- [ ] 访问 `https://dev-api.deepdive.app/api/v1/health` 返回健康状态
- [ ] 手动触发部署正常工作
