...
本指南定义了 DeepDive Engine 项目的自动化开发闭环系统，通过 Claude Code Hooks 和 TDD 工作流实现：
...
```
用 TDD 方式为 DeepDive Engine 实现 [功能名称]：
