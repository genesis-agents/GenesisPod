...
- `frontend/lib/agent-playground/derive.ts:408` 已规范化 `social.* / agent-playground.* / ai-radar.*` namespace，"剥离 namespace 让 derive 跨 domain 通用"
- `frontend/components/common/mission-detail/`（StageStepper 等）**已被** `ai-social` + `agent-playground` 消费
- `frontend/lib/ai-social/derive-social-stages.ts`——social 已采用 playground 式纯派生
...
deriveMissionView(events)         纯函数：events → { mission, stages, agents, cost, todos, artifacts }
   （参考 lib/agent-playground/derive.ts + todo-ledger.ts；通用部分提到 lib/missions/）
        │ 只读 view-model
        ▼
...
1. **「事件→阶段」step-map**：声明本域 mission 的阶段拓扑（参考 `derive.ts` 的 `STAGE_STEPS` / `lib/ai-social/derive-social-stages.ts`），配置化，不复制派生引擎。
...
- 事件→视图必须是**纯函数派生层**（`lib/{feature}/` 或通用 `lib/missions/`），幂等可重放，**带 fixture 回归测试**（参考 `lib/agent-playground/__tests__/`：5 个生产快照 + 回归 spec）
...
| 纯派生           | `frontend/lib/agent-playground/derive.ts` · `todo-ledger.ts`                     |
| 派生测试         | `frontend/lib/agent-playground/__tests__/`（fixtures + 回归）                    |
| 共享框架         | `frontend/components/common/mission-detail/`                                     |
| 列表层           | `frontend/components/common/{page-header-hero,asset-card,missions}/`             |
| 域 step-map 范例 | `frontend/lib/ai-social/derive-social-stages.ts`                                 |
