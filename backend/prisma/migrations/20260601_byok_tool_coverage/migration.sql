...
-- 方案：docs/architecture/ai-app/byok/byok-tool-coverage-extension-2026-05-27.md (v0.2 基线)
--
-- 关键决策落地：
...
--   §16:   users 加 onboarded_byok（首次配 Key 引导 banner 控制）。
--   D5.5:  authorization_requests / authorization_grants（向系统申请授权 + admin 审批）。
--
-- 幂等：全部 IF NOT EXISTS / DO $$ EXCEPTION，可重复 apply 不报错。
...
-- 4. users 加 byok_mode + onboarded_byok ------------------------------------
ALTER TABLE "users" ADD COLUMN IF NOT EXISTS "byok_mode" "ByokMode" NOT NULL DEFAULT 'STRICT';
ALTER TABLE "users" ADD COLUMN IF NOT EXISTS "onboarded_byok" BOOLEAN NOT NULL DEFAULT false;
