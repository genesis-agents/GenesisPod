...
describe("ContentAnalysisService", () => {
  ...
  describe("analyzeContent()", () => {
    ...
    it("maps entities from AI response correctly", async () => {
      mockAiFacade.chat.mockResolvedValue({
        content: buildJsonResponse({
          ...
          keyTopics: [],
          entities: [
            { type: "organization", value: "OpenAI", count: 3, importance: 0.9 },
      ...
      const result = await service.analyzeContent({ content: "OpenAI released GPT-4." });
    ...
    it("maps visualization opportunities correctly", async () => {
      ...
      const result = await service.analyzeContent({ content: "Sales grew 50% in Q2." });
      ...
      expect(result.features.visualizationOpportunities[0].priority).toBe("high");
    });
  ...
  describe("data density calculation", () => {
    ...
    it("returns DATA_HEAVY when numbers exceed 10% of words", async () => {
      ...
      const result = await service.analyzeContent({ content: `${numbers} ${words}` });
    ...
    it("returns BALANCED for moderate data density", async () => {
      ...
      const result = await service.analyzeContent({ content: `${words} ${numbers}` });
  ...
  describe("temporal dimension detection", () => {
    ...
    it("returns TIMELINE when many year+stage markers exist", async () => {
      const content = "2019年开始，2020年阶段一，2021年阶段二，2022年里程碑，2023年里程碑完成。";
      const result = await service.analyzeContent({ content });
      expect(result.features.temporalDimension).toBe(TemporalDimension.TIMELINE);
    });

    it("returns FUTURE for future-oriented content", async () => {
      const content = "未来展望：规划中的项目将会在预测期内实现目标，计划于明年完成。";
      const result = await service.analyzeContent({ content });
      expect(result.features.temporalDimension).toBe(TemporalDimension.FUTURE);
    });

    it("returns HISTORICAL for historical content", async () => {
      const content = "历史回顾：过去十年中，曾经的发展历程值得回顾。过去的成就令人印象深刻。";
      const result = await service.analyzeContent({ content });
      expect(result.features.temporalDimension).toBe(TemporalDimension.HISTORICAL);
    });

    it("returns NONE when no temporal indicators exist", async () => {
      const content = "This is a simple technical document about software architecture patterns.";
      const result = await service.analyzeContent({ content });
  ...
  describe("suggestedStructure generation", () => {
    it("includes timeline template when hasTimeline=true", async () => {
      ...
      const content = "2020年开始。2021年扩张。2022年上市。第一阶段完成。里程碑达成。";
      const result = await service.analyzeContent({ content });
      // Basic features are merged - hasTimeline comes from rule engine
      expect(result.suggestedStructure.forSlides?.suggestedTemplates).toContain("cover");
    });
    ...
    it("generates chapter breakdown for topics from AI", async () => {
      ...
      const result = await service.analyzeContent({ content: "Business analysis content." });
    ...
    it("generates default chapter when no topics", async () => {
      ...
      const result = await service.analyzeContent({ content: "Simple content." });
      const breakdown = result.suggestedStructure.forSlides?.chapterBreakdown;
    ...
    it("suggests technical style for content with code blocks", async () => {
      ...
      const result = await service.analyzeContent({ content });
      expect(result.suggestedStructure.forDocs?.documentStyle).toBe("technical");
    });
    ...
    it("suggests executive style for persuasive content", async () => {
      ...
      const result = await service.analyzeContent({ content: "Proposal content." });
      expect(result.suggestedStructure.forDocs?.documentStyle).toBe("executive");
    });

    it("includes statistics section when hasStatistics=true", async () => {
      mockAiFacade.chat.mockRejectedValue(new Error("forced fail"));

      const content = "数据显示增长50%，同比提升30%，ROI达200%，总收入5亿元，环比持续增长。";
      const result = await service.analyzeContent({ content });
    ...
    it("includes comparison section when hasComparison=true", async () => {
      ...
      const content = "方案A vs 方案B。优势劣势对比分析。相比之下的区别不同点。方案C对比。";
      const result = await service.analyzeContent({ content });
      const sections = result.suggestedStructure.forDocs?.suggestedSections;
    ...
    it("word count scales with complexity", async () => {
      ...
      const lowWords = lowResult.suggestedStructure.forDocs?.suggestedWordCount ?? 0;
      const highWords = highResult.suggestedStructure.forDocs?.suggestedWordCount ?? 0;
  ...
  describe("summary generation fallback", () => {
    it("generates summary from features when AI gives no summary", async () => {
      ...
      const result = await service.analyzeContent({
        content: "Hello world this is some content for testing summary generation.",
      });
    ...
    it("handles null entity fields gracefully", async () => {
      mockAiFacade.chat.mockResolvedValue({
        content: buildJsonResponse({
          ...
          complexity: "medium",
          keyTopics: [],
          entities: [{ type: null, value: null, count: null, importance: null }],
          visualizationOpportunities: [],
          summary: "test",
        }),
      });
...
async function runWithComplexity(complexity: "low" | "high") {
  const service = new ContentAnalysisService({ chat: jest.fn().mockResolvedValue({
    content: `\`\`\`json\n${JSON.stringify({
      contentCategory: "informational",
      complexity,
      keyTopics: [],
  ...
  }), embed: jest.fn() } as unknown as AIEngineFacade);
