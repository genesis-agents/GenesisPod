...
describe('SkillOutputManager', () => {
  ...
  describe('normalizeKey()', () => {
    it('strips the "slides-" prefix', () => {
      expect(manager.normalizeKey('slides-outline-planning')).toBe('outline-planning');
    });

    it('strips the "teams-" prefix', () => {
      expect(manager.normalizeKey('teams-task-decomposition')).toBe('task-decomposition');
    });

    it('strips the "office-" prefix', () => {
      expect(manager.normalizeKey('office-doc-generation')).toBe('doc-generation');
    });
  ...
  describe('custom config — knownPrefixes', () => {
    it('strips a custom prefix when configured', () => {
      const customManager = new SkillOutputManager({ knownPrefixes: ['custom-'] });
    ...
    it('does not strip default prefixes when custom knownPrefixes provided', () => {
      ...
      expect(customManager.normalizeKey('slides-outline')).toBe('slides-outline');
    });
  });
