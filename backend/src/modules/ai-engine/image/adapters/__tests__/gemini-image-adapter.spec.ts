...
import { IMAGE_MODELS, IMAGE_PROVIDERS } from "../../abstractions/image-adapter.interface";
...
describe("GeminiImageAdapter", () => {
  ...
  it("throws when API key is not set", async () => {
    ...
    await expect(
      freshAdapter.generate({ prompt: "a cat" })
  ...
  it("calls generateContent endpoint for gemini models", async () => {
    ...
    const result = await adapter.generate({ prompt: "a sunset", model: "gemini-2.0-flash-exp" });
  ...
  it("calls generateImages endpoint for imagen models", async () => {
    ...
    expect(result.images[0].url).toContain("data:image/jpeg;base64,imagenbytes");
  });
      ...
      .mockReturnValueOnce(of({
        data: {
          candidates: [
            {
              content: {
                parts: [{ inlineData: { data: "flashbytes", mimeType: "image/png" } }],
              },
            },
          ],
        },
    ...
    mockPost.mockReturnValue(of({
      data: {
        candidates: [
          {
            content: {
              parts: [{ inlineData: { data: "i2ibytes", mimeType: "image/png" } }],
            },
          },
        ],
      },
  ...
  it("throws when imageToImage called without API key", async () => {
    ...
    await expect(
      fresh.imageToImage({ prompt: "test", referenceImage: "data:image/png;base64,abc" })
    ).rejects.toThrow("Gemini API key not configured");
  });
    ...
    mockPost.mockReturnValue(of({
      data: {
        candidates: [
          {
            content: {
              parts: [{ inlineData: { data: "bytes", mimeType: "image/png" } }],
            },
          },
        ],
      },
