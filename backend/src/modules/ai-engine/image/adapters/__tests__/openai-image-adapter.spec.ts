...
describe("OpenAIImageAdapter", () => {
  ...
  it("calls images/generations endpoint and returns images", async () => {
    mockPost.mockReturnValue(
      of({
        data: {
          data: [
            { url: "https://oaidalleapiprodscus.blob.core.windows.net/img1.png", revised_prompt: "revised" },
          ],
        },
      })
    );
  ...
  it("uses 1024x1024 for square dimensions", async () => {
    mockPost.mockReturnValue(of({ data: { data: [{ url: "https://example.com/img.png" }] } }));
  ...
  it("always uses 1024x1024 for DALL-E 2 regardless of dimensions", async () => {
    mockPost.mockReturnValue(of({ data: { data: [{ url: "https://example.com/img.png" }] } }));
    await adapter.generate({ prompt: "test", model: IMAGE_MODELS.DALLE_2, width: 512, height: 512 });
