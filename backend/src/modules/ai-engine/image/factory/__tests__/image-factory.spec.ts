...
function buildMockAdapter(id: string, provider: ImageProvider, models: string[]): jest.Mocked<IImageAdapter> {
  return {
    id,
    name: `${id} Adapter`,
    provider,
...
describe("ImageFactory", () => {
  ...
  it("registers an adapter and retrieves it by id", () => {
    const adapter = buildMockAdapter("gemini", IMAGE_PROVIDERS.GEMINI, ["gemini-2.0-flash-exp"]);
  ...
  it("returns default provider adapter when no id provided", () => {
    const geminiAdapter = buildMockAdapter("gemini", IMAGE_PROVIDERS.GEMINI, ["gemini-2.0-flash-exp"]);
  ...
  it("finds adapter that supports the given model", () => {
    const openaiAdapter = buildMockAdapter("openai", IMAGE_PROVIDERS.OPENAI, ["dall-e-3", "dall-e-2"]);
  ...
  it("returns undefined when no adapter supports the model", () => {
    const openaiAdapter = buildMockAdapter("openai", IMAGE_PROVIDERS.OPENAI, ["dall-e-3"]);
    factory.registerAdapter(openaiAdapter);
  ...
  it("generate routes to correct adapter by model", async () => {
    ...
    const result = await factory.generate({ prompt: "test", model: "dall-e-3" });
  ...
  it("throws when no adapter available for imageToImage", async () => {
    await expect(
      factory.imageToImage({ prompt: "test", referenceImage: "data:image/png;base64,abc" })
    ).rejects.toThrow("No image adapter available");
  });

  it("throws when adapter does not support imageToImage", async () => {
    const adapter = buildMockAdapter("openai", IMAGE_PROVIDERS.OPENAI, ["dall-e-3"]);
  ...
  it("getSupportedModels returns deduplicated model list", () => {
    const a1 = buildMockAdapter("gemini", IMAGE_PROVIDERS.GEMINI, ["model-x", "model-y"]);
    const a2 = buildMockAdapter("openai", IMAGE_PROVIDERS.OPENAI, ["model-y", "model-z"]);
    factory.registerAdapter(a1);
    factory.registerAdapter(a2);
