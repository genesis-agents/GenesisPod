...
function makeTool(
  ...
  return {
    ...
    toCompactSummary(): CompactToolSummary {
      return { id, name: `Tool ${id}`, brief: `Description of ${id}`, category };
    },
    ...overrides,
  };
}
...
describe('ToolRegistry', () => {
  ...
  describe('register()', () => {
    ...
    it('indexes tool by tag when tags are provided', () => {
      const tool = makeTool('tagged-tool', 'processing', { tags: ['nlp', 'text'] });
      registry.register(tool);
  ...
  describe('getByTag()', () => {
    ...
    it('returns tools matching the tag', () => {
      ...
      registry.register(makeTool('other-tool', 'processing', { tags: ['search'] }));
  ...
  describe('getTags()', () => {
    ...
    it('returns all tags from registered tools', () => {
      registry.register(makeTool('t', 'information', { tags: ['search', 'web'] }));
  ...
  describe('getEnabled()', () => {
    ...
    it('excludes disabled tools', () => {
      registry.register(makeTool('enabled-tool', 'information', { enabled: true }));
      registry.register(makeTool('disabled-tool', 'information', { enabled: false }));
  ...
  describe('getAllFunctionDefinitions()', () => {
    ...
    it('excludes disabled tools', () => {
      ...
      registry.register(makeTool('disabled', 'information', { enabled: false }));
      const defs = registry.getAllFunctionDefinitions();
  ...
  describe('getToolList()', () => {
    beforeEach(() => {
      registry.register(
        makeTool('t-info-1', 'information', { tags: ['search'], enabled: true }),
      );
      registry.register(
  ...
  describe('isAvailable()', () => {
    it('returns true for a registered and enabled tool', () => {
      registry.register(makeTool('avail-tool', 'information', { enabled: true }));
  ...
  describe('search()', () => {
    ...
    it('combines keyword + category filter', () => {
      const results = registry.search({ keyword: 'search', category: 'information' });
      expect(results).toHaveLength(1);
