...
import { ITool, ToolCategory, ToolContext, ToolResult } from '../../abstractions/tool.interface';
...
function makeTool(id: string = 'test-tool', defaultTimeout?: number): ITool {
  return {
    ...
    toCompactSummary: () => ({ id, name: `Tool ${id}`, brief: 'Test', category: 'information' as ToolCategory }),
  };
}
...
describe('TimeoutMiddleware', () => {
  ...
  describe('wrapExecution() — executor throws', () => {
    it('resolves with error result when executor rejects', async () => {
      ...
      const result = await mw.wrapExecution(
        tool,
        undefined,
        context,
