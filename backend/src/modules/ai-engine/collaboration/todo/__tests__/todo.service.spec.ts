...
describe("TodoService", () => {
  ...
  describe("create", () => {
    ...
    it("未指定 priority 时默认设置为 medium", async () => {
      prisma.engineTodo.create.mockResolvedValue(buildDbTodo({ priority: "medium" }));
  ...
  describe("getStats", () => {
    ...
    it("各状态计数正确计算", async () => {
      prisma.engineTodo.findMany.mockResolvedValue([
        { status: "pending", priority: "low", dueDate: null, completedAt: null },
        { status: "in_progress", priority: "medium", dueDate: null, completedAt: null },
        { status: "completed", priority: "high", dueDate: null, completedAt: new Date() },
        { status: "cancelled", priority: "urgent", dueDate: null, completedAt: null },
        { status: "blocked", priority: "low", dueDate: null, completedAt: null },
      ]);
    ...
    it("各 priority 计数正确计算", async () => {
      prisma.engineTodo.findMany.mockResolvedValue([
        { status: "pending", priority: "low", dueDate: null, completedAt: null },
        { status: "pending", priority: "medium", dueDate: null, completedAt: null },
        { status: "pending", priority: "high", dueDate: null, completedAt: null },
        { status: "pending", priority: "urgent", dueDate: null, completedAt: null },
      ]);
    ...
    it("已完成或已取消的 Todo 不计入 overdue", async () => {
      ...
      prisma.engineTodo.findMany.mockResolvedValue([
        { status: "completed", priority: "medium", dueDate: pastDate, completedAt: new Date() },
        { status: "cancelled", priority: "medium", dueDate: pastDate, completedAt: null },
      ]);
  ...
  describe("batchUpdateStatus", () => {
    ...
    it("模型未使用时返回空数组", async () => {
      const svc = createServiceWithoutModel();
      const result = await svc.batchUpdateStatus(
        ["t1"],
        "completed",
        "user-1",
      );
  ...
  describe("update 的 description 字段", () => {
    ...
    it("指定 assigneeId 时包含在数据中", async () => {
      ...
      await service.update("todo-001", { assigneeId: "new-assignee" }, "user-1");
