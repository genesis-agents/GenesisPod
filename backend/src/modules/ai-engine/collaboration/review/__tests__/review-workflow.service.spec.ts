...
describe("ReviewWorkflowService", () => {
  ...
  describe("assignReviewer", () => {
    it("分配审阅者并发出 review.assigned 事件", async () => {
      ...
      const updatedRecord = buildDbReview({ reviewerId: "new-reviewer", version: 2 });
  ...
  describe("startReview", () => {
    it("将状态更新为 in_progress", async () => {
      ...
      const updatedRecord = buildDbReview({ status: "in_progress", version: 2 });
      (prisma.review as Record<string, jest.Mock>).update.mockResolvedValue(
        updatedRecord,
      );
    ...
    it("模型未使用时抛出 NotFoundException", async () => {
      const svc = createServiceWithoutModel();
      await expect(
        svc.startReview("review-001", "reviewer"),
      ).rejects.toThrow(NotFoundException);
    });
  ...
  describe("submitFeedback", () => {
    it("approve 反馈使状态变为 approved", async () => {
      ...
      (prisma.review as Record<string, jest.Mock>).update.mockResolvedValue(
        buildDbReview({ status: "approved", version: 2, completedAt: new Date() }),
      );
    ...
    it("reject 反馈使状态变为 rejected", async () => {
      ...
      (prisma.review as Record<string, jest.Mock>).update.mockResolvedValue(
        buildDbReview({ status: "rejected", version: 2, completedAt: new Date() }),
      );
  ...
  describe("cancelReview", () => {
    it("取消 review（更新为 rejected 状态）", async () => {
      ...
      const result = await service.cancelReview("review-001", "admin", "No longer needed");
      expect(result.status).toBe("rejected");
    });
    ...
    it("乐观锁冲突时抛出 ConflictException", async () => {
      ...
      await expect(
        service.cancelReview("review-001", "admin"),
      ).rejects.toThrow(ConflictException);
    });
  ...
  describe("reopenReview", () => {
    ...
    it("乐观锁冲突时抛出 ConflictException", async () => {
      ...
      await expect(
        service.reopenReview("review-001", "admin"),
      ).rejects.toThrow(ConflictException);
    });
  ...
  describe("各方法的非 P2025 错误重新抛出", () => {
    ...
    it("updateStatus：rejected 状态也设置 completedAt", async () => {
      ...
      const result = await service.updateStatus("review-001", "rejected", "admin");
      expect(result.status).toBe("rejected");
    });
