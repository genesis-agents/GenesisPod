...
import { VoteRequest, VoteOption } from "../../abstractions/collaborator.interface";
  ...
  describe("closeVote", () => {
    ...
    describe("ranked 策略", () => {
      it("第一偏好票数最多者为 winner", () => {
        ...
        manager.castVote(session.id, "voter-1", "option-1", { rank: [0, 1, 2] });
        manager.castVote(session.id, "voter-2", "option-1", { rank: [0, 2, 1] });
        manager.castVote(session.id, "voter-3", "option-2", { rank: [1, 0, 2] });
        const result = manager.closeVote(session.id, 3);
