...
import { EngineEventEmitterService } from "../engine-event-emitter.service";
import type {
  ProgressEvent,
  RoomConfig,
...
describe("ProgressTrackerService", () => {
  ...
  describe("cleanup", () => {
    ...
    it("should not remove recently completed tasks", () => {
      ...
      const removed = service.cleanup(new Date(Date.now() - 2 * 60 * 60 * 1000));
      expect(removed).toBe(0);
    });
...
describe("calculateOverallProgress", () => {
  ...
  it("should return 0 when total weight is 0", () => {
    const phases = [
      { id: "p1", name: "P1", order: 0, weight: 0, status: "completed" as const },
    ];
  ...
  it("should return 100 when all phases completed", () => {
    const phases = [
      { id: "p1", name: "P1", order: 0, weight: 1, status: "completed" as const },
      { id: "p2", name: "P2", order: 1, weight: 1, status: "completed" as const },
    ];
  ...
  it("should ignore in_progress phases (partial progress handled elsewhere)", () => {
    const phases = [
      { id: "p1", name: "P1", order: 0, weight: 1, status: "in_progress" as const },
