...
import {
  ...
  A2APriority,
  A2AMessageHandler,
} from "../abstractions/a2a-message.interface";
...
export class A2AMessageBusService {
  private readonly logger = new Logger(A2AMessageBusService.name);
  ...
  subscribe(
    ...
    this.logger.debug(
      `[A2A] Agent ${agentId} subscribed in session ${sessionId}`,
    );
  ...
  async publish<TPayload = unknown>(params: {
    ...
    if (!sessionSubs) {
      this.logger.debug(
        `[A2A] No subscribers in session ${params.sessionId} for message from ${params.fromAgentId}`,
      );
      return message;
    }
      ...
      try {
        ...
        if (result instanceof Promise) {
          result.catch((err: unknown) => {
            this.logger.error(
              `[A2A] Handler error for message ${message.id}: ${err}`,
            );
          });
        }
      } catch (err) {
        this.logger.error(
          `[A2A] Sync handler error for message ${message.id}: ${err}`,
        );
      }
    }

    this.logger.debug(
      `[A2A] ${params.fromAgentId} → ${params.toAgentId ?? "broadcast"} [${params.type}] delivered to ${deliverTo.length} handler(s)`,
    );
  ...
  clearSession(sessionId: string): void {
    ...
    this.logger.debug(`[A2A] Session ${sessionId} cleared`);
  }
