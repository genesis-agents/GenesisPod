...
describe("MemoryBridge", () => {
  ...
  it("preExecute returns envelope unchanged when userId is missing", async () => {
    ...
    const bridge = new MemoryBridge(coordinator as any);
  ...
  it("preExecute injects recalled fragments as a reminder", async () => {
    const coordinator = {
      recall: jest.fn(async () => ({
        fragments: [
          { layer: 3 as const, key: "pref.lang", value: "English", relevanceScore: 0.9, type: "preference" },
          { layer: 1 as const, key: "last-topic", value: "AI agents", relevanceScore: 0.7, type: "conversation" },
        ],
    ...
    expect(result.reminders[0].source).toBe("memory-bridge");
