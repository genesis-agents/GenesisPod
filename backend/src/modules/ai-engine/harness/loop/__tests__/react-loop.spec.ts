...
function mkToolRegistry(tools: Record<string, { success: boolean; data?: unknown; error?: string }>) {
  return {
...
describe("ReActLoop (Phase 2)", () => {
  ...
  it("falls back to finalize when LLM output is not JSON", async () => {
    ...
    const events = await drain(loop.run(makeEnvelope(), criteria, { agentId: "a1" }));
  ...
  it("returns tool error as recoverable; loop continues to next iteration", async () => {
    ...
    const events = await drain(loop.run(makeEnvelope(["missing"]), criteria, { agentId: "a1" }));
  ...
  it("PreToolUse hook can block tool execution", async () => {
    ...
    const events = await drain(loop.run(makeEnvelope(["calc"]), criteria, { agentId: "a1" }));
  ...
  it("terminates with reason budget after maxIterations", async () => {
    ...
    const events = await drain(
      loop.run(makeEnvelope(["noop"]), { maxIterations: 2, terminateOn: ["finalize"] }, {
        agentId: "a1",
      }),
    );
    const terminated = events.find((e) => e.type === "terminated");
