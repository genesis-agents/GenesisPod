...
describe('InMemoryCheckpointStore', () => {
  ...
  it('should return latest checkpoint', async () => {
    const now = Date.now();
    await store.save({ id: 'cp-1', executionId: 'exec-1', workflowId: 'wf', stepId: 's1', context: makeContext(), timestamp: new Date(now) });
    await store.save({ id: 'cp-2', executionId: 'exec-1', workflowId: 'wf', stepId: 's2', context: makeContext(), timestamp: new Date(now + 1000) });
  ...
  it('should delete a checkpoint by id', async () => {
    await store.save({ id: 'cp-1', executionId: 'exec-1', workflowId: 'wf', stepId: 's1', context: makeContext(), timestamp: new Date() });
  ...
  it('should delete all checkpoints by executionId', async () => {
    await store.save({ id: 'cp-1', executionId: 'exec-1', workflowId: 'wf', stepId: 's1', context: makeContext(), timestamp: new Date() });
    await store.save({ id: 'cp-2', executionId: 'exec-1', workflowId: 'wf', stepId: 's2', context: makeContext(), timestamp: new Date() });
    await store.save({ id: 'cp-3', executionId: 'exec-2', workflowId: 'wf', stepId: 's3', context: makeContext(), timestamp: new Date() });
...
describe('CheckpointManager', () => {
  ...
  it('should create a checkpoint and return it', async () => {
    ...
    const checkpoint = await manager.createCheckpoint('exec-1', 'wf-1', 'step-1', ctx);
  ...
  it('should retrieve a checkpoint by id', async () => {
    const ctx = makeContext();
    const created = await manager.createCheckpoint('exec-1', 'wf-1', 'step-1', ctx);
  ...
  it('should get latest checkpoint', async () => {
    const ctx = makeContext();
    await manager.createCheckpoint('exec-1', 'wf-1', 'step-1', ctx);
    const second = await manager.createCheckpoint('exec-1', 'wf-1', 'step-2', ctx);
  ...
  it('should cleanup expired checkpoints by TTL', async () => {
    ...
    const ctx = makeContext();
    const created = await mgr.createCheckpoint('exec-1', 'wf-1', 'step-1', ctx);
