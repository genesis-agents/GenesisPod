...
describe('SearchService', () => {
  ...
  const mockAxiosResponse = (data: unknown) =>
    of({ data, status: 200, statusText: 'OK', headers: {}, config: {} as unknown });

  // Helper: build a rejecting Observable
  const mockAxiosError = (status: number, message = 'API Error') => {
  ...
  const setupDefaultConfig = (
    ...
    secretsService.getValueInternal.mockImplementation((name: string) => {
      if (name.includes('TAVILY') || name.includes('tavily')) return Promise.resolve(tavilySecret);
      if (name.includes('SERPER') || name.includes('serper')) return Promise.resolve(serperSecret);
      return Promise.resolve(null);
    });

    prisma.systemSetting.findFirst.mockImplementation(
      async (args: { where: { key: string } }) => {
        if (args.where.key === 'search.provider') return { value: JSON.stringify(provider) };
        if (args.where.key === 'search.enabled') return { value: JSON.stringify(enabled) };
        return null;
      },
    );
  };
  ...
  describe('formatResultsForContext', () => {
    ...
    it('formats results with numbering', () => {
      const results = [
        { title: 'Article 1', url: 'https://example.com', content: 'Summary 1' },
  ...
  describe('fetchUrlContent', () => {
    it('successfully fetches and parses HTML content', async () => {
      ...
      (httpService.get as jest.Mock).mockReturnValue(mockAxiosResponse(html));
    ...
    it('strips script and style tags', async () => {
      const html = '<html><head><title>T</title><script>alert("xss")</script></head><body><style>.a{color:red}</style><p>Content</p></body></html>';
      (httpService.get as jest.Mock).mockReturnValue(mockAxiosResponse(html));
    ...
    it('decodes HTML entities', async () => {
      const html = '<html><head><title>T</title></head><body><p>Tom &amp; Jerry &lt;3&gt;</p></body></html>';
      (httpService.get as jest.Mock).mockReturnValue(mockAxiosResponse(html));
    ...
    it('uses longer timeout for PDF URLs', async () => {
      const html = '<html><head><title>PDF</title></head><body>content</body></html>';
      (httpService.get as jest.Mock).mockReturnValue(mockAxiosResponse(html));
    ...
    it('returns failure on HTTP error', async () => {
      (httpService.get as jest.Mock).mockReturnValue(
        new (require('rxjs').Observable)((subscriber: { error: (e: Error) => void }) => {
          subscriber.error(Object.assign(new Error(), { response: { status: 404, statusText: 'Not Found' } }));
        }),
      );
    ...
    it('handles non-string response', async () => {
      (httpService.get as jest.Mock).mockReturnValue(mockAxiosResponse({ json: 'data' }));
  ...
  describe('fetchUrlsForContext', () => {
    ...
    it('returns formatted context when URLs succeed', async () => {
      const html = '<html><head><title>Article</title></head><body>Summary text here</body></html>';
      (httpService.get as jest.Mock).mockReturnValue(mockAxiosResponse(html));
  ...
  describe('search - disabled config', () => {
    it('falls back to duckduckgo when search is disabled', async () => {
      ...
      mockDuckSearch.mockResolvedValue({
        noResults: false,
        results: [{ title: 'DDG Result', url: 'https://ddg.com', description: 'DDG content', hostname: 'ddg.com' }],
      });
  ...
  describe('search - tavily', () => {
    ...
    it('returns successful results from Tavily', async () => {
      (httpService.post as jest.Mock).mockReturnValue(
        mockAxiosResponse({
          results: [
            { title: 'Tavily Result', url: 'https://news.com/article', content: 'Article content here', score: 0.9 },
          ],
        }),
      );
    ...
    it('failovers to duckduckgo when Tavily fails with 429', async () => {
      (httpService.post as jest.Mock).mockReturnValue(
        new (require('rxjs').Observable)((subscriber: { error: (e: Error) => void }) => {
          subscriber.error(Object.assign(new Error('Rate limited'), { response: { status: 429 } }));
        }),
      );
      mockDuckSearch.mockResolvedValue({
        noResults: false,
    ...
    it('failovers to duckduckgo on network error', async () => {
      ...
      mockDuckSearch.mockResolvedValue({
        noResults: false,
        results: [{ title: 'DDG', url: 'https://ddg.com', description: 'content', hostname: 'ddg.com' }],
      });
    ...
    it('returns failure when all providers fail', async () => {
      (httpService.post as jest.Mock).mockReturnValue(
        new (require('rxjs').Observable)((subscriber: { error: (e: Error) => void }) => {
          subscriber.error(Object.assign(new Error('API error'), { response: { status: 500 } }));
        }),
      );
    ...
    it('applies diversity filter limiting results per domain', async () => {
      ...
      const results = [
        { title: 'A1', url: 'https://same-domain.com/a1', content: 'Content '.repeat(50), score: 0.9 },
        { title: 'A2', url: 'https://same-domain.com/a2', content: 'Content '.repeat(50), score: 0.8 },
        { title: 'A3', url: 'https://same-domain.com/a3', content: 'Content '.repeat(50), score: 0.7 },
        { title: 'A4', url: 'https://same-domain.com/a4', content: 'Content '.repeat(50), score: 0.6 },
        { title: 'B1', url: 'https://other1.com/b1', content: 'Content '.repeat(50), score: 0.85 },
        { title: 'C1', url: 'https://other2.com/c1', content: 'Content '.repeat(50), score: 0.75 },
      ];
      (httpService.post as jest.Mock).mockReturnValue(
        mockAxiosResponse({ results }),
      );
      ...
      const sameDomainCount = result.results.filter(
        (r) => r.url?.includes('same-domain.com'),
      ).length;
  ...
  describe('search - serper', () => {
    ...
    it('returns results from Serper', async () => {
      (httpService.post as jest.Mock).mockReturnValue(
        mockAxiosResponse({
          organic: [
            { title: 'Serper Result', link: 'https://google.com/result', snippet: 'Result snippet' },
          ],
        }),
      );
    ...
    it('passes X-API-KEY header to Serper', async () => {
      ...
      expect(httpService.post).toHaveBeenCalledWith(
        ...
        expect.any(Object),
        expect.objectContaining({ headers: expect.objectContaining({ 'X-API-KEY': 'serper-api-key-1' }) }),
      );
    });
  ...
  describe('search - duckduckgo', () => {
    ...
    it('returns results from DuckDuckGo', async () => {
      mockDuckSearch.mockResolvedValue({
        noResults: false,
        results: [
          { title: 'DDG Result', url: 'https://example.com', description: 'Desc', hostname: 'example.com' },
        ],
      });
  ...
  describe('search - key rotation', () => {
    it('rotates to next key after one fails with 429', async () => {
      ...
      (httpService.post as jest.Mock).mockImplementation(() => {
        callCount++;
        if (callCount === 1) {
          return new (require('rxjs').Observable)((subscriber: { error: (e: Error) => void }) => {
            subscriber.error(Object.assign(new Error('Rate limited'), { response: { status: 429 } }));
          });
        }
        return mockAxiosResponse({
          results: [{ title: 'Result', url: 'https://good.com', content: 'content' }],
        });
      });
    ...
    it('falls back to duckduckgo when all keys exhausted', async () => {
      ...
      mockDuckSearch.mockResolvedValue({
        noResults: false,
        results: [{ title: 'DDG', url: 'https://ddg.com', description: 'ddg', hostname: 'ddg.com' }],
      });
  ...
  describe('search - env var fallback', () => {
    it('uses env vars when secret manager throws', async () => {
      ...
      secretsService.getValueInternal.mockRejectedValue(new Error('Secret manager down'));
  ...
  describe('getKeyHealthStatus', () => {
    ...
    it('shows unhealthy key after failure and cooldown not elapsed', async () => {
      ...
      (httpService.post as jest.Mock).mockReturnValue(
        new (require('rxjs').Observable)((subscriber: { error: (e: Error) => void }) => {
          subscriber.error(Object.assign(new Error('Quota'), { response: { status: 429 } }));
        }),
      );
      mockDuckSearch.mockResolvedValue({ noResults: false, results: [] });
  ...
  describe('result ranking', () => {
    it('ranks high-authority domains higher', async () => {
      setupDefaultConfig({ provider: 'tavily', tavilySecret: 'tvly-test-key1234' });

      (httpService.post as jest.Mock).mockReturnValue(
        mockAxiosResponse({
          results: [
            { title: 'Low authority', url: 'https://randomsite.net/article', content: 'AI news test content here', score: 0.5 },
            { title: 'High authority', url: 'https://nature.com/article', content: 'AI news test content here', score: 0.5 },
          ],
        }),
      );
      ...
      const highIdx = result.results.findIndex((r) => r.url?.includes('nature.com'));
      const lowIdx = result.results.findIndex((r) => r.url?.includes('randomsite.net'));
    ...
    it('uses raw score from Tavily in relevance scoring', async () => {
      ...
      (httpService.post as jest.Mock).mockReturnValue(
        mockAxiosResponse({
          results: [
            { title: 'Low score', url: 'https://a.com', content: 'content', score: 0.1 },
            { title: 'High score', url: 'https://b.com', content: 'content', score: 0.9 },
          ],
        }),
      );
    ...
    it('includes freshness scoring for recent content', async () => {
      ...
      (httpService.post as jest.Mock).mockReturnValue(
        mockAxiosResponse({
          results: [
            { title: 'Old article', url: 'https://old.com', content: 'test query content here', published_date: oldDate },
            { title: 'Recent article', url: 'https://recent.com', content: 'test query content here', published_date: recentDate },
          ],
        }),
      );
  ...
  describe('search - config auto-switching', () => {
    it('auto-switches to serper when tavily keys empty and serper has keys', async () => {
      ...
      (httpService.post as jest.Mock).mockReturnValue(
        mockAxiosResponse({ organic: [{ title: 'Serper', link: 'https://s.com', snippet: 'result' }] }),
      );
    ...
    it('auto-switches to tavily when serper keys empty and tavily has keys', async () => {
      ...
      (httpService.post as jest.Mock).mockReturnValue(
        mockAxiosResponse({ results: [{ title: 'Tavily', url: 'https://t.com', content: 'content' }] }),
      );
