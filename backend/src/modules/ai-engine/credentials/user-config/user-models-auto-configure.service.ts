...
} from "../../llm/selection/default-recommendations";
...
export class AutoConfigureService {
  ...
  async runForUser(userId: string): Promise<AutoConfigureResult> {
    ...
    const activePersonal = personalKeys.filter(
      (k: any) => k.isActive && k.mode === "personal",
    );
    ...
    const existingKeys = new Set(
      existing.map(
        (c: any) => `${c.provider}:${c.modelId.toLowerCase()}:${c.modelType}`,
      ),
    );
    const defaultedTypes = new Set<AIModelType>();
    existing
      .filter((c: any) => c.isEnabled && c.isDefault)
      .forEach((c: any) => defaultedTypes.add(c.modelType));
    ...
    for (const [modelTypeStr, preferredProviders] of Object.entries(
      ...
      for (const provider of preferredProviders) {
        ...
        const rec = providerRecs.find((r: any) => r.modelType === modelType);
