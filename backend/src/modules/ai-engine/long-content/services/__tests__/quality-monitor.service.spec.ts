...
const mockAiChatService = {
  chat: jest.fn().mockResolvedValue({ content: AI_JSON_RESPONSE, tokensUsed: 50 }),
};
...
describe("QualityMonitorService", () => {
  ...
  describe("updateTotalTasks", () => {
    ...
    it("should warn but not throw for unknown project", () => {
      expect(() =>
        service.updateTotalTasks("nonexistent", 10),
      ).not.toThrow();
    });
  });
  ...
  describe("evaluateTask", () => {
    ...
    it("should add +2 when word count meets minimum", async () => {
      ...
      const metrics = await service.evaluateTask(content, expected, NO_AI_CONFIG);
    ...
    it("should set completion ratio to 1 when no minWords specified", async () => {
      const metrics = await service.evaluateTask("some content.", {}, NO_AI_CONFIG);
      expect(metrics.completionRatio).toBe(1);
    });
    ...
    it("should include AI scores when aiEvaluation enabled", async () => {
      const content = "a".repeat(100) + "。";
      const metrics = await service.evaluateTask(content, expected, DEFAULT_QUALITY_MONITOR_CONFIG);
  ...
  describe("updateTrend", () => {
    ...
    it("should throw for unknown project", () => {
      expect(() =>
        service.updateTrend("unknown", "t1", "Task 1", sampleMetrics, NO_AI_CONFIG),
      ).toThrow("Project not found");
    });

    it("should increment completedTasks on first call for a task", () => {
      service.updateTrend("proj-1", "t1", "Task 1", sampleMetrics, NO_AI_CONFIG);
      const dashboard = service.getDashboard("proj-1");
    ...
    it("should NOT double-count revisited tasks", () => {
      service.updateTrend("proj-1", "t1", "Task 1", sampleMetrics, NO_AI_CONFIG);
      service.updateTrend("proj-1", "t1", "Task 1", { ...sampleMetrics, overallScore: 9 }, NO_AI_CONFIG);
      const dashboard = service.getDashboard("proj-1");
      expect(dashboard.progress.completedTasks).toBe(1);
    });
    ...
    it("should return stable trend with consistent scores", () => {
      for (let i = 0; i < 5; i++) {
        service.updateTrend("proj-1", `t${i}`, `Task ${i}`, sampleMetrics, NO_AI_CONFIG);
      }
      const trend = service.updateTrend("proj-1", "t5", "Task 5", sampleMetrics, NO_AI_CONFIG);
    ...
    it("should detect anomaly for low quality score", () => {
      ...
      const lowQualityAnomaly = dashboard.anomalies.find((a) => a.issue === "low_quality");
    ...
    it("should detect short_content anomaly", () => {
      ...
      const dashboard = service.getDashboard("proj-1");
      const shortAnomaly = dashboard.anomalies.find((a) => a.issue === "short_content");
    ...
    it("should detect incomplete anomaly for no structured end", () => {
      ...
      const dashboard = service.getDashboard("proj-1");
      const incompleteAnomaly = dashboard.anomalies.find((a) => a.issue === "incomplete");
      expect(incompleteAnomaly).toBeDefined();
    });

    it("should update word stats correctly after revision", () => {
      service.updateTrend("proj-1", "t1", "Task 1", { ...sampleMetrics, wordCount: 200 }, NO_AI_CONFIG);
      service.updateTrend("proj-1", "t1", "Task 1", { ...sampleMetrics, wordCount: 300 }, NO_AI_CONFIG);
      const dashboard = service.getDashboard("proj-1");
  ...
  describe("getInterventionRecommendation", () => {
    ...
    it("should return Level 1 (soft_reminder) for consecutive declines >= 2", () => {
      ...
      const result = service.getInterventionRecommendation("proj-1", trend, NO_AI_CONFIG);
  ...
  describe("getDashboard", () => {
    ...
    it("should throw for unknown project", () => {
      expect(() => service.getDashboard("unknown")).toThrow("Project not found");
    });
    ...
    it("should track word stats across multiple tasks", () => {
      service.updateTrend("proj-1", "t1", "T1", { ...sampleMetrics, wordCount: 100 }, NO_AI_CONFIG);
      service.updateTrend("proj-1", "t2", "T2", { ...sampleMetrics, wordCount: 200 }, NO_AI_CONFIG);
      const dash = service.getDashboard("proj-1");
