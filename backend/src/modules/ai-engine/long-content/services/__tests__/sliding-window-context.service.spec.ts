...
const mockAiChatService = {
  chat: jest.fn().mockResolvedValue({ content: "AI generated summary", tokensUsed: 30 }),
};
...
describe("SlidingWindowContextService", () => {
  ...
  beforeEach(async () => {
    ...
    service = module.get<SlidingWindowContextService>(SlidingWindowContextService);
  });
  ...
  describe("buildWorkingMemory", () => {
    ...
    it("should include currentTaskContent (possibly truncated)", async () => {
      ...
      expect(ctx.currentTaskContent.length).toBeLessThanOrEqual(longContent.length);
    });
    ...
    it("should compute token usage", async () => {
      const ctx = await service.buildWorkingMemory("p1", "t1", "some content here");
  ...
  describe("slideWindow", () => {
    ...
    it("should throw for unknown project", async () => {
      await expect(
        service.slideWindow("unknown", { id: "t1", title: "T1", result: "content" }),
      ).rejects.toThrow("Project not found");
    });

    it("should increment completedTaskCount", async () => {
      await service.slideWindow("p1", { id: "t1", title: "Task 1", result: "done." });
    ...
    it("should evict oldest summaries when exceeding recentTaskCount", async () => {
      ...
      for (let i = 0; i < 5; i++) {
        await service.slideWindow(
          "p1",
          { id: `t${i}`, title: `Task ${i}`, result: "done.", summary: `S${i}` },
          config,
        );
      }
  ...
  describe("retrieveRelevantHistory", () => {
    beforeEach(async () => {
      ...
      await service.slideWindow("p1", {
        ...
        result: "Machine learning is a type of artificial intelligence that allows computer systems to learn.",
    ...
    it("should return empty array when query has no keyword matches", async () => {
      const results = await service.retrieveRelevantHistory("p1", "cooking recipes");
      expect(results).toHaveLength(0);
    });
    ...
    it("should return matches limited by maxChunks", async () => {
      ...
      for (let i = 2; i <= 5; i++) {
        await service.slideWindow("p1", {
          ...
          result: "Machine learning machine systems learn data neural networks.",
          summary: `Summary ${i}`,
        });
      }
      const results = await service.retrieveRelevantHistory("p1", "Machine learning", { maxChunks: 2 });
  ...
  describe("clearProject", () => {
    it("should remove project context", async () => {
      ...
      await expect(
        service.buildWorkingMemory("p1", "t2", "x"),
      ).rejects.toThrow("Project not found");
    });
  ...
  describe("updateGlobalSummary", () => {
    it("should call AI to generate a new summary", async () => {
      service.initProject("p1", { title: "Novel", description: "A novel project" });
      await service.slideWindow("p1", {
        id: "t1",
        title: "Chapter 1",
        result: "Long content...",
