...
import { MemoryType } from "../../abstractions/memory.interface";
...
describe("InMemoryStore", () => {
  ...
  describe("search()", () => {
    beforeEach(async () => {
      await store.add({ type: "fact", content: "The capital of France is Paris" });
      await store.add({ type: "episode", content: "I visited Paris last year" });
      await store.add({ type: "summary", content: "A summary about technology trends" });
    });
  ...
  describe("cleanup()", () => {
    ...
    it("does not remove future-expiry entries", async () => {
      ...
      await store.add({ type: "fact", content: "future expiry", expiresAt: future });
...
describe("ConversationMemory", () => {
  ...
  describe("addMessage()", () => {
    ...
    it("preserves provided id and timestamp", async () => {
      ...
      await memory.addMessage({ id: "msg-1", role: "user", content: "Hi", timestamp: ts });
      const messages = await memory.getMessages();
