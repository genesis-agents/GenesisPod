...
describe("ShortTermMemoryService", () => {
  ...
  describe("appendWithSession()", () => {
    ...
    it("handles complex objects in appended values", async () => {
      await service.appendWithSession("sess", "events", { type: "click", x: 10 });
      await service.appendWithSession("sess", "events", { type: "scroll", y: 200 });
      const result = (await service.getWithSession("sess", "events")) as unknown[];
  ...
  describe("clearSession()", () => {
    ...
    it("does not throw when clearing non-existent session", async () => {
      await expect(service.clearSession("nonexistent")).resolves.toBeUndefined();
    });
  });
  ...
  describe("cleanup()", () => {
    it("removes expired entries and returns count", () => {
      ...
      service.setWithSession("sess", "k1", "v1", 1); // expires in 1s
      service.setWithSession("sess", "k2", "v2", 3600); // expires in 1h
      service.setWithSession("sess", "k3", "v3"); // no expiry
    ...
    it("removes empty sessions after cleanup", () => {
      ...
      service.setWithSession("empty-sess", "k", "v", 1);
      jest.advanceTimersByTime(2000);
    ...
    it("returns 0 when no expired items", () => {
      service.setWithSession("sess", "k", "v"); // no TTL
      const count = service.cleanup();
      expect(count).toBe(0);
    });
    ...
    it("handles multiple sessions with mixed expiry", () => {
      ...
      service.setWithSession("sess-a", "expired", "v", 1);
      service.setWithSession("sess-a", "valid", "v", 3600);
      service.setWithSession("sess-b", "expired", "v", 1);
  ...
  describe("LRU eviction behavior", () => {
    it("evicts oldest session when capacity is exceeded", async () => {
      ...
      const smallService = module.get<ShortTermMemoryService>(ShortTermMemoryService);
  ...
  describe("TTL edge cases", () => {
    ...
    it("preserves createdAt when updating existing key via setWithSession", async () => {
      ...
      const originalCreatedAt = (items1[0] as { createdAt?: Date }).createdAt;
