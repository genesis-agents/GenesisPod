...
AI Engine 是 DeepDive Engine 的核心基座层，提供统一的多 Agent 协作引擎能力。
...
```typescript
interface ITool<TInput, TOutput> extends IExecutable<TInput, TOutput, ToolContext> {
...
```typescript
interface ISkill<TInput, TOutput> extends IExecutable<TInput, TOutput, SkillContext> {
...
```typescript
interface IAgent<TInput, TOutput> extends IExecutable<TInput, TOutput, AgentContext> {
