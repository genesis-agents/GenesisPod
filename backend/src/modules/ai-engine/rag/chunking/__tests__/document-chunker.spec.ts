...
describe("DocumentChunker", () => {
  ...
  describe("chunkDocument() - basic structure", () => {
    ...
    it("returns at least one parent chunk for non-empty content", () => {
      const content = "This is a sentence. And another sentence. More text here.";
    ...
    it("sets metadata with processedAt timestamp", () => {
      const result = chunker.chunkDocument("doc-1", "Some content here.", "Test");
    ...
    it("sets totalChildChunks to sum of all child chunks", () => {
      const content = "Sentence one. Sentence two. More content here. Even more text.";
      const result = chunker.chunkDocument("doc-1", content, "Test");
  ...
  describe("chunkDocument() - child chunk properties", () => {
    ...
    it("child chunk parentPosition matches parent position", () => {
      const content = Array(50).fill("Child chunk test sentence here.").join(" ");
      const config: ChunkingConfig = {
        ...DEFAULT_CHUNKING_CONFIG,
        parentChunkSize: 100,
        parentChunkOverlap: 10,
    ...
    it("child chunks have sequential positions within each parent", () => {
      const content = Array(30).fill("A test sentence for child chunking.").join(" ");
      const config: ChunkingConfig = {
        ...DEFAULT_CHUNKING_CONFIG,
        parentChunkSize: 200,
        childChunkSize: 50,
  ...
  describe("chunkDocument() - large document splitting", () => {
    it("creates multiple parent chunks for large content", () => {
      ...
      const content = Array(200).fill("This is a sentence with some reasonable length for testing.").join(" ");
  ...
  describe("chunkDocument() - custom config", () => {
    ...
    it("respects custom parentChunkSize", () => {
      const content = Array(100).fill("Token count test sentence here now.").join(" ");
      ...
      const smallResult = chunker.chunkDocument("doc-1", content, "Test", smallConfig);
      const largeResult = chunker.chunkDocument("doc-1", content, "Test", largeConfig);
  ...
  describe("edge cases", () => {
    it("handles content with only whitespace", () => {
      const result = chunker.chunkDocument("doc-1", "   \n  \t  ", "Whitespace");
      expect(result.parentChunks).toHaveLength(0);
    });

    it("handles single very long sentence", () => {
      const longSentence = "word ".repeat(500).trim();
      const result = chunker.chunkDocument("doc-1", longSentence, "Long Sentence");
      expect(result.parentChunks.length).toBeGreaterThan(0);
    });
    ...
    it("handles Chinese-only content", () => {
      const content = "人工智能正在改变我们的生活方式。机器学习是其核心技术之一。深度学习取得了巨大突破。";
