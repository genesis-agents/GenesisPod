...
describe("EmbeddingService", () => {
  ...
  describe("getEmbeddingConfig()", () => {
    ...
    it("resolves API key from SecretsService when secretKey is set", async () => {
      ...
      mockSecretsService.getValueInternal.mockResolvedValue("resolved-secret-key");

      const config = await service.getEmbeddingConfig();

      expect(mockSecretsService.getValueInternal).toHaveBeenCalledWith("my-secret-key");
  ...
  describe("generateEmbeddings()", () => {
    ...
    it("processes texts in batches of 96 for Cohere", async () => {
      ...
      mockAiApiCallerService.callCohereEmbeddingAPI
        .mockResolvedValueOnce({ embeddings: Array(96).fill([0.1]), totalTokens: 500 })
        .mockResolvedValueOnce({ embeddings: Array(96).fill([0.1]), totalTokens: 500 })
        .mockResolvedValueOnce({ embeddings: Array(8).fill([0.1]), totalTokens: 40 });

      await service.generateEmbeddings(texts);

      // 200 texts / 96 per batch = 3 calls (ceil(200/96) = 3)
      expect(mockAiApiCallerService.callCohereEmbeddingAPI).toHaveBeenCalledTimes(3);
    });
    ...
    it("throws when API call fails", async () => {
      ...
      await expect(service.generateEmbeddings(["test"])).rejects.toThrow("API error");
    });

    it("accumulates totalTokens across batches", async () => {
      const texts = Array.from({ length: 150 }, (_, i) => `text ${i}`);
      mockAiApiCallerService.callOpenAICompatibleEmbeddingAPI
        .mockResolvedValueOnce({ embeddings: Array(100).fill([0.1]), totalTokens: 1000 })
        .mockResolvedValueOnce({ embeddings: Array(50).fill([0.1]), totalTokens: 500 });
