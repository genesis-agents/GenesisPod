...
describe("WebhookDispatcherService", () => {
  ...
  describe("dispatch", () => {
    it("should dispatch webhook to matching subscriptions", async () => {
      // Arrange
      (prismaService.webhookSubscription.findMany as jest.Mock).mockResolvedValue([mockSubscription]);
      (prismaService.webhookDelivery.create as jest.Mock).mockResolvedValue(mockDelivery);
      (prismaService.webhookDelivery.findUnique as jest.Mock).mockResolvedValue(mockDelivery);
    ...
    it("should not dispatch if no matching subscriptions found", async () => {
      // Arrange
      (prismaService.webhookSubscription.findMany as jest.Mock).mockResolvedValue([]);
  ...
  describe("attemptDelivery", () => {
    it("should mark delivery as success on successful HTTP response", async () => {
      ...
      (prismaService.webhookSubscription.update as jest.Mock).mockResolvedValue(mockSubscription);
      (global.fetch as jest.Mock).mockResolvedValue({
        ok: true,
        status: 200,
  ...
  describe("sendWebhook", () => {
    it("should send webhook with correct headers and signature", async () => {
      ...
      // Act
      const result = await service["sendWebhook"](mockSubscription, payload as any);
    ...
    it("should handle HTTP error responses", async () => {
      ...
      // Act
      const result = await service["sendWebhook"](mockSubscription, mockDelivery.payload as any);
  ...
  describe("processRetryQueue", () => {
    it("should process pending retries", async () => {
      ...
      (prismaService.webhookDelivery.findMany as jest.Mock).mockResolvedValue([pendingDelivery]);
      (prismaService.webhookDelivery.findUnique as jest.Mock).mockResolvedValue(pendingDelivery);
      (prismaService.webhookDelivery.update as jest.Mock).mockResolvedValue({
        ...pendingDelivery,
        attemptCount: 1,
      });
