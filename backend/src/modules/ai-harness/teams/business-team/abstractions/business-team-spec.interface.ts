 ...
 * 业务方（playground / research / writing / TI / ...）实现 BusinessAgentTeamSpec
 ...
 *   - playground 是 reference 实现：satisfies 本规约 via structural typing
 *
 ...
 *   - eventNamespace: 事件 type 字符串前缀（如 "agent-playground" / "research"）
...
export interface BusinessAgentTeamSpec<TInput = unknown> {
   ...
   *
   * 命名约定：lowercase + kebab-case，与 module key 一致（如 "agent-playground"）。
   */
