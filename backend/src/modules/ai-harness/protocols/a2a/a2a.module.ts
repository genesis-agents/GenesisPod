 ...
 * 将 GenesisPod 暴露为 A2A 兼容的 Agent，让外部 AI Agent 可以发现和调用 GenesisPod 能力。
 *
 * Controller (A2AController) 迁移至 open-api/a2a/a2a-server.controller.ts (PR-X17)。
...
@Module({
  ...
  providers: [
    ...
    A2ATaskStore,
    // DI token bindings: A2AController (in open-api/a2a/a2a-server.controller.ts) injects via token
    {
