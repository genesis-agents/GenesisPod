...
import { SlidesOrchestratorService } from "./slides-orchestrator.service";
...
import {
  NaturalEditService,
  EditResult,
} from "../editing/natural-edit.service";
import {
  SlidesVersionService,
  VersionInfo,
  VersionDiff,
  RollbackResult,
} from "../editing/slides-version.service";
import { ContentExtractorService } from "../../../../../common/content-processing";
import { TemplateMatcher } from "../planning/template-matcher.service";
import { QualityCheckService } from "../quality/quality-check.service";
import { SourceAnalysisService } from "../generation/source-analysis.service";
import { BatchOperationService } from "../editing/batch-operation.service";
import {
  PPTGenerationInput,
  PPTDocument,
  PPTStreamEvent,
...
export class SlidesController {
  ...
  constructor(
    ...
    private readonly slidesExport: SlidesExportService,
    private readonly naturalEdit: NaturalEditService,
    private readonly versionService: SlidesVersionService,
  ...
  async exportPPT(
    ...
    try {
      ...
      switch (dto.format) {
        case "pptx":
          // 使用增强的 PPTX 导出服务
          result = await this.slidesExport.exportToPPTX(document);
          break;

        case "pdf":
          // PDF 导出
          result = await this.slidesExport.exportToPDF(document);
          break;

        case "png":
          // PNG 图片导出（ZIP 压缩包）
          result = await this.slidesExport.exportToPNG(document);
          break;
