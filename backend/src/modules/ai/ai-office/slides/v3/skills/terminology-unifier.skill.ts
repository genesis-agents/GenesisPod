/**
 * Slides Engine v3.0 - Terminology Unifier Skill
 *
 * 术语统一技能 (Layer 5)：检测并统一演示文稿中的术语使用
 * 确保同一概念在整个演示中使用一致的术语
 */
...
export class TerminologyUnifierSkill {
  ...
  async check(
    ...
    this.logger.log(
      `[check] Checking terminology across ${pages.length} pages`,
    );
    ...
    if (ruleBasedVariations.length > 0 || pages.length > 5) {
      return this.checkWithAI(pages, ruleBasedVariations, sessionId);
    }
    ...
    return {
      ...
      consistencyScore: this.calculateScore(ruleBasedVariations, pages.length),
      checkedAt: new Date(),
    };
  }
