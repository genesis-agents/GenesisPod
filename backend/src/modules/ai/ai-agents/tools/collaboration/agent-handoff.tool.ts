...
> {
  ...
  readonly type = ToolType.AGENT_HANDOFF;
  ...
  constructor() {
    super();
    this.defaultTimeout = 310000; // 稍大于默认任务超时时间
  }

  /**
   * 验证输入
   */
  validateInput(input: AgentHandoffInput): boolean {
    // 验证目标 Agent
    if (!Object.values(AgentType).includes(input.targetAgent)) {
