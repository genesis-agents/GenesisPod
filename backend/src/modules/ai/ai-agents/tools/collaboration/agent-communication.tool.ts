...
> {
  ...
  readonly type = "agent_communication" as ToolType;
  ...
  private inboxes: Map<AgentType, string[]> = new Map(); // Agent -> messageIds

  constructor() {
    super();
    this.defaultTimeout = 5000; // 5 秒超时

    // 初始化收件箱
    Object.values(AgentType).forEach((agentType) => {
      this.inboxes.set(agentType, []);
    });
  }
   ...
   * 验证输入
   */
  validateInput(input: AgentCommunicationInput): boolean {
    // 验证操作类型
    ...
    // 验证发送者 Agent
    if (!Object.values(AgentType).includes(input.fromAgent)) {
      return false;
    }
    ...
    switch (input.operation) {
      case CommunicationOperation.SEND:
        return (
          !!input.toAgent &&
          Object.values(AgentType).includes(input.toAgent) &&
