...
describe('SchemaValidator', () => {
  ...
  describe('String Validation', () => {
    ...
    it('should validate uri format', () => {
      ...
      expect(validator.validate('ftp://files.example.com', schema).valid).toBe(true);
  ...
  describe('Complex Tool Schema', () => {
    it('should validate web search input schema', () => {
      ...
      expect(validator.validate({ query: 'test', numResults: 5 }, schema).valid).toBe(true);
      expect(validator.validate({ query: 'test', language: 'zh-CN' }, schema).valid).toBe(true);
      ...
      expect(validator.validate({ query: 'test', numResults: 20 }, schema).valid).toBe(false); // Over max
      expect(validator.validate({ query: 'test', language: 'fr-FR' }, schema).valid).toBe(false); // Invalid enum
    });
  });
