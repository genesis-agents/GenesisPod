...
describe("ToolError", () => {
  describe("Constructor", () => {
    ...
    it("should inherit retryable from error code meta", () => {
      const retryableError = new ToolError(
        ToolErrorCode.EXECUTION_TIMEOUT,
        "Timeout",
      );
      expect(retryableError.retryable).toBe(true);

      const nonRetryableError = new ToolError(
        ToolErrorCode.VALIDATION_ERROR,
        "Invalid",
      );
  ...
  describe("Static Factories", () => {
    it("should create validation error", () => {
      const error = ToolError.validation("Field is required", {
        field: "name",
      });
  ...
  describe("toJSON", () => {
    it("should convert to JSON", () => {
      ...
      expect(json.numericCode).toBe(
        TOOL_ERROR_CODES[ToolErrorCode.VALIDATION_ERROR].numericCode,
      );
...
describe("Error Code Metadata", () => {
  ...
  it("should have correct HTTP status codes", () => {
    expect(TOOL_ERROR_CODES[ToolErrorCode.VALIDATION_ERROR].httpStatus).toBe(
      400,
    );
    expect(TOOL_ERROR_CODES[ToolErrorCode.PERMISSION_DENIED].httpStatus).toBe(
      403,
    );
    expect(TOOL_ERROR_CODES[ToolErrorCode.RESOURCE_NOT_FOUND].httpStatus).toBe(
      404,
    );
    expect(TOOL_ERROR_CODES[ToolErrorCode.RATE_LIMIT_EXCEEDED].httpStatus).toBe(
      429,
    );
...
describe("Helper Functions", () => {
  ...
  describe("shouldRetry", () => {
    it("should return true when within retry limit", () => {
      // Use EXECUTION_ERROR which has maxRetries: 3
      const error = new ToolError(ToolErrorCode.EXECUTION_ERROR, "Error");
    ...
    it("should return false when at retry limit", () => {
      const error = new ToolError(ToolErrorCode.EXECUTION_TIMEOUT, "Timeout");
      const maxRetries =
        TOOL_ERROR_CODES[ToolErrorCode.EXECUTION_TIMEOUT].maxRetries!;
