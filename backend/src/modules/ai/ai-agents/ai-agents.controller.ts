...
export class AiAgentsController {
  ...
  async execute(
    ...
    // 创建任务记录
    const task = await this.aiAgentsService.createTask({
      userId,
  ...
  async downloadArtifact(
    ...
    return this.aiAgentsService.getArtifactDownload(artifactId);
  }
  ...
  private async executeTaskAsync(
    ...
    try {
      await this.aiAgentsService.updateTaskStatus(taskId, "PLANNING");
      ...
      for await (const event of this.orchestrator.execute(
        ...
        // 发布事件到 SSE 流
        this.aiAgentsService.publishEvent(taskId, event);

        // 更新任务状态
        if (event.type === "plan_ready") {
          await this.aiAgentsService.updateTaskStatus(taskId, "EXECUTING");
          await this.aiAgentsService.updateTaskPlan(taskId, event.plan);
        }

        if (event.type === "artifact") {
          await this.aiAgentsService.saveArtifact(taskId, event.artifact);
        }

        if (event.type === "complete") {
          await this.aiAgentsService.updateTaskStatus(taskId, "COMPLETED");
          await this.aiAgentsService.updateTaskResult(taskId, event.result);
        }

        if (event.type === "error") {
          await this.aiAgentsService.updateTaskStatus(
            taskId,
            "FAILED",
            event.error,
          );
