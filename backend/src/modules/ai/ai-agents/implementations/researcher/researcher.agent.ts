...
import { AiStudioChatService } from "../../../ai-studio/ai-studio-chat.service";
import { AiStudioSourceService } from "../../../ai-studio/ai-studio-source.service";
import { AiStudioOutputService } from "../../../ai-studio/ai-studio-output.service";
...
export class ResearcherAgent extends BaseAgent {
  ...
  async plan(input: AgentInput): Promise<AgentPlan> {
    ...
    if (input.options?.saveToMemory !== false) {
      steps.push({
        ...
        tool: ToolType.LONG_TERM_MEMORY,
        dependencies: [steps[steps.length - 1].id],
        estimatedDuration: 3000,
      });
    }
  ...
  private async executeStep(
    ...
    switch (step.tool) {
      ...
      case ToolType.KNOWLEDGE_GRAPH:
        // 构建知识图谱（简化实现）
        return {
          content: context.previousContent + "\n\n## 知识图谱\n[知识图谱数据]",
        };

      case ToolType.LONG_TERM_MEMORY:
        // 保存到长期记忆
        if (projectId) {
