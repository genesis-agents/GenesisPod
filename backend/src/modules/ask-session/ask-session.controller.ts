...
export class AskSessionController {
  private readonly logger = new Logger(AskSessionController.name);

  constructor(private readonly askSessionService: AskSessionService) {}
  ...
  async createSession(@Request() req: any, @Body() dto: CreateSessionDto) {
    ...
    return this.askSessionService.createSession(req.user.id, dto);
  }
  ...
  async getSessions(
    ...
    return this.askSessionService.getSessions(req.user.id, pageNum, limitNum);
  }
  ...
  async searchSessions(
    ...
    return this.askSessionService.searchSessions(req.user.id, query, limitNum);
  }
  ...
  async getSession(@Request() req: any, @Param("id") id: string) {
    return this.askSessionService.getSession(id, req.user.id);
  }
  ...
  async updateSession(
    ...
    return this.askSessionService.updateSession(id, req.user.id, dto);
  }
  ...
  async deleteSession(@Request() req: any, @Param("id") id: string) {
    return this.askSessionService.deleteSession(id, req.user.id);
  }
  ...
  async sendMessage(
    ...
    return this.askSessionService.sendMessage(sessionId, req.user.id, dto);
  }
  ...
  async getMessages(
    ...
    return this.askSessionService.getMessages(
      sessionId,
      req.user.id,
      limitNum,
      beforeDate,
  ...
  async regenerateMessage(
    ...
    return this.askSessionService.regenerateMessage(
      sessionId,
      messageId,
      req.user.id,
    );
