...
export class MissionStore {
  ...
  async markCompleted(
    ...
    data: {
      ...
      // ★ PR-6 v1.6 (2026-05-07 overhaul): D4 硬合约 qualityGap 持久化
      // assertHardContract 返回的 gaps 数组（QualityGap[]）；UI 渲染黄色 banner
      qualityGaps?: unknown;
    },
    ...
    const update: Prisma.AgentPlaygroundMissionUpdateInput = {
      ...
      // ★ PR-6 v1.6 (2026-05-07 overhaul): D4 硬合约 qualityGap 持久化
      qualityGaps:
        data.qualityGaps !== undefined
          ? ((data.qualityGaps ?? null) as Prisma.InputJsonValue)
          : undefined,
    };
   ...
   */
  /**
   * ★ PR-3 v1.6 (2026-05-07 overhaul): 章节最终态写 chapters 新表（dual-write 期）
   *
   * 与 saveChapterDraft 区别：
   *   - chapter_drafts (saveChapterDraft) = attempts 历史，allow 多 attempt 同 dim+idx
   *   - chapters (本方法) = 最终发布态，每 mission/dim/chapter_index 唯一 row
   *   - 本表是 rerun 重建源（PR-10 切读源后）
   *   - user_id 双重隔离（CWE-639 cross-user 防护）
   *
   * sub_section_count / sub_section_structure 是 PR-13 字段（quick/standard 路径传 null）。
   */
  async upsertPublishedChapter(args: {
    ...
    thesis?: string;
    content: string;
    /** backend 重算真值（D2 派生 — 永远使用 countCJKWords 的值，不信任 LLM） */
    wordCount: number;
    status: string;
    score?: number;
    /** PR-13 sub-section 路径才有值；quick/standard 单 LLM call 路径 = null */
    subSectionCount?: number;
    subSectionStructure?: unknown;
  }): Promise<void> {
    await this.prisma.agentPlaygroundChapter
      .upsert({
        where: {
        ...
        create: {
          ...
          status: args.status,
          score: args.score,
          subSectionCount: args.subSectionCount,
          subSectionStructure: (args.subSectionStructure as never) ?? undefined,
        },
        update: {
          heading: args.heading.slice(0, 500),
          thesis: args.thesis,
      ...
      .catch((err: unknown) => {
        this.log.warn(
          `[upsertPublishedChapter] mission=${args.missionId} dim=${args.dimension} ch=${args.chapterIndex} failed: ${err instanceof Error ? err.message : String(err)}`,
        );
      });
  }

  /**
   * ★ PR-5 v1.6 D6: 写章节图（figure-curator stage 落库）
   *   - user_id + mission_id 双重隔离（CWE-639）
   *   - 老 figures 全删后批量插（chapter 重写时刷新）
   *   - watermarkOverlayRequired / sourceLicense 字段透传供前端 CSS overlay 决策
   */
  async replaceChapterFigures(args: {
    chapterId: string;
    missionId: string;
    userId: string;
    figures: Array<{
      ...
      width: number | null;
      height: number | null;
      sourceType: "scraped" | "ai-generated" | "user-uploaded" | "hotlink";
    ...
    await this.prisma
      .$transaction(async (tx) => {
        // 删旧（双重 WHERE）
        await tx.agentPlaygroundChapterFigure.deleteMany({
          where: { chapterId: args.chapterId, userId: args.userId },
        });
        // 批量插新
        if (args.figures.length > 0) {
          await tx.agentPlaygroundChapterFigure.createMany({
            data: args.figures.map((f) => ({
      ...
      .catch((err: unknown) => {
        this.log.warn(
          `[replaceChapterFigures] chapter=${args.chapterId} user=${args.userId} failed: ${err instanceof Error ? err.message : String(err)}`,
        );
      });
  }

  /**
   * ★ PR-5 v1.6: 加载某 chapter 的所有 figures（user_id WHERE 隔离）
   */
  ...
  > {
    const rows = await this.prisma.agentPlaygroundChapterFigure
      .findMany({
        where: { chapterId, userId },
        orderBy: { positionInChapter: "asc" },
      })
      .catch((err: unknown) => {
        this.log.warn(
          `[loadChapterFigures] chapter=${chapterId} user=${userId} failed: ${err instanceof Error ? err.message : String(err)}`,
        );
        return [] as Awaited<
          ReturnType<typeof this.prisma.agentPlaygroundChapterFigure.findMany>
        >;
      });
    return rows.map((r) => ({
      id: r.id,
      ...
      watermarkOverlayRequired: r.watermarkOverlayRequired,
      positionInChapter: r.positionInChapter,
    }));
  }

  /**
   * ★ PR-3 v1.6: 加载某 mission 的 chapters 新表（rerun 重建源）。
   * WHERE mission_id AND user_id 双重隔离（CWE-639）。
   * 返回空数组 = 老 mission（chapters 新表无数据）→ 调方应 fallback chapter_drafts。
   */
  ...
  > {
    const rows = await this.prisma.agentPlaygroundChapter
      .findMany({
      ...
      .catch((err: unknown) => {
        this.log.warn(
          `[loadPublishedChapters] mission=${missionId} user=${userId} failed: ${err instanceof Error ? err.message : String(err)}`,
        );
        return [] as Awaited<
          ReturnType<typeof this.prisma.agentPlaygroundChapter.findMany>
        >;
      });
    return rows.map((r) => ({
      id: r.id,
      ...
      subSectionStructure: r.subSectionStructure,
    }));
  }
