...
describe("MissionEventBuffer", () => {
  ...
  // accepts
  it("accepts: returns true for agent-playground.* events", () => {
    const ev = makeEvent("m1");
    expect(buffer.accepts(ev)).toBe(true);
  });

  it("accepts: returns false for non-agent-playground events", () => {
  ...
  it("broadcast: stores event in memory buffer", async () => {
    ...
    expect(events[0].type).toBe("agent-playground.stage:started");
  });

  it("broadcast: no-op if missionId is missing", async () => {
    const ev = {
      type: "agent-playground.test",
      scope: {},
      payload: {},
      timestamp: Date.now(),
    } as unknown as DomainEvent;
  ...
  it("broadcast: caps buffer at MAX_PER_MISSION (5000)", async () => {
    ...
    for (let i = 0; i < 5002; i++) {
      await buffer.broadcast(makeEvent("m1", "agent-playground.test", { i }));
    }
    const events = buffer.read("m1");
  ...
  it("read: returns all events when sinceTs not provided", async () => {
    await buffer.broadcast(makeEvent("m1", "agent-playground.a"));
    await buffer.broadcast(makeEvent("m1", "agent-playground.b"));
    const events = buffer.read("m1");
    expect(events).toHaveLength(2);
  });

  it("read: filters by sinceTs", async () => {
    const ev1 = { ...makeEvent("m1", "agent-playground.a"), timestamp: 1000 };
    const ev2 = { ...makeEvent("m1", "agent-playground.b"), timestamp: 3000 };
    await buffer.broadcast(ev1 as DomainEvent);
    await buffer.broadcast(ev2 as DomainEvent);
    const events = buffer.read("m1", 2000);
    expect(events).toHaveLength(1);
    expect(events[0].type).toBe("agent-playground.b");
  });
  ...
  it("readPersisted: maps DB rows to BufferedEvent shape", async () => {
    ...
    expect(events[0].type).toBe("agent-playground.test");
  ...
  it("different missions have independent buffers", async () => {
    await buffer.broadcast(makeEvent("m1", "agent-playground.a"));
    await buffer.broadcast(makeEvent("m2", "agent-playground.b"));
    expect(buffer.read("m1")).toHaveLength(1);
    expect(buffer.read("m2")).toHaveLength(1);
    expect(buffer.read("m1")[0].type).toBe("agent-playground.a");
  });
});
