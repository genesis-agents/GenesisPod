...
// ── G 三道清理管线 regression ─────────────────────────────────────────────────
// ★ G (2026-05-06): chapter body 经过 stripChartJsonFromContent 三道清理管线
//   1. chapter writer 输出后（G-1）
//   2. stitchedFullMarkdown 拼接前（G-2）
//   3. 报告最终拼接后（TI assembler 已有，playground 新增 G-1/G-2）
//
// 这里验证 pipeline 产出的 chapter.body 不含残留 chartjs fence，
// 以及 fullMarkdown 不含 chart JSON（G-2 path）。
...
jest.mock(
  ...
  () => ({
    stripChartJsonFromContent: jest.fn((body: string) =>
      // 测试 stub：直接返回 body（功能测试不关心实际 strip，只关心调用路径）
...
describe("G 三道清理管线 — chapter body sanitize", () => {
  it("[G1] chapter.body does not contain chartjs fence after pipeline (G-1 sanitize applied)", async () => {
    // chapter writer 返回带 chartjs fence 的 body（含足够多的正文让 validateWrittenChapters 通过）
    ...
    expect(result.chapters).toBeDefined();
    expect(result.chapters!.length).toBeGreaterThan(0);
    // G-1 sanitize 已把 chartjs fence 剥除，chapter body 不应含 ```chartjs
    for (const ch of result.chapters!) {
      expect(ch.body).not.toContain("```chartjs");
    }
  });

  it("[G2] fullMarkdown does not contain chartjs fence (G-2 stitchedFullMarkdown sanitize applied)", async () => {
    // G-2: 每个 ch.body 拼入 stitchedFullMarkdown 前都过一遍 stripChartJsonFromContent
    // stub 已经清掉 ```chartjs fence，所以 fullMarkdown 不应含 chartjs
    const substantiveProse =
    ...
    // stitchedFullMarkdown は chapter.body を stripChartJsonFromContent で清理して組み立て
    // stub removes ```chartjs fences, so fullMarkdown should not contain chartjs
    expect(result.fullMarkdown).toBeDefined();
