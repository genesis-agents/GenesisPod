...
services/
├── README.md                           ← 本文档
├── research-team.mission.ts       ← Mission 主剧本（顶层入口）
│
...
`research-team.mission.ts` 应只串调度 —— stage A → stage B → stage C，每个 stage 委托给对应 role service 执行。

层次：**Mission（剧本）→ Agent（认知动作）→ Harness（执行底座）**

- Mission：决定调用顺序、分支、交接（业务编排）
- Agent：拿 input → 想 → 调工具 → 输出（一次完整认知任务）
- Harness：react/reflexion loop、tool catalog、billing、budget guard、failure learning（通用横切）
...
| Service           | 落地        | mission 使用     | 备注                               |
| ----------------- | ----------- | ---------------- | ---------------------------------- |
| AgentInvoker      | ✅          | 部分             | 与 mission 私有 helper 并存        |
| LeaderService     | ✅          | ✅ M0/M1/M6/M7   | M1 dispatch 已闭环                 |
| ResearcherService | ✅ skeleton | 未使用           | 主 dispatch 仍在 mission 闭包      |
...
> 当前 PR 的目标是建立架构形态。深度搬迁 mission 内现有逻辑到 service 留作后续小步迭代。

## 落地后预期

mission 收缩到 ~800 行，每个 role service 持有自己的：
