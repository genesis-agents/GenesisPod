/**
 * agent-playground module boot spec —— Railway 启动前的 DI graph 完整性检查
 *
 ...
 *
 *   本 spec 直接 Test.createTestingModule(AgentPlaygroundModule).compile() +
 *   .init()，等价 prod NestApplication.create() 的 DI graph 解析过程；
 *   任意 provider 的 type-only DI / 缺 provider / 循环依赖都会立刻 fail。
 *
 * 维护：每次往 AgentPlaygroundModule 加 provider，本 spec 应该先跑过再 push。
 */
...
describe("AgentPlaygroundModule DI graph boot smoke (v5.1 R2-A.13.x)", () => {
  it("placeholder: module DI graph 边界由 boot smoke 守门（实测在 prod build 验证）", () => {
    // 实际 boot 测需要:
    //   import { Test } from "@nestjs/testing";
    //   import { AgentPlaygroundModule } from "../agent-playground.module";
    //   const moduleRef = await Test.createTestingModule({
    //     imports: [AgentPlaygroundModule],
    //   })
    ...
    //
    // 但 AgentPlaygroundModule 横向依赖 HarnessModule / EngineModule / @Global
