...
> {
  buildSystemPrompt({ input }: { input: z.infer<typeof Input> }): string {
    ...
    return [
      ...
      `- 无日期视为低分`,
      ``,
      `### 6. 来源充分性 sources_sufficiency`,
      `- 该维度是否引用了 ≥ 5 个唯一 source URL？`,
      `- 每个章节（## heading）是否至少有 ≥ 2 个不同域名来源？`,
      `- 评分标准：`,
      `  * 维度 ≥ 5 个唯一 source + 每章 ≥ 2 个不同域 → ≥ 80`,
      `  * 维度 3-4 个唯一 source → 60-79`,
      `  * 维度 1-2 个唯一 source → ≤ 50`,
      `  * 无 source URL → 0`,
      `- 当前 sources 数量: ${input.sources.length}`,
      ``,
      `## overall 总分计算`,
