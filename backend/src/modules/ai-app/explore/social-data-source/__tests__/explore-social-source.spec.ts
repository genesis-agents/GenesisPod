...
const makeVideoResource = (overrides: Partial<ReturnType<typeof makeResource>> = {}) =>
  makeResource({
    id: 'res-video',
...
describe('ExploreSocialSourceProvider', () => {
  ...
  it('listItems: maps YOUTUBE_VIDEO resource to contentKind "video"', async () => {
    ...
    expect(item.thumbnailUrl).toBe('https://img.youtube.com/vi/abc123/hqdefault.jpg');
  });
  ...
  it('fetchBundle: returns empty string body when both content and abstract are null', async () => {
    mockFindMany.mockResolvedValueOnce([makeResource({ content: null, abstract: null })]);
  ...
  it('fetchBundle: batch with mixed types maps each to correct contentKind', async () => {
    ...
    expect(articleBundle?.sourceMetadata).toMatchObject({ contentKind: 'article' });
