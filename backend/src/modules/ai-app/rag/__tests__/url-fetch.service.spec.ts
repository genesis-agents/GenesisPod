...
describe('UrlFetchService', () => {
  ...
  describe('fetchUrl', () => {
    it('should throw BadRequestException for invalid URL', async () => {
      await expect(service.fetchUrl('not-a-url')).rejects.toThrow(BadRequestException);
    });

    it('should throw BadRequestException for non-HTTP protocols', async () => {
      await expect(service.fetchUrl('ftp://example.com')).rejects.toThrow(BadRequestException);
    });

    it('should throw BadRequestException for file:// protocol', async () => {
      await expect(service.fetchUrl('file:///etc/passwd')).rejects.toThrow(BadRequestException);
    });
    ...
    it('should throw BadRequestException when HTTP response is not ok', async () => {
      ...
      await expect(service.fetchUrl('https://example.com/not-found')).rejects.toThrow(
        BadRequestException,
      );
    });
    ...
    it('should handle timeout errors', async () => {
      ...
      await expect(service.fetchUrl('https://slow.example.com')).rejects.toThrow(
        BadRequestException,
      );
    });
    ...
    it('should handle DNS errors (ENOTFOUND)', async () => {
      ...
      await expect(service.fetchUrl('https://example.invalid')).rejects.toThrow(BadRequestException);
    });

    it('should handle connection refused errors', async () => {
      mockFetch.mockRejectedValueOnce(new Error('ECONNREFUSED 127.0.0.1:80'));

      await expect(service.fetchUrl('https://localhost:80')).rejects.toThrow(BadRequestException);
    });
  ...
  describe('importUrls', () => {
    it('should import multiple URLs into knowledge base', async () => {
      const html = '<html><head><title>Test</title></head><body><p>Test content with enough words for extraction</p></body></html>';

      mockFetch.mockResolvedValue({
        ok: true,
        text: () => Promise.resolve(html),
      });
      (knowledgeBaseService.addDocument as jest.Mock).mockResolvedValue({ id: 'doc-1', title: 'Test', url: 'https://example.com/1' });
