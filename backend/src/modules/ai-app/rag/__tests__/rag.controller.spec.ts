...
describe('RAGController', () => {
  ...
  beforeEach(async () => {
    const mockKbService = {
      ...
      addDocument: jest.fn().mockResolvedValue({ id: 'doc-1', title: 'Test Doc' }),
      deleteDocument: jest.fn().mockResolvedValue(undefined),
      processAllDocuments: jest.fn().mockResolvedValue({ processed: 1 }),
      getStats: jest.fn().mockResolvedValue({ totalDocuments: 5, totalChunks: 50 }),
      listDocuments: jest.fn().mockResolvedValue([{ id: 'doc-1', title: 'Doc 1' }]),
      ...
      getDocumentById: jest.fn().mockResolvedValue({ id: 'doc-1', title: 'Doc 1' }),
    };

    const mockRagPipelineService = {
      query: jest.fn().mockResolvedValue({ results: [], context: '' }),
      simpleQuery: jest.fn().mockResolvedValue([
        { childChunkId: 'chunk-1', content: 'Result', score: 0.9, documentId: 'doc-1', metadata: {} },
      ]),
    };

    const mockGoogleDriveService = {
      syncKnowledgeBase: jest.fn().mockResolvedValue({ added: 2, updated: 1, failed: 0 }),
      listFolders: jest.fn().mockResolvedValue([{ id: 'folder-1', name: 'My Folder' }]),
    };

    const mockAiFacade = {
      embedding: {
        getConfigInfo: jest.fn().mockResolvedValue({ model: 'text-embedding-3-small', dimensions: 1536 }),
      },
    };
    ...
    const mockUrlFetchService = {
      ...
      importUrls: jest.fn().mockResolvedValue({ success: 2, failed: [], documents: [] }),
    };

    const mockPlatformImportService = {
      getAvailableBookmarks: jest.fn().mockResolvedValue({ bookmarks: [], total: 0 }),
  ...
  describe('getEmbeddingConfig', () => {
    it('should return embedding config from facade', async () => {
      ...
      expect(result).toEqual({ model: 'text-embedding-3-small', dimensions: 1536 });
    });
  });
  ...
  describe('createKnowledgeBase', () => {
    it('should create knowledge base without Google Drive files', async () => {
      ...
      const result = await controller.createKnowledgeBase(mockRequest, dto as any);

      expect(knowledgeBaseService.create).toHaveBeenCalledWith('user-1', dto);
      expect(knowledgeBaseService.findById).toHaveBeenCalledWith('kb-1', 'user-1');
    ...
    it('should auto-sync when Google Drive fileIds are provided', async () => {
      const dto = { name: 'GDrive KB', googleDriveFileIds: ['file-1', 'file-2'] };
      await controller.createKnowledgeBase(mockRequest, dto as any);

      expect(googleDriveRAGService.syncKnowledgeBase).toHaveBeenCalledWith('kb-1');
    });
    ...
    it('should not throw when auto-sync fails', async () => {
      ...
      (googleDriveRAGService.syncKnowledgeBase as jest.Mock).mockRejectedValueOnce(
        new Error('Sync failed'),
      );

      // Should not throw
      await expect(controller.createKnowledgeBase(mockRequest, dto as any)).resolves.toBeDefined();
    });
  });
  ...
  describe('getKnowledgeBaseStats', () => {
    it('should return knowledge base stats', async () => {
      const result = await controller.getKnowledgeBaseStats(mockRequest, 'kb-1');
  ...
  describe('updateKnowledgeBase', () => {
    it('should update knowledge base', async () => {
      ...
      expect(knowledgeBaseService.update).toHaveBeenCalledWith('kb-1', 'user-1', dto);
    });
  });
  ...
  describe('deleteKnowledgeBase', () => {
    it('should delete knowledge base and return success message', async () => {
      ...
      expect(knowledgeBaseService.delete).toHaveBeenCalledWith('kb-1', 'user-1');
  ...
  describe('addDocument', () => {
    it('should add document to knowledge base', async () => {
      ...
      const result = await controller.addDocument(mockRequest, 'kb-1', dto as any);

      expect(knowledgeBaseService.findById).toHaveBeenCalledWith('kb-1', 'user-1');
      expect(knowledgeBaseService.addDocument).toHaveBeenCalledWith('kb-1', expect.objectContaining({
        title: 'New Doc',
        content: 'Content',
        sourceType: 'manual',
      }));
    ...
    it('should default sourceType to manual when not provided', async () => {
      ...
      expect(knowledgeBaseService.addDocument).toHaveBeenCalledWith('kb-1',
  ...
  describe('deleteDocument', () => {
    it('should delete document and return success', async () => {
      ...
      expect(knowledgeBaseService.deleteDocument).toHaveBeenCalledWith('doc-1', 'user-1');
  ...
  describe('processDocuments', () => {
    it('should process all pending documents', async () => {
      ...
      expect(knowledgeBaseService.findById).toHaveBeenCalledWith('kb-1', 'user-1');
      expect(knowledgeBaseService.processAllDocuments).toHaveBeenCalledWith('kb-1');
  ...
  describe('listGoogleDriveFolders', () => {
    it('should list Google Drive folders', async () => {
      ...
      expect(googleDriveRAGService.listFolders).toHaveBeenCalledWith('user-1', undefined);
      expect(Array.isArray(result)).toBe(true);
    });
    ...
    it('should pass parentId when provided', async () => {
      ...
      expect(googleDriveRAGService.listFolders).toHaveBeenCalledWith('user-1', 'folder-1');
    });
  });
  ...
  describe('fetchUrl', () => {
    it('should preview URL content', async () => {
      ...
      expect(knowledgeBaseService.findById).toHaveBeenCalledWith('kb-1', 'user-1');
      expect(urlFetchService.fetchUrl).toHaveBeenCalledWith('https://example.com');
  ...
  describe('importUrls', () => {
    it('should import URLs to knowledge base', async () => {
      ...
      const result = await controller.importUrls(mockRequest, 'kb-1', dto as any);
  ...
  describe('getAvailableBookmarks', () => {
    ...
    it('should parse page and limit as integers', async () => {
      await controller.getAvailableBookmarks(mockRequest, 'kb-1', 'test', '2', '20');
  ...
  describe('importBookmarks', () => {
    it('should import bookmarks to knowledge base', async () => {
      ...
      const result = await controller.importBookmarks(mockRequest, 'kb-1', dto as any);

      expect(platformImportService.importBookmarks).toHaveBeenCalledWith('kb-1', 'user-1', ['bm-1', 'bm-2']);
  ...
  describe('importOcr', () => {
    it('should import OCR documents to knowledge base', async () => {
      const dto = {
        documents: [
          { title: 'Page 1', content: 'OCR text here', imageUrl: 'https://img.example.com/1.jpg' },
          { title: 'Page 2', content: 'More OCR text', imageUrl: 'https://img.example.com/2.jpg' },
        ],
      };
      const result = await controller.importOcr(mockRequest, 'kb-1', dto as any);
  ...
  describe('getMembers', () => {
    it('should return knowledge base members', async () => {
      ...
      expect(knowledgeBaseService.getMembers).toHaveBeenCalledWith('kb-1', 'user-1');
      expect(Array.isArray(result)).toBe(true);
    });
  });
  ...
  describe('addMember', () => {
    it('should add member to knowledge base', async () => {
      ...
      expect(knowledgeBaseService.addMember).toHaveBeenCalledWith(
        'kb-1', 'user-1', 'member@example.com', 'VIEWER',
      );
    });
    ...
    it('should add member without role (optional)', async () => {
      ...
      expect(knowledgeBaseService.addMember).toHaveBeenCalledWith(
        'kb-1', 'user-1', 'member@example.com', undefined,
      );
    });
  });
  ...
  describe('removeMember', () => {
    it('should remove member and return success', async () => {
      const result = await controller.removeMember(mockRequest, 'kb-1', 'member-1');

      expect(knowledgeBaseService.removeMember).toHaveBeenCalledWith('kb-1', 'user-1', 'member-1');
  ...
  describe('query', () => {
    it('should execute full RAG query', async () => {
      ...
      (knowledgeBaseService.findById as jest.Mock).mockResolvedValue(mockKnowledgeBase);
  ...
  describe('simpleQuery', () => {
    it('should execute simple vector search and enhance results', async () => {
      ...
      expect(ragPipelineService.simpleQuery).toHaveBeenCalledWith('AI basics', ['kb-1'], 3);
      expect(knowledgeBaseService.getDocumentById).toHaveBeenCalledWith('doc-1');
      expect(result).toHaveProperty('results');
    ...
    it('should use Unknown Document when document not found', async () => {
      (knowledgeBaseService.getDocumentById as jest.Mock).mockResolvedValue(null);
  ...
  describe('addResources', () => {
    it('should handle Google Drive resources with sync', async () => {
      ...
      const dto = {
        resources: [{ sourceType: 'google_drive', sourceId: 'file-1', title: 'GDrive File' }],
      };
      const result = await controller.addResources(mockRequest, 'kb-1', dto as any);
    ...
    it('should handle platform resource type with content fetching', async () => {
      const platformResource = {
        id: 'res-1',
        content: 'Long enough content that exceeds the minimum threshold for import operations here',
        abstract: null,
        aiSummary: null,
        sourceUrl: null,
        type: 'article',
      };
      (knowledgeBaseService.getResourcesByIds as jest.Mock).mockResolvedValue([platformResource]);
      ...
      expect(knowledgeBaseService.getResourcesByIds).toHaveBeenCalledWith(['res-1']);
    ...
    it('should handle platform resource with short content by fetching URL', async () => {
      ...
      (urlFetchService.fetchUrl as jest.Mock).mockResolvedValue({
        content: 'Full article content that is much longer and more detailed than the short snippet',
      });
      ...
      expect(urlFetchService.fetchUrl).toHaveBeenCalledWith('https://example.com/article');
    });
    ...
    it('should handle legacy source types with placeholder', async () => {
      ...
      expect(knowledgeBaseService.addDocument).toHaveBeenCalledWith('kb-1',
        expect.objectContaining({ content: '[Pending content fetch from notion]' }),
      );
      expect(result).toHaveProperty('count', 1);
    });
  });
