...
describe('PlatformImportService', () => {
  ...
  describe('getAvailableBookmarks', () => {
    ...
    it('should filter by search query', async () => {
      ...
      const findManyCall = (prisma.resource.findMany as jest.Mock).mock.calls[0][0];
  ...
  describe('importBookmarks', () => {
    it('should import bookmarks into knowledge base', async () => {
      ...
      (prisma.resource.findMany as jest.Mock).mockResolvedValue(mockResources);
      (knowledgeBaseService.addDocument as jest.Mock).mockResolvedValue({ id: 'doc-1' });
    ...
    it('should handle URL fetch failure gracefully', async () => {
      ...
      (prisma.resource.findMany as jest.Mock).mockResolvedValue(mockResources);
      (urlFetchService.fetchUrl as jest.Mock).mockRejectedValue(new Error('Network error'));
      (knowledgeBaseService.addDocument as jest.Mock).mockResolvedValue({ id: 'doc-1' });
    ...
    it('should track bookmarks not found', async () => {
      ...
      const result = await service.importBookmarks('kb-1', 'user-1', ['not-found-id']);
    ...
    it('should handle addDocument failure gracefully', async () => {
      ...
      (prisma.resource.findMany as jest.Mock).mockResolvedValue(mockResources);
      (knowledgeBaseService.addDocument as jest.Mock).mockRejectedValue(new Error('DB error'));
