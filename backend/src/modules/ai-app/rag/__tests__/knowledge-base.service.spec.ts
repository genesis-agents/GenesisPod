...
describe('KnowledgeBaseService', () => {
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        ...
        { provide: EmbeddingProcessorService, useValue: mockEmbeddingProcessor },
      ],
    }).compile();
  ...
  describe('create', () => {
    it('should create a personal knowledge base', async () => {
      (prisma.knowledgeBase.create as jest.Mock).mockResolvedValue(mockKnowledgeBase);
    ...
    it('should auto-detect Google Drive connection when sourceType is GOOGLE_DRIVE', async () => {
      ...
      (prisma.googleDriveConnection.findUnique as jest.Mock).mockResolvedValue(mockConnection);
    ...
    it('should throw when no Google Drive connection found', async () => {
      (prisma.googleDriveConnection.findUnique as jest.Mock).mockResolvedValue(null);
  ...
  describe('findById', () => {
    it('should return knowledge base by id with userId', async () => {
      (prisma.knowledgeBase.findFirst as jest.Mock).mockResolvedValue(mockKnowledgeBase);
    ...
    it('should throw NotFoundException when not found', async () => {
      ...
      await expect(service.findById('not-found', 'user-1')).rejects.toThrow(NotFoundException);
    });
  });

  describe('findByUser', () => {
    it('should return knowledge bases for user', async () => {
      (prisma.knowledgeBase.findMany as jest.Mock).mockResolvedValue([mockKnowledgeBase]);
  ...
  describe('update', () => {
    ...
    it('should auto-connect Google Drive when adding as source type', async () => {
      const existingKb = { ...mockKnowledgeBase, googleDriveConnectionId: null };
      const mockConnection = { id: 'conn-456' };

      (prisma.knowledgeBase.findFirst as jest.Mock).mockResolvedValue(existingKb);
      (prisma.googleDriveConnection.findUnique as jest.Mock).mockResolvedValue(mockConnection);
      (prisma.knowledgeBase.update as jest.Mock).mockResolvedValue(existingKb);

      await service.update('kb-123', 'user-1', { sourceTypes: ['GOOGLE_DRIVE'] });

      expect(prisma.knowledgeBase.update).toHaveBeenCalledWith(
        expect.objectContaining({
          data: expect.objectContaining({ googleDriveConnectionId: 'conn-456' }),
        }),
      );
    });
  });
  ...
  describe('delete', () => {
    ...
    it('should throw NotFoundException when not found', async () => {
      ...
      await expect(service.delete('not-found', 'user-1')).rejects.toThrow(NotFoundException);
    });
  });
  ...
  describe('addDocument', () => {
    it('should add document to knowledge base', async () => {
      ...
      (prisma.knowledgeBaseDocument.create as jest.Mock).mockResolvedValue(mockDoc);
    ...
    it('should sanitize NULL bytes in title and content', async () => {
      ...
      const createCall = (prisma.knowledgeBaseDocument.create as jest.Mock).mock.calls[0][0];
  ...
  describe('processAllDocuments', () => {
    it('should process all documents and generate embeddings', async () => {
      (prisma.knowledgeBase.update as jest.Mock).mockResolvedValue(mockKnowledgeBase);
      (documentProcessor.processAllPendingDocuments as jest.Mock).mockResolvedValue(3);
      (embeddingProcessor.generateEmbeddingsForKnowledgeBase as jest.Mock).mockResolvedValue(15);

      const result = await service.processAllDocuments('kb-123');

      expect(result).toEqual({ processedCount: 3, embeddingCount: 15 });
      expect(documentProcessor.processAllPendingDocuments).toHaveBeenCalledWith('kb-123');
      expect(embeddingProcessor.generateEmbeddingsForKnowledgeBase).toHaveBeenCalledWith('kb-123');
    });

    it('should set KB status to ERROR on failure', async () => {
      (prisma.knowledgeBase.update as jest.Mock).mockResolvedValue(mockKnowledgeBase);
      (documentProcessor.processAllPendingDocuments as jest.Mock).mockRejectedValue(
        new Error('Processing failed'),
      );

      await expect(service.processAllDocuments('kb-123')).rejects.toThrow('Processing failed');
  ...
  describe('getStats', () => {
    ...
    it('should return zero stats when no data', async () => {
      ...
      (prisma.$queryRaw as jest.Mock).mockResolvedValue([]);
      (prisma.knowledgeBase.findUnique as jest.Mock).mockResolvedValue({ lastSyncedAt: null });
  ...
  describe('listDocuments', () => {
    it('should return list of documents with embedding counts', async () => {
      ...
      (prisma.knowledgeBaseDocument.findMany as jest.Mock).mockResolvedValue(mockDocs);
      (prisma.$queryRaw as jest.Mock).mockResolvedValue([
  ...
  describe('deleteDocument', () => {
    it('should delete document and related data', async () => {
      ...
      (prisma.knowledgeBaseDocument.findFirst as jest.Mock).mockResolvedValue(mockDoc);
      (prisma.$transaction as jest.Mock).mockResolvedValue(undefined);
    ...
    it('should throw NotFoundException when document not found', async () => {
      (prisma.knowledgeBaseDocument.findFirst as jest.Mock).mockResolvedValue(null);

      await expect(service.deleteDocument('not-found', 'user-1')).rejects.toThrow(NotFoundException);
    });
  ...
  describe('hasAccess', () => {
    ...
    it('should return false for team KB when user is not a member', async () => {
      ...
      (prisma.knowledgeBaseMember.findFirst as jest.Mock).mockResolvedValue(null);
  ...
  describe('getResourcesByIds', () => {
    ...
    it('should merge YouTube transcripts into resources', async () => {
      ...
      (prisma.resource.findMany as jest.Mock).mockResolvedValue(mockResources);
      (prisma.youTubeTranscriptCache.findMany as jest.Mock).mockResolvedValue(mockTranscripts);
  ...
  describe('getMembers', () => {
    ...
    it('should throw NotFoundException when kb not found', async () => {
      ...
      await expect(service.getMembers('not-found', 'user-1')).rejects.toThrow(NotFoundException);
    });
  });
  ...
  describe('addMember', () => {
    it('should add member to knowledge base', async () => {
      ...
      const result = await service.addMember('kb-123', 'user-1', 'new@example.com', 'VIEWER');
