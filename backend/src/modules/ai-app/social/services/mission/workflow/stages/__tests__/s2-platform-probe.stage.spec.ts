...
function makeCtx(overrides: Partial<MissionInvariants> = {}): Ctx {
  return {
    ...
    contentRaw: { title: "S2 title", body: "S2 body", digest: "summary", coverImageUrl: null },
    stewardInputs: {
...
describe("runPlatformProbeStage (s2)", () => {
  ...
  describe("happy path", () => {
    ...
    it("should emit narrative searching at start and success at end", async () => {
      ...
      const tags = narrativeCalls.map(
        (args: unknown[]) => (args[0] as { payload: { tag: string } }).payload.tag,
      );
  ...
  describe("platformProbe failure", () => {
    ...
    it("should throw when output is null even if state is not 'failed'", async () => {
      const deps = makeDeps({
        platformProbe: {
          run: jest.fn().mockResolvedValue({ state: "completed", output: null }),
        } as unknown as CommonDeps["platformProbe"],
      });
      const ctx = makeCtx();
  ...
  describe("narrate success text includes platform states", () => {
    it("should include platform probe states in success narrative", async () => {
      ...
      const ctx = makeCtx({ input: { contentId: "c2", platforms: ["wechat"], connectionIds: { wechat: "conn-w" }, depth: "standard", budgetProfile: "standard", language: "zh-CN" } });
      ...
      const text = (successNarrative![0] as { payload: { text: string } }).payload.text;
