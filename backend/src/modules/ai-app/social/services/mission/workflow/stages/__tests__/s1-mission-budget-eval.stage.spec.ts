...
function makeCtx(overrides: Partial<MissionInvariants> = {}): MissionInvariants {
  return {
    ...
    contentRaw: { title: "Test title", body: "Test body", digest: null, coverImageUrl: null },
    stewardInputs: {
...
describe("runMissionBudgetEvalStage (s1)", () => {
  ...
  describe("happy path — steward approves", () => {
    it("should emit social.mission:started event", async () => {
      ...
      const startedCall = emitCalls.find(
        (args: unknown[]) => (args[0] as { type: string }).type === "social.mission:started",
      );
      expect(startedCall).toBeDefined();
      expect((startedCall![0] as { missionId: string }).missionId).toBe("mission-s1-test");
    });
    ...
    it("should include workspaceId in mission:started payload when provided", async () => {
      ...
      expect(
        (startedCall![0] as { payload: { workspaceId: string } }).payload.workspaceId,
      ).toBe("workspace-xyz");
    });
  });
  ...
  describe("steward verdict=gated", () => {
    it("should emit social.mission:gated and throw when verdict is gated", async () => {
      ...
      const gatedCall = emitCalls.find(
        (args: unknown[]) => (args[0] as { type: string }).type === "social.mission:gated",
      );
      expect(gatedCall).toBeDefined();
      expect(
        (gatedCall![0] as { payload: { gateFailed: string } }).payload.gateFailed,
      ).toBe("budget");
    });
  ...
  describe("emit failure resilience", () => {
    it("should not throw when social.mission:started emit fails", async () => {
      const emitMock = jest.fn().mockImplementation((event: { type: string }) => {
      ...
      await expect(runMissionBudgetEvalStage(ctx, deps)).resolves.toBeUndefined();
    });
  });

  describe("steward run with pool and billing passed", () => {
    it("should pass billing and pool from ctx to steward.run", async () => {
      const mockBilling = { type: "billing-adapter" } as unknown as MissionInvariants["billing"];
      const mockPool = { type: "budget-pool" } as unknown as MissionInvariants["pool"];
      const deps = makeDeps();
