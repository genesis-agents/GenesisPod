...
describe('PlanningController', () => {
  ...
  beforeEach(async () => {
    const mockOrchestrator = {
      ...
      getPlanDetail: jest.fn().mockResolvedValue({ ...mockPlanSummary, description: null, depth: PlanningDepth.STANDARD, autoAdvance: true, members: [], references: [] }),
  ...
  describe('getPlans', () => {
    ...
    it('should pass search query to orchestrator', async () => {
      ...
      expect(orchestrator.getPlans).toHaveBeenCalledWith('user-1', 'test search');
    });
  });
  ...
  describe('getPlanDetail', () => {
    it('should return plan detail', async () => {
      ...
      expect(orchestrator.getPlanDetail).toHaveBeenCalledWith('plan-123', 'user-1');
  ...
  describe('updatePlan', () => {
    it('should update plan', async () => {
      ...
      expect(orchestrator.updatePlan).toHaveBeenCalledWith('plan-123', 'user-1', dto);
    });
  });
  ...
  describe('advancePhase', () => {
    it('should advance to next phase', async () => {
      ...
      expect(orchestrator.advancePhase).toHaveBeenCalledWith('plan-123', 'user-1');
  ...
  describe('retryPhase', () => {
    it('should retry specified phase', async () => {
      const result = await controller.retryPhase(mockRequest, 'plan-123', '3');

      expect(orchestrator.retryPhase).toHaveBeenCalledWith('plan-123', 3, 'user-1');
    });
  });
  ...
  describe('replanFromPhase', () => {
    it('should replan from specified phase', async () => {
      ...
      expect(orchestrator.replanFromPhase).toHaveBeenCalledWith('plan-123', 2, 'user-1');
    });
  });
  ...
  describe('cancelPhase', () => {
    it('should cancel current phase', async () => {
      ...
      expect(orchestrator.cancelPhase).toHaveBeenCalledWith('plan-123', 'user-1');
  ...
  describe('exportPlan', () => {
    it('should export plan as markdown', async () => {
      await controller.exportPlan(mockRequest, 'plan-123', 'report', mockResponse);

      expect(orchestrator.exportPlan).toHaveBeenCalledWith('plan-123', 'user-1', 'report');
      expect(mockResponse.setHeader).toHaveBeenCalledWith('Content-Type', 'text/markdown; charset=utf-8');
      expect(mockResponse.send).toHaveBeenCalledWith('# Plan Markdown');
    });

    it('should use full mode when mode is full', async () => {
      await controller.exportPlan(mockRequest, 'plan-123', 'full', mockResponse);

      expect(orchestrator.exportPlan).toHaveBeenCalledWith('plan-123', 'user-1', 'full');
    });

    it('should default to report mode for unknown mode', async () => {
      await controller.exportPlan(mockRequest, 'plan-123', 'unknown', mockResponse);
  ...
  describe('deletePlan', () => {
    it('should delete plan and return success', async () => {
      ...
      expect(orchestrator.deletePlan).toHaveBeenCalledWith('plan-123', 'user-1');
      expect(result).toEqual({ success: true });
    });
  });
});
