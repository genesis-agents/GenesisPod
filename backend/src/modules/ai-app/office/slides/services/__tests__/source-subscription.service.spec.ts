...
describe('SourceSubscriptionService', () => {
  ...
  it('should handle zero missions updated gracefully', async () => {
    ...
    // Should not throw
    await expect(service.handleTopicReportRefreshed(event)).resolves.not.toThrow();
    expect(mockPrisma.$executeRaw).toHaveBeenCalledTimes(1);
  });

  it('should catch and log errors when database query fails', async () => {
    (mockPrisma.$executeRaw as jest.Mock).mockRejectedValue(new Error('DB connection lost'));
