...
describe('SlidesDataImportService', () => {
  ...
  let researchExport: { getTopicForExport: jest.Mock; listTopicsForExport: jest.Mock };
  let researchProjectExport: { getProjectForExport: jest.Mock; listProjectsForExport: jest.Mock };
  let writingExport: { getProjectForExport: jest.Mock; listProjectsForExport: jest.Mock };
  ...
  const mockResearchData = {
    ...
    latestReport: {
      fullReport: 'Full report content with some data https://example.com/report',
      charts: [
        {
          type: 'bar',
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        ...
        { provide: RESEARCH_PROJECT_DATA_EXPORT, useValue: researchProjectExport },
  ...
  describe('importFromResearch', () => {
    ...
    it('should fallback to dimensions when no dimensionAnalyses in report', async () => {
      const dataWithoutAnalyses = {
        ...mockResearchData,
        latestReport: { ...mockResearchData.latestReport, dimensionAnalyses: [] },
      };
      researchExport.getTopicForExport.mockResolvedValueOnce(dataWithoutAnalyses);
    ...
    it('should skip invalid chart types', async () => {
      const dataWithBadChart = {
        ...
        latestReport: {
          ...mockResearchData.latestReport,
          charts: [
            { type: 'invalid_type', title: 'Bad Chart', labels: [], series: [] },
    ...
    it('should extract references from report URLs', async () => {
      ...
      expect(result.references!.some((r) => r.url.includes('example.com'))).toBe(
        true,
      );
    });
  ...
  describe('importFromResearchProject', () => {
    it('should import data from research project', async () => {
      ...
      const result = await service.importFromResearchProject('project-1', 'user-1');
  ...
  describe('importFromLibrary', () => {
    it('should import resources from library', async () => {
      ...
      (prisma.resource.findMany as jest.Mock).mockResolvedValueOnce(mockResources);

      const result = await service.importFromLibrary(['res-1', 'res-2'], 'user-1');
