...
import { QualityAuditSkill, QualityAuditInput, SemanticIssue } from '../quality-audit.skill';
import { PageOutline, PageContent, ContentSection } from '../../checkpoint/checkpoint.types';
...
const makeChartSection = (
  ...
  type: 'chart',
  position,
  content: { type: 'bar', data: [], labels } as unknown as ContentSection['content'],
});
...
describe('QualityAuditSkill', () => {
  ...
  describe('execute() - input validation', () => {
    it('should return error for missing pageOutline.templateType', async () => {
      const input: QualityAuditInput = {
        pageOutline: { ...buildPageOutline(), templateType: undefined as unknown as 'splitLayout' },
    ...
    it('should succeed with valid direct input', async () => {
      const input: QualityAuditInput = {
        pageOutline: buildPageOutline(),
        pageContent: buildPageContent([makeTextSection('这里有足够的内容文本，确保超过100字的文本内容完整显示所有测试场景的功能运行正常。')]),
        html: VALID_HTML,
      };
      const result = await skill.execute(input, buildSkillContext());
  ...
  describe('execute() - orchestrator input format', () => {
    it('should normalize orchestrator input and execute successfully', async () => {
      const orchestratorInput = {
        ...
        context: {
          pageOutline: buildPageOutline(),
          pageContent: buildPageContent([makeTextSection('测试内容文本，保证超过100个字符长度的内容文本显示正常，用于质量审核功能的各种场景测试。')]),
          html: VALID_HTML,
        },
        previousOutputs: {},
      };

      const result = await skill.execute(orchestratorInput as QualityAuditInput, buildSkillContext());
      expect(result.success).toBe(true);
    });
    ...
    it('should get html from previousOutputs rendering result', async () => {
      const orchestratorInput = {
        ...
        context: {
          pageOutline: buildPageOutline(),
          pageContent: buildPageContent([makeTextSection('内容文本，超过100字，用于测试从previousOutputs中获取html的质量审核功能能够正常运行。')]),
        },
        previousOutputs: {
      ...
      const result = await skill.execute(orchestratorInput as unknown as QualityAuditInput, buildSkillContext());
      expect(result.success).toBe(true);
    });
  ...
  describe('execute() - auditOnly mode', () => {
    it('should return audit result without fix when auditOnly=true', async () => {
      const input: QualityAuditInput = {
        pageOutline: buildPageOutline(),
        pageContent: buildPageContent([makeTextSection('内容文本，超过100字，用于测试auditOnly模式下只审核不修复的功能运行情况，包含各种场景。')]),
        html: VALID_HTML,
        auditOnly: true,
      };
      const result = await skill.execute(input, buildSkillContext());
  ...
  describe('execute() - result metadata', () => {
    it('should include correct metadata in result', async () => {
      ...
      const input: QualityAuditInput = {
        pageOutline: buildPageOutline(),
        pageContent: buildPageContent([makeTextSection('测试元数据内容文本，足够长度超过100字，用于验证执行元数据包含正确的executionId和时间戳信息。')]),
        html: VALID_HTML,
      };
  ...
  describe('auditPage()', () => {
    it('should return passing audit for good content', () => {
      ...
      const content = buildPageContent([
        makeTextSection('这是高质量的分析内容，测试分栏布局模板与内容语义匹配性，保证超过100字，以确认质量审核结果为通过。'),
      ]);
    ...
    it('should not flag filler content when title has innovation context', () => {
      ...
      const fillerIssues = result.issues.filter(
        (i) =>
          i.type === 'content_logic' &&
          i.message.includes('创新驱动'),
      );
      ...
      const chartContent = buildPageContent([
        {
          type: 'chart',
          position: 'full',
          content: { type: 'bar', data: [], labels: ['北京', '上海', '广州', '深圳'] } as unknown as ContentSection['content'],
        },
      ]);
      ...
      const chartContent = buildPageContent([
        {
          type: 'chart',
          position: 'full',
          content: { type: 'bar', data: [], labels: ['2020年', '2021年', '2022年'] } as unknown as ContentSection['content'],
        },
      ]);
      ...
      const pages = [
        {
          ...
          content: buildPageContent([makeTextSection('页面1内容，测试多页审核功能的运行情况是否正确处理跨页一致性检查。')]),
          html: VALID_HTML,
        },
        {
          outline: buildPageOutline({ pageNumber: 2 }),
          content: buildPageContent([makeTextSection('页面2内容，测试多页审核功能的运行情况是否正确处理跨页一致性检查。')]),
          html: VALID_HTML,
        },
      ];
  ...
  describe('auditAndFix()', () => {
    it('should return structured result with audit, fix, and diagnostic', () => {
      const outline = buildPageOutline();
      const content = buildPageContent([
        makeTextSection('测试内容超过100字，用于验证审核并修复功能的正常运行，包含audit、fix和diagnostic三个部分的返回结果。'),
      ]);
      ...
      const content = buildPageContent([
        {
          type: 'chart',
          position: 'full',
          content: { type: 'bar', data: [], labels: ['北京', '上海', '广州'] } as unknown as ContentSection['content'],
        },
      ]);
  ...
  describe('score calculation', () => {
    it('should return score >= 60 (passed) when content is good', () => {
      ...
      `;
      const content = buildPageContent([
        makeTextSection('足够长的内容文本，超过100字，用于测试没有质量问题时得分应该是100，质量审核通过的情况的评分是否正确。'),
      ]);
    ...
    it('should fail when score < 60', () => {
      ...
      if (result.score < 60 || result.issues.some((i) => i.severity === 'error')) {
  ...
  describe('diagnostic generation', () => {
    it('should include timestamp and pageNumber in diagnostic', () => {
      ...
      const content = buildPageContent([
        makeTextSection('测试诊断信息生成，超过100字，验证诊断信息是否包含正确的时间戳和页码信息以及其他必要字段。'),
      ]);
      const result = skill.auditAndFix(outline, content, VALID_HTML);
    ...
    it('should have fixAttempted correspond to whether issues exist', () => {
      ...
      `;
      const content = buildPageContent([
        makeTextSection('测试没有问题时诊断信息fixAttempted字段应该为false，超过100字的内容确保没有布局问题。'),
      ]);
  ...
  describe('section content handling', () => {
    ...
    it('should handle array content in sections', () => {
      const outline = buildPageOutline();
      const content = buildPageContent([makeListSection(['项目1', '项目2', '项目3'])]);
      const result = skill.auditPage(outline, content, VALID_HTML);
      expect(result).toBeDefined();
    });
    ...
    it('should handle object content in sections', () => {
      const outline = buildPageOutline();
      const content = buildPageContent([
        { type: 'stat', position: 'left', content: { value: '100', label: '指标' } as ContentSection['content'] },
      ]);
      const result = skill.auditPage(outline, content, VALID_HTML);
      expect(result).toBeDefined();
    });
  ...
  describe('summary generation', () => {
    it('should generate meaningful summary string', () => {
      ...
      `;
      const content = buildPageContent([
        makeTextSection('超过100字的内容：这里有足够的内容文本，用于测试质量审核功能，保证超过100个字符长度的文本内容。'),
      ]);
      const result = skill.auditPage(outline, content, richHtml);
  ...
  describe('edge cases', () => {
    it('should handle error thrown inside try block', async () => {
      ...
      const input = {
        pageOutline: buildPageOutline(),
        pageContent: buildPageContent([makeTextSection('内容足够长超过100字用于测试的各种异常情况处理，包含null和undefined的防御性检查。')]),
        html: VALID_HTML,
      };
      ...
      // Restore
      (skill.auditPage as jest.MockedFunction<typeof skill.auditPage>).mockRestore();
      void originalAuditPage;
    });

    it('should handle pageContent with no sections (undefined sections)', () => {
      const outline = buildPageOutline();
      const content = { title: '标题', sections: undefined as unknown as ContentSection[] };
      const html = '<div>内容</div>';
      // Should not throw
