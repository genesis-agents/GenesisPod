...
const buildPage = (index: number, title: string, content: string, keyPoints?: string[]) => ({
  index,
  title,
  content,
  keyPoints,
...
describe('VoiceNarrationSkill', () => {
  ...
  it('should use template fallback when AI facade is not available', async () => {
    ...
    const result = await skillWithoutFacade.execute(
      { pages, presentationTitle: 'Test Deck', language: 'en', style: 'formal' },
      buildSkillContext(),
    );
  ...
  it('should estimate duration based on word count', async () => {
    ...
    mockFacade.chat.mockResolvedValue({ content: narrationText, tokensUsed: 50 });
    ...
    expect(result.data!.totalDuration).toBe(result.data!.narrations[0].estimatedDuration);
  });
  ...
  it('should support different narration styles', async () => {
    ...
    await skill.execute(
      { pages, presentationTitle: 'Story Time', style: 'storytelling', language: 'en' },
      buildSkillContext(),
    );
  ...
  it('should calculate total words and average stats', async () => {
    mockFacade.chat
      ...
      .mockResolvedValueOnce({ content: 'Longer narration with more words here.', tokensUsed: 30 });
  ...
  it('should include targetAudience in prompt when provided', async () => {
    ...
    await skill.execute(
      { pages, presentationTitle: 'Deck', language: 'en', targetAudience: 'executives' },
      buildSkillContext(),
    );
