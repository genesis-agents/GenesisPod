...
describe('SlideThinkingSkill', () => {
  ...
  beforeEach(async () => {
    ...
    emitSpy = eventEmitter.emit as jest.MockedFunction<typeof eventEmitter.emit>;
  });
  ...
  describe('execute()', () => {
    ...
    it('should emit correct type prefix for each entry type', async () => {
      const types: ThinkingInput['type'][] = ['step', 'decision', 'insight', 'warning', 'output'];

      for (const type of types) {
        jest.clearAllMocks();
        const input = buildThinkingInput({ type, missionId: 'mission-type-test' });
    ...
    it('should keep entries separate for different missions', async () => {
      await skill.execute(buildThinkingInput({ missionId: 'mission-A' }), buildSkillContext());
      await skill.execute(buildThinkingInput({ missionId: 'mission-B' }), buildSkillContext());
  ...
  describe('getSummary()', () => {
    ...
    it('should count entries by type correctly', async () => {
      ...
      await skill.execute(buildThinkingInput({ missionId, type: 'step' }), buildSkillContext());
      await skill.execute(buildThinkingInput({ missionId, type: 'step' }), buildSkillContext());
      await skill.execute(buildThinkingInput({ missionId, type: 'decision', decision: 'Decision1' }), buildSkillContext());
      await skill.execute(buildThinkingInput({ missionId, type: 'insight' }), buildSkillContext());
      await skill.execute(buildThinkingInput({ missionId, type: 'warning' }), buildSkillContext());
    ...
    it('should include key decisions in summary', async () => {
      ...
      await skill.execute(
        buildThinkingInput({ missionId, type: 'decision', decision: 'Use pillars template' }),
        buildSkillContext(),
      );
      await skill.execute(
        buildThinkingInput({ missionId, type: 'decision', decision: 'Add data chart' }),
        buildSkillContext(),
      );
    ...
    it('should include insights in summary', async () => {
      ...
      await skill.execute(
        buildThinkingInput({ missionId, type: 'insight', content: 'Content is too dense' }),
        buildSkillContext(),
      );
    ...
    it('should cap key decisions at 10', async () => {
      ...
      for (let i = 0; i < 15; i++) {
        await skill.execute(
          buildThinkingInput({ missionId, type: 'decision', decision: `Decision ${i}` }),
          buildSkillContext(),
        );
      }
    ...
    it('should calculate total duration', async () => {
      ...
      await skill.execute(buildThinkingInput({ missionId }), buildSkillContext());
      await skill.execute(buildThinkingInput({ missionId }), buildSkillContext());
  ...
  describe('clearEntries()', () => {
    ...
    it('should not affect entries of other missions', async () => {
      await skill.execute(buildThinkingInput({ missionId: 'keep' }), buildSkillContext());
      await skill.execute(buildThinkingInput({ missionId: 'delete' }), buildSkillContext());
  ...
  describe('getMemoryStats()', () => {
    ...
    it('should count missions and entries correctly', async () => {
      ...
      const freshSkill = freshModule.get<SlideThinkingSkill>(SlideThinkingSkill);

      await freshSkill.execute(buildThinkingInput({ missionId: 'stats-m1' }), buildSkillContext());
      await freshSkill.execute(buildThinkingInput({ missionId: 'stats-m1' }), buildSkillContext());
      await freshSkill.execute(buildThinkingInput({ missionId: 'stats-m2' }), buildSkillContext());
