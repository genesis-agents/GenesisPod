...
import { PageContent, PageOutline, ContentSection, StatContent } from '../../checkpoint/checkpoint.types';
...
describe('ContentCompressionSkill', () => {
  ...
  describe('execute', () => {
    it('should compress content and return result', async () => {
      const validSections: ContentSection[] = [
        ...
        buildListSection(['Growth trend 1: 20% YoY', 'Growth trend 2: key markets', 'Trend 3: competitive landscape']),
        buildTextSection('Market analysis shows strong growth and positive outlook'),
      ];
    ...
    it('should return failure when AIEngineFacade is not available', async () => {
    ...
    it('should include token usage in metadata', async () => {
      const validSections: ContentSection[] = [
        ...
        buildListSection(['Detail 1: with specifics', 'Detail 2: with data', 'Detail 3: more info']),
    ...
    it('should not supplement data for toc page type', async () => {
      ...
      const validSections: ContentSection[] = [buildTextSection('Table of contents content')];
      aiFacade.chat.mockResolvedValueOnce({
        content: buildValidJsonResponse(validSections),
        tokensUsed: 100,
      });
    ...
    it('should attempt data supplement for non-cover slides', async () => {
      const validSections: ContentSection[] = [
        ...
        buildListSection(['Insight 1: quantified finding', 'Insight 2: more detail', 'Insight 3: conclusion']),
    ...
    it('should handle data supplement failure gracefully', async () => {
      const validSections: ContentSection[] = [
        ...
        buildListSection(['Key point 1 detail', 'Key point 2 detail', 'Key point 3 detail']),
      ...
      dataSupplementSkill.execute.mockRejectedValueOnce(new Error('Supplement failed'));
    ...
    it('should include retry context in message when provided', async () => {
      const validSections: ContentSection[] = [
        ...
        buildListSection(['Improved point 1 with data', 'Improved point 2 concrete', 'Improved point 3 specific']),
    ...
    it('should handle OrchestratorInput format successfully', async () => {
      const validSections: ContentSection[] = [
        ...
        buildListSection(['Process 1: streamlined', 'Process 2: automated', 'Process 3: optimized']),
    ...
    it('should truncate very long source text at 8000 chars', async () => {
      ...
      const validSections: ContentSection[] = [
        ...
        buildListSection(['Summary point 1: from analysis', 'Summary point 2: key finding']),
      ];
      aiFacade.chat.mockResolvedValueOnce({
        content: buildValidJsonResponse(validSections),
        tokensUsed: 200,
  ...
  describe('executeBatch', () => {
    it('should compress multiple pages in batches', async () => {
      const validSections: ContentSection[] = [
        ...
        buildListSection(['Batch item 1 detail', 'Batch item 2 detail', 'Batch item 3 detail']),
    ...
    it('should handle individual batch failures with fallback', async () => {
      aiFacade.chat
        .mockResolvedValueOnce({
          content: buildValidJsonResponse([buildTextSection('Valid content page')]),
          tokensUsed: 100,
        })
  ...
  describe('willOverflow', () => {
    it('should return overflow=false for small content', () => {
      const content = buildPageContent('Simple', [buildTextSection('Short text')]);
    ...
    it('should return overflow=true when ContentAnalyzer says it does not fit', () => {
      ...
      contentAnalyzerSkill.analyze.mockReturnValueOnce({
        ...
        pillars: { detected: false, count: 0 },
        timeline: { detected: false, nodeCount: 0 },
        sectionTypes: { stat: 0, list: 0, text: 8, chart: 0, image: 0, quote: 0 },
    ...
    it('should include analysis in result', () => {
      ...
      contentAnalyzerSkill.analyze.mockReturnValueOnce({
        estimatedCapacity: { fitsOnOnePage: true, suggestedPageCount: 1, overflowSections: 0 },
      } as any);
  ...
  describe('compressSection', () => {
    ...
    it('should compress list sections by limiting items', () => {
      const section = buildListSection(['Item 1', 'Item 2', 'Item 3', 'Item 4', 'Item 5']);
  ...
  describe('autoCompress', () => {
    it('should trim sections exceeding capacity', () => {
      ...
      contentAnalyzerSkill.analyze.mockReturnValue({
        ...
        pillars: { detected: false, count: 0 },
        timeline: { detected: false, nodeCount: 0 },
        sectionTypes: { stat: 0, list: 0, text: 6, chart: 0, image: 0, quote: 0 },
        visualComplexity: 'simple',
        estimatedCapacity: { fitsOnOnePage: true, suggestedPageCount: 1, overflowSections: 0 },
      } as any);
    ...
    it('should truncate title over 50 chars', () => {
      contentAnalyzerSkill.analyze.mockReturnValue({
        ...
        pillars: { detected: false, count: 0 },
        timeline: { detected: false, nodeCount: 0 },
        sectionTypes: { stat: 0, list: 0, text: 0, chart: 0, image: 0, quote: 0 },
        visualComplexity: 'simple',
        estimatedCapacity: { fitsOnOnePage: true, suggestedPageCount: 1, overflowSections: 0 },
      } as any);
  ...
  describe('analyzeContent', () => {
    it('should delegate to contentAnalyzer.analyze', () => {
      ...
      contentAnalyzerSkill.analyze.mockReturnValueOnce({ totalSections: 1 } as any);
