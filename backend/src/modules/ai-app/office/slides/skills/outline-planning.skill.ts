...
请为每一页生成详细的大纲规划，输出完整的 JSON 格式结果。`;
  ...
  /**
   * 解析 AI 响应
   * 处理各种格式：完整 JSON、markdown 包裹、截断的响应
   */
  private parseResponse(
    content: string,
    taskDecomposition: TaskDecomposition,
  ): OutlinePlan {
    // 1. 尝试提取 markdown 代码块中的 JSON（支持截断的响应）
    let jsonStr = content;

    // 完整的 markdown 代码块
    const completeMatch = content.match(/```json\s*([\s\S]*?)\s*```/);
    if (completeMatch) {
      jsonStr = completeMatch[1];
    } else {
      // 截断的 markdown 代码块（只有开头没有结尾）
      const truncatedMatch = content.match(/```json\s*([\s\S]*)/);
      if (truncatedMatch) {
        jsonStr = truncatedMatch[1];
        this.logger.warn(
          "[parseResponse] Detected truncated markdown, attempting to repair JSON",
        );
      }
    }
    ...
    try {
      ...
      this.logger.debug(
        `[parseResponse] Failed JSON (first 500 chars): ${jsonStr.substring(0, 500)}`,
      );
  ...
  private repairTruncatedJson(json: string): string {
    ...
    // 移除末尾不完整的 key-value 对（如 "key": 或 "key": "val）
  ...
  private ensureRequiredPages(
    ...
    if (!hasToc) {
      ...
      const tocPage: PageOutline = {
        ...
        keyElements: (taskDecomposition?.chapters || []).map(
          (ch) => ch?.title || "章节",
        ),
    ...
    // v3.5: 注入章节分隔页
    const pagesWithChapters = this.ensureChapterSeparators(
      result,
      taskDecomposition,
    );
  ...
  /**
   * 生成默认页面
   * 防御性处理：确保即使 taskDecomposition 不完整也能生成有效页面
   */
  private generateDefaultPages(
    taskDecomposition: TaskDecomposition,
  ): PageOutline[] {
    const pages: PageOutline[] = [];
    // 安全获取 totalPages，默认 10 页
    const totalPages =
      typeof taskDecomposition?.totalPages === "number"
        ? taskDecomposition.totalPages
        : 10;
    // 安全获取 chapters
    const chapters = Array.isArray(taskDecomposition?.chapters)
      ? taskDecomposition.chapters
      : [];
    ...
    pages.push({
      ...
      templateType: "toc",
      contentBrief: "内容目录",
      keyElements: chapters.map((ch) => ch?.title || "章节"),
      layoutHints: [{ type: "alignment", value: "left" }],
    });

    // 内容页
    const contentPageCount = Math.max(0, totalPages - 3);
    ...
    // 总结页
    pages.push({
      pageNumber: Math.max(totalPages, 3),
      title: "总结与建议",
      logicType: "parallel",
      templateType: "recommendations",
