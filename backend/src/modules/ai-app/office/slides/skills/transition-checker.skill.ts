/**
 * Slides Engine v4.0 - Transition Checker Skill
 *
 * 过渡检查技能 (Layer Quality)：检查页面之间的过渡质量
 * 确保演示文稿的叙事流畅性
 ...
 * 过渡检查输入
 */
export interface TransitionCheckerInput {
  /** 页面状态列表 */
  pages: PageState[];
  /** 会话 ID */
  sessionId?: string;
  ...
  implements ISkill<TransitionCheckerInput, TransitionCheckerResult>
{
  ...
  // ISkill 接口必需属性
  readonly id = "slides-transition-checker";
  readonly name = "过渡检查";
  readonly description = "检查幻灯片之间的过渡逻辑";
  ...
  async execute(
    ...
    try {
      ...
      let result: TransitionCheckResult;

      // 如果规则检测到问题或页面较多，使用 AI 深度分析
      if (ruleBasedIssues.length > 2 || pages.length > 8) {
        result = await this.checkWithAI(pages, ruleBasedTransitions, sessionId);
      } else {
        // 否则返回规则检测结果
        result = {
...
请深度分析页面过渡质量，输出完整的检查报告（JSON 格式）。`;
    ...
    try {
      ...
      return this.parseResponse(response.content, ruleBasedTransitions);
    } catch (error) {
      this.logger.error("[checkWithAI] AI call failed:", error);
      return {
        transitions: ruleBasedTransitions,
        issueCount: ruleBasedTransitions.filter((t) => t.quality !== "smooth")
          .length,
