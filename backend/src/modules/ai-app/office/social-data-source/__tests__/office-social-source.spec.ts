...
import { SourceListFilter } from '../../../contracts/social-data-source';
...
describe('OfficeSocialSourceProvider', () => {
  describe('static metadata', () => {
    it('has correct id and displayName', () => {
      const provider = new OfficeSocialSourceProvider(makePrisma([]));
  ...
  describe('listItems', () => {
    ...
    it('maps ARTICLE type to contentKind article', async () => {
      ...
      const provider = new OfficeSocialSourceProvider(makePrisma([doc]));
      const result = await provider.listItems('user-a', {});
    ...
    it('maps REPORT and RESEARCH types to contentKind article', async () => {
      ...
      const provider = new OfficeSocialSourceProvider(makePrisma(docs));
      const result = await provider.listItems('user-a', {});
      expect(result.items[0].contentKind).toBe('article');
    ...
    it('returns nextCursor when there are more items than the limit', async () => {
      ...
      const provider = new OfficeSocialSourceProvider(prisma);
    ...
    it('passes search filter and cursor to prisma', async () => {
      ...
      const provider = new OfficeSocialSourceProvider(prisma);
      const filter: SourceListFilter = { search: 'hello', cursor: 'abc', limit: 10 };
  ...
  describe('fetchBundle', () => {
    ...
    it('does NOT return documents owned by a different user', async () => {
      ...
      const provider = new OfficeSocialSourceProvider(prismaReturningOnlyUserA);
      ...
      expect(prismaReturningOnlyUserA.officeDocument.findMany).toHaveBeenCalledWith(
        expect.objectContaining({
