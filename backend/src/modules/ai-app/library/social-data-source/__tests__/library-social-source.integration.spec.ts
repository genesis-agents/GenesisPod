...
const makeNoteListRow = (overrides: Partial<{
  id: string;
  title: string | null;
  content: string;
  tags: unknown;
...
const makeKbDocListRow = (overrides: Partial<{
  id: string;
  title: string;
...
const makeNoteBundleRow = (overrides: Partial<{
  id: string;
  title: string | null;
  content: string;
  tags: unknown;
...
const makeKbDocBundleRow = (overrides: Partial<{
  id: string;
  title: string;
  rawContent: string;
  rawContentUri: string | null;
...
describe('LibrarySocialSourceProvider — userId isolation (integration)', () => {
  let provider: LibrarySocialSourceProvider;

  beforeEach(() => {
    jest.clearAllMocks();
    provider = new LibrarySocialSourceProvider(mockPrisma);
    mockNoteFindMany.mockResolvedValue([]);
    mockKbDocFindMany.mockResolvedValue([]);
  });

  it('uses the real LibrarySocialSourceProvider class', () => {
    expect(provider).toBeInstanceOf(LibrarySocialSourceProvider);
