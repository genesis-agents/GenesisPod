...
const makeMockService = () => ({
  getAvailableModels: jest.fn().mockResolvedValue({ textModels: [], imageModels: [] }),
  generateImage: jest.fn().mockResolvedValue({ id: 'img-1', imageUrl: 'http://example.com/img.png' }),
  generateImageStream: jest.fn().mockReturnValue(of({ data: JSON.stringify({ type: 'complete', result: {} }) })),
  getHistory: jest.fn().mockResolvedValue([]),
  getBookmarkedImages: jest.fn().mockResolvedValue([]),
  getPublicImage: jest.fn().mockResolvedValue({ id: 'img-1', imageUrl: 'http://example.com/img.png' }),
...
describe('AiImageController', () => {
  ...
  describe('generateImage()', () => {
    it('calls service.generateImage with userId from request', async () => {
      ...
      expect(result).toEqual({ id: 'img-1', imageUrl: 'http://example.com/img.png' });
    ...
    it('fetches referenceImageUrl and converts to base64 when imageBase64 is absent', async () => {
      ...
      const dto = { prompt: 'Test', referenceImageUrl: 'https://example.com/ref.png' };
      const req = makeAuthReq('user-1');
    ...
    it('continues without imageBase64 when referenceImageUrl fetch fails', async () => {
      ...
      const dto = { prompt: 'Test', referenceImageUrl: 'https://example.com/fail.png' };
      const req = makeAuthReq('user-1');
    ...
    it('continues without imageBase64 when fetch response is not ok', async () => {
      ...
      const dto = { referenceImageUrl: 'https://example.com/notfound.png', prompt: 'Test' };
      const req = makeAuthReq('user-1');
  ...
  describe('generateImageStream() GET', () => {
    ...
    it('passes undefined for urls when urls query param is empty', () => {
      const req = makeAuthReq('user-1');
      controller.generateImageStream('Test', '', '', '', '', '', '', '', '', '', req);
    ...
    it('accepts valid aspectRatio values', () => {
      ...
      for (const ratio of validRatios) {
        ...
        const req = makeAuthReq('user-1');
        controller.generateImageStream('Test', '', '', '', '', '', ratio, '', '', '', req);
        expect(mockService.generateImageStream).toHaveBeenCalledWith(
    ...
    it('passes undefined for invalid aspectRatio', () => {
      const req = makeAuthReq('user-1');
      controller.generateImageStream('Test', '', '', '', '', '', 'bad-ratio', '', '', '', req);
    ...
    it('accepts valid templateLayout values', () => {
      const validLayouts = ['cards', 'center_visual', 'timeline', 'comparison', 'pyramid', 'radial'];
      for (const layout of validLayouts) {
        mockService.generateImageStream.mockClear();
        const req = makeAuthReq('user-1');
        controller.generateImageStream('Test', '', '', '', '', '', '', '', '', layout, req);
        expect(mockService.generateImageStream).toHaveBeenCalledWith(
    ...
    it('passes undefined for invalid templateLayout', () => {
      const req = makeAuthReq('user-1');
      controller.generateImageStream('Test', '', '', '', '', '', '', '', '', 'invalid-layout', req);
    ...
    it('converts skipEnhancement string "true" to boolean true', () => {
      const req = makeAuthReq('user-1');
      controller.generateImageStream('Test', '', '', '', '', '', '', '', 'true', '', req);
    ...
    it('converts skipEnhancement string other than "true" to boolean false', () => {
      const req = makeAuthReq('user-1');
      controller.generateImageStream('Test', '', '', '', '', '', '', '', 'false', '', req);
    ...
    it('returns Observable from service', () => {
      const req = makeAuthReq('user-1');
      const observable = controller.generateImageStream('Test', '', '', '', '', '', '', '', '', '', req);
  ...
  describe('generateImageStreamPost() POST', () => {
    it('sets SSE response headers', () => {
      ...
      mockService.generateImageStream.mockReturnValueOnce(subject.asObservable());
    ...
    it('writes SSE data: events for each next emission', () => {
      ...
      expect(res.write).toHaveBeenCalledWith('data: {"type":"complete","result":{}}\n\n');
    });
    ...
    it('unsubscribes on res close event', () => {
      ...
      const closeCallback = (res.on as jest.Mock).mock.calls[0][1] as () => void;
      closeCallback();
  ...
  describe('generateImageWithFiles()', () => {
    it('processes multer files into fileContents array', async () => {
      ...
      expect(mockService.generateImage).toHaveBeenCalledWith(
        expect.objectContaining({
          files: [
            { buffer: files[0].buffer, mimeType: 'application/pdf', filename: 'doc.pdf' },
          ],
        }),
      );
    });
    ...
    it('passes empty files array when no files uploaded', async () => {
      const req = makeAuthReq('user-1');
      await controller.generateImageWithFiles([], { prompt: 'Test' } as never, req);
  ...
  describe('getPublicImage()', () => {
    ...
    it('throws NotFoundException when service returns null', async () => {
      ...
      await expect(controller.getPublicImage('missing-id')).rejects.toThrow(NotFoundException);
    });
  });
  ...
  describe('getImageStats()', () => {
    ...
    it('throws ForbiddenException when key is wrong', async () => {
      await expect(controller.getImageStats('wrong-key')).rejects.toThrow(ForbiddenException);
    });

    it('throws ForbiddenException when key is empty', async () => {
      await expect(controller.getImageStats('')).rejects.toThrow(ForbiddenException);
    });
  });
  ...
  describe('adminDeleteAllImages()', () => {
    it('deletes all images and returns count with correct key', async () => {
      ...
      expect(result).toEqual({ deletedCount: 10, message: 'Deleted 10 images' });
      expect(mockService.deleteAllImages).toHaveBeenCalled();
    });

    it('throws ForbiddenException when key is wrong', async () => {
      await expect(controller.adminDeleteAllImages('bad-key')).rejects.toThrow(ForbiddenException);
    });

    it('throws ForbiddenException when key is missing', async () => {
      await expect(controller.adminDeleteAllImages('')).rejects.toThrow(ForbiddenException);
    });
  });
  ...
  describe('removeBookmark()', () => {
    it('calls service.removeBookmark with id and userId', async () => {
      ...
      expect(mockService.removeBookmark).toHaveBeenCalledWith('img-1', 'user-1');
  ...
  describe('updateVisibility()', () => {
    it('calls service.updateVisibility with id, visibility and userId', async () => {
      ...
      expect(mockService.updateVisibility).toHaveBeenCalledWith('img-1', 'PUBLIC', 'user-1');
    ...
    it('works with PRIVATE visibility', async () => {
      ...
      expect(mockService.updateVisibility).toHaveBeenCalledWith('img-1', 'PRIVATE', 'user-1');
    });
  });
  ...
  describe('cleanupOldImages()', () => {
    it('calls service.cleanupOldImages with user id', async () => {
      ...
      expect(result).toEqual({ deletedCount: 3, message: 'Cleaned up 3 old images' });
    });
  ...
  describe('adminCleanupAllImages()', () => {
    ...
    it('throws ForbiddenException with wrong key', async () => {
      await expect(controller.adminCleanupAllImages('wrong')).rejects.toThrow(ForbiddenException);
    });
  });
  ...
  describe('autoTagImages()', () => {
    ...
    it('throws UnauthorizedException when user is not authenticated', async () => {
      const req = makeAuthReq();
      await expect(controller.autoTagImages(req)).rejects.toThrow(UnauthorizedException);
    });
  });
  ...
  describe('analyzeStyles()', () => {
    ...
    it('throws UnauthorizedException when user is not authenticated', async () => {
      const req = makeAuthReq();
      await expect(controller.analyzeStyles(req)).rejects.toThrow(UnauthorizedException);
    });
  });
  ...
  describe('clusterVisualThemes()', () => {
    ...
    it('throws UnauthorizedException when user is not authenticated', async () => {
      const req = makeAuthReq();
      await expect(controller.clusterVisualThemes(req)).rejects.toThrow(UnauthorizedException);
    });
  });
});
