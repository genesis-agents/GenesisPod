...
const makeMockPrisma = () => ({
  generatedImage: {
    create: jest.fn().mockResolvedValue({ id: 'img-1', createdAt: new Date('2024-01-01') }),
  },
});

const makeMockContentExtractor = () => ({
  extractFromUrl: jest.fn().mockResolvedValue('Extracted URL content that is long enough to pass validation checks and exceed minimum length'),
  extractFromFile: jest.fn().mockResolvedValue('Extracted file content that is long enough to pass validation checks'),
});

const makeMockInfographicTemplate = () => ({
  generateInfographic: jest.fn().mockResolvedValue('http://storage.example.com/infographic.png'),
});

const makeMockDataFetching = () => ({
  detectDataFetchingNeed: jest.fn().mockReturnValue({ needsFetching: false }),
  processDataFetching: jest.fn().mockResolvedValue({ fetchedData: [], enrichedContent: '' }),
});
...
const makeMockPromptEnhancement = () => ({
  ...
  parsePromptEnhancementResponse: jest.fn().mockReturnValue(makeDefaultInsights()),
...
const makeMockImageGeneration = () => ({
  getDefaultTextModel: jest.fn().mockResolvedValue({ modelId: 'gpt-4o', displayName: 'GPT-4o' }),
  ...
  callImageGenerationAPI: jest.fn().mockResolvedValue('http://example.com/image.png'),
});

const makeMockImageStorage = () => ({
  uploadImageToStorage: jest.fn().mockResolvedValue('http://storage.example.com/image.png'),
  ...
  deleteImage: jest.fn().mockResolvedValue({ success: true, message: 'Deleted' }),
  getBookmarkedImages: jest.fn().mockResolvedValue([]),
  addBookmark: jest.fn().mockResolvedValue({ success: true, message: 'Bookmarked' }),
  removeBookmark: jest.fn().mockResolvedValue({ success: true, message: 'Removed' }),
  updateVisibility: jest.fn().mockResolvedValue({ success: true, message: 'Updated' }),
  cleanupAllUsersImages: jest.fn().mockResolvedValue({ totalDeleted: 5, usersCleaned: 2, orphanDeleted: 1 }),
  ...
  getAvailableModels: jest.fn().mockImplementation((type: AIModelType) => {
    if (type === AIModelType.CHAT) {
      return Promise.resolve([
        { dbId: 'db-1', id: 'gpt-4o', name: 'GPT-4o', provider: 'openai', icon: undefined, isDefault: true },
      ]);
    }
    return Promise.resolve([
      { dbId: 'db-img-1', id: 'dall-e-3', name: 'DALL-E 3', provider: 'openai', icon: 'openai-icon', isDefault: false },
    ]);
  }),
});
...
describe('AiImageService', () => {
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        ...
        { provide: InfographicTemplateService, useValue: mockInfographicTemplate },
    ...
    jest.spyOn(BillingContext, 'run').mockImplementation((_ctx, fn) => fn() as ReturnType<typeof fn>);
  });
  ...
  describe('getAvailableModels()', () => {
    ...
    it('falls back to model id when dbId is absent', async () => {
      mockAIFacade.getAvailableModels.mockImplementation((type: AIModelType) => {
        if (type === AIModelType.CHAT) {
          return Promise.resolve([
    ...
    it('calls aiFacade.getAvailableModels with CHAT and IMAGE_GENERATION types', async () => {
      ...
      expect(mockAIFacade.getAvailableModels).toHaveBeenCalledWith(AIModelType.CHAT);
      expect(mockAIFacade.getAvailableModels).toHaveBeenCalledWith(AIModelType.IMAGE_GENERATION);
    });
  });
  ...
  describe('generateImageStream() - with prompt only', () => {
    it('emits step events and a complete event', async () => {
      const events = await collectStreamEvents(
        service.generateImageStream({ prompt: 'A beautiful landscape', userId: 'user-1' }),
      );
    ...
    it('complete event contains expected result fields', async () => {
      const events = await collectStreamEvents(
        service.generateImageStream({ prompt: 'Test prompt', userId: 'user-1' }),
      );
    ...
    it('does not trigger cleanupOldImages when userId is absent', async () => {
      await collectStreamEvents(
        service.generateImageStream({ prompt: 'A prompt that is definitely long enough' }),
      );
    ...
    it('does not use BillingContext when userId is absent', async () => {
      await collectStreamEvents(
        service.generateImageStream({ prompt: 'A sufficiently long prompt text here' }),
      );
  ...
  describe('generateImageStream() - skipEnhancement=true', () => {
    it('does not call imagen4PromptService when skipEnhancement is true', async () => {
      await collectStreamEvents(
        service.generateImageStream({ prompt: 'Direct prompt', skipEnhancement: true }),
      );
    ...
    it('uses direct input as image prompt', async () => {
      ...
      const stepEvents = events.filter((e) => e.type === 'step');
      const directStep = stepEvents.find(
        (e) => (e.step as { title?: string })?.title === 'Using Direct Input' ||
               (e as { step?: { title?: string } }).step?.title === 'Using Direct Input',
      );
  ...
  describe('generateImageStream() - with URLs', () => {
    it('calls contentExtractor.extractFromUrl for each URL', async () => {
      ...
      expect(mockContentExtractor.extractFromUrl).toHaveBeenCalledWith('https://example.com/article1');
      expect(mockContentExtractor.extractFromUrl).toHaveBeenCalledWith('https://example.com/article2');
    });
    ...
    it('emits error step when URL extraction throws', async () => {
      mockContentExtractor.extractFromUrl.mockRejectedValueOnce(new Error('Network error'));
  ...
  describe('generateImageStream() - with files', () => {
    ...
    it('emits error step and propagates when file extraction fails', async () => {
      mockContentExtractor.extractFromFile.mockRejectedValueOnce(new Error('Parse error'));
  ...
  describe('generateImageStream() - fallback to single LLM', () => {
    it('falls back to single LLM when imagen4PromptService throws', async () => {
      mockImagen4Prompt.generateImagen4Prompt.mockRejectedValueOnce(new Error('Team error'));
  ...
  describe('generateImageStream() - html_render mode', () => {
    ...
    it('calls infographicTemplate.generateInfographic in html_render mode', async () => {
      const events = await collectStreamEvents(
        service.generateImageStream({ prompt: 'Create infographic', userId: 'user-1' }),
      );
    ...
    it('maps sections to InfographicSection format correctly', async () => {
      ...
      const callArg = mockInfographicTemplate.generateInfographic.mock.calls[0][0];
    ...
    it('propagates error when generateInfographic throws', async () => {
      mockInfographicTemplate.generateInfographic.mockRejectedValueOnce(new Error('Render failed'));
  ...
  describe('generateImageStream() - hybrid mode', () => {
    ...
    it('uses custom imageModelId in hybrid mode', async () => {
      await collectStreamEvents(
        service.generateImageStream({ prompt: 'Hybrid test', imageModelId: 'custom-model' }),
      );
      expect(mockImageGeneration.getModelById).toHaveBeenCalledWith('custom-model');
    });
    ...
    it('continues with solid background when background generation fails', async () => {
      mockImageGeneration.callImageGenerationAPI.mockRejectedValueOnce(new Error('API error'));
  ...
  describe('generateImageStream() - ai_image mode no apiKey', () => {
    ...
    it('uses custom imageModelId when provided', async () => {
      await collectStreamEvents(
        service.generateImageStream({ prompt: 'Test', imageModelId: 'custom-img-model' }),
      );
      expect(mockImageGeneration.getModelById).toHaveBeenCalledWith('custom-img-model');
    });
  });
  ...
  describe('generateImageStream() - data fetching', () => {
    ...
    it('continues when detectDataFetchingNeed returns needsFetching=false', async () => {
      mockDataFetching.detectDataFetchingNeed.mockReturnValueOnce({ needsFetching: false });
  ...
  describe('generateImageStream() - step emission for insights', () => {
    it('emits journal steps when designJournal has entries', async () => {
      ...
      const journalSteps = stepEvents.filter((e) =>
        String((e.step as { step?: string })?.step || '').startsWith('prompt_journal_'),
      );
  ...
  describe('generateImage()', () => {
    ...
    it('rejects when an error event is emitted', async () => {
      ...
      mockImagen4Prompt.generateImagen4Prompt.mockRejectedValueOnce(new Error('Team failure'));
      mockImageGeneration.getDefaultTextModel.mockResolvedValueOnce(null);
    ...
    it('rejects when stream completes without result', async () => {
      ...
      jest.spyOn(service, 'generateImageStream').mockReturnValueOnce(
        rxOf({ data: JSON.stringify({ type: 'step', step: {} }) }),
      );

      await expect(
        service.generateImage({ prompt: 'Test' }),
      ).rejects.toThrow('Generation completed without result');
    });
  });
    ...
    it('creates fallback bullets from imagePrompt when sections are empty', async () => {
      mockImagen4Prompt.generateImagen4Prompt.mockResolvedValueOnce({
        ...
        insights: {
          ...makeDefaultInsights(),
          renderingMode: 'html_render',
          imagePrompt: 'First sentence. Second sentence. Third sentence. Fourth sentence. Fifth sentence.',
          informationArchitecture: {
  ...
  describe('generateImageStream() - URL with inline description', () => {
    it('appends user description to content parts when URL has trailing description', async () => {
      ...
      expect(mockContentExtractor.extractFromUrl).toHaveBeenCalledWith('https://example.com/article');
      expect(events.find((e) => e.type === 'complete')).toBeDefined();
    });
  });
  ...
  describe('generateImageStream() - imagen4 progress callback', () => {
    it('invokes progress callback and emits agent phase step events', async () => {
      mockImagen4Prompt.generateImagen4Prompt.mockImplementationOnce(
        async (_opts: unknown, progressCb: (event: { phase: string; status: string; message?: string }) => void) => {
          // Simulate the callback being called by imagen4PromptService
          progressCb({ phase: 'content', status: 'started', message: 'Starting content analysis' });
          progressCb({ phase: 'layout', status: 'completed', message: 'Layout done' });
          progressCb({ phase: 'visual', status: 'in_progress', message: 'Visual in progress' });
          progressCb({ phase: 'style', status: 'failed', message: 'Style failed' });
          progressCb({ phase: 'complete', status: 'completed', message: undefined });
          return {
      ...
      const agentSteps = stepEvents.filter((e) => {
        const stepObj = e.step as { step?: string } | undefined;
        return typeof stepObj?.step === 'string' && stepObj.step.startsWith('agent_');
      });
      ...
      const allStepsArr = (lastStepEvent?.allSteps as Array<{ step: string }>) ?? [];
      const agentStepsInAll = allStepsArr.filter((s) => s.step?.startsWith('agent_'));
  ...
  describe('generateImage() - Observable error subscriber', () => {
    it('rejects via Observable error (not data.type error) path', async () => {
      ...
      await expect(
        service.generateImage({ prompt: 'Test' }),
      ).rejects.toThrow('Observable level error');
    });
  });
  ...
  describe('storage delegation methods', () => {
    ...
    it('deleteImage delegates to imageStorageService.deleteImage', async () => {
      ...
      expect(mockImageStorage.deleteImage).toHaveBeenCalledWith('img-1', 'user-1');
    });
    ...
    it('updateVisibility delegates to imageStorageService.updateVisibility', async () => {
      ...
      expect(mockImageStorage.updateVisibility).toHaveBeenCalledWith('img-1', 'PUBLIC', 'user-1');
    });
    ...
    it('cleanupAllUsersImages delegates to imageStorageService.cleanupAllUsersImages', async () => {
      ...
      expect(result).toEqual({ totalDeleted: 5, usersCleaned: 2, orphanDeleted: 1 });
    });
    ...
    it('clusterVisualThemes delegates to imageStorageService.clusterVisualThemes', async () => {
      ...
      expect(mockImageStorage.clusterVisualThemes).toHaveBeenCalledWith('user-1');
