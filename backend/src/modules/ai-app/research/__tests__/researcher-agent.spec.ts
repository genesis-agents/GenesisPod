...
import { RESEARCH_SERVICE_TOKEN } from '../../../ai-engine/interfaces/research.interface';

jest.mock('../../../ai-engine/agents/base/plan-based-agent', () => {
...
jest.mock('../../../ai-engine/interfaces/research.interface', () => ({
...
describe('ResearcherAgent', () => {
  ...
  describe('plan', () => {
    ...
    it('should include web-search step', async () => {
      ...
      const webSearchStep = plan.steps.find(
        (s) => s.toolId === 'web-search',
      );
    ...
    it('should include data analysis step for data analysis tasks', async () => {
      ...
      const dataStep = plan.steps.find(
        (s) => s.toolId === 'data-analysis',
      );
    ...
    it('should include knowledge graph step when buildKnowledgeGraph is true', async () => {
      ...
      const kgStep = plan.steps.find(
        (s) => s.toolId === 'knowledge-graph',
      );
    ...
    it('should include long-term memory step by default', async () => {
      ...
      const memStep = plan.steps.find(
        (s) => s.toolId === 'long-term-memory',
      );
