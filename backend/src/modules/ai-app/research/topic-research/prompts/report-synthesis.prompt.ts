...
报告必须包含 3-6 个数据可视化图表。**图表必须根据正文内容的需求来生成**，而不是为了图表而图表。

### 核心原则

⚠️ **图表必须服务于正文内容** ⚠️

1. 先写正文分析，当正文提到具体数据、对比、趋势时，才在该位置生成图表
2. 图表必须嵌入到相关章节的 \`inlineCharts\` 数组中
3. 图表的 \`position\` 字段指定在章节内的渲染位置

### 图表嵌入方式【新格式】

每个章节的 \`inlineCharts\` 数组包含该章节需要的图表：
  ...
  "sections": [
    {
      "sectionNumber": "1",
      "title": "市场规模分析",
      "content": "根据多家机构数据，全球AI市场规模预计从2024年的5400亿美元增长至2030年的1.8万亿美元。...",
      "inlineCharts": [
        {
...
- \`"after_paragraph_N"\`: 在第 N 段落之后渲染（N=1,2,3...）
- \`"after_heading_N"\`: 在第 N 个小标题之后渲染
- \`"end_of_section"\`: 在章节末尾渲染

### 正确示例

章节内容分析某数据后，在 inlineCharts 中添加图表：
- 正文第1段讨论市场规模趋势 → inlineCharts 添加 \`{"position": "after_paragraph_1", "type": "line", ...}\`
- 正文第3段对比中美差距 → inlineCharts 添加 \`{"position": "after_paragraph_3", "type": "bar", ...}\`

### 错误示例（禁止）

❌ 所有图表放在一个单独的"数据可视化"章节
❌ 图表与正文内容无关（为了图表而图表）
❌ 使用旧的 charts 顶层数组（已废弃）
...
**7. 图表数据要求【重要】**
- 图表必须根据正文内容的需求来生成，而不是为了图表而图表
- 当正文提到具体数据、对比、趋势时，在该章节的 inlineCharts 数组中添加图表
- 图表类型根据数据特点选择：趋势用 line、对比用 bar、占比用 pie、分布用 area、多维用 radar
- 每个图表必须指定 position 字段（如 "after_paragraph_1"）表示在章节内的渲染位置
- 图表数据必须基于报告中引用的真实数据点，不可虚构
- 3-6 个图表应分散在不同章节的 inlineCharts 中，禁止集中在一个章节
