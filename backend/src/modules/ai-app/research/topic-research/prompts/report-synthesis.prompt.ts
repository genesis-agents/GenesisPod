...
**每个图表必须嵌入到相关章节的 content 字段中**，使用占位符标记位置：
...
在 sections 数组的某个章节 content 中：
\`\`\`markdown
根据多家机构数据，全球AI市场规模预计从2024年的5400亿美元增长至2030年的1.8万亿美元。

<!-- chart:chart-global-ai-market -->

从上图可以看出，美国和中国是两大主要市场...
\`\`\`

### 错误示例（禁止）

❌ 所有图表放在报告最后的"数据可视化"章节
❌ 图表占位符不在任何 section.content 中
❌ charts 数组中的图表没有对应的占位符

### 强制规则

1. **每个 chart 必须有占位符**：charts 数组中的每个图表，都必须在某个 section.content 中有对应的 \`<!-- chart:图表id -->\` 占位符
2. **占位符位置**：紧跟相关数据分析段落之后，不是章节开头或结尾
3. **id 必须匹配**：占位符中的 id 必须与 charts 数组中的 id 完全一致
4. **分散到各章节**：3-6 个图表应分散在不同章节中，而不是集中在一个章节
  ...
  "sections": [
    {
      ...
      "content": "章节完整内容（Markdown格式，⚠️ 必须包含图表占位符如 <!-- chart:chart-1 --> 在相关数据分析段落后）",
