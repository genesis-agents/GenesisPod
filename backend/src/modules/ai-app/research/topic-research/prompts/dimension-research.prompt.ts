...
### 图表使用原则
1. **原图优先**：如果证据中有相关图表，优先引用原始图表（figureReferences）。仅在没有可用原图、且有明确可量化数据时，才在 generatedCharts 中生成图表。不要强制生成图表。
...
### generatedCharts 字段说明（可选）
- 仅在证据中没有可用原图、且有明确可量化数据时才生成图表
...
图表规则：
- **优先使用 Leader 分配的原始参考图表**（在 figureReferences 中引用）
- 仅在没有分配原图、且有明确可量化数据时，才在 generatedCharts 中生成图表
