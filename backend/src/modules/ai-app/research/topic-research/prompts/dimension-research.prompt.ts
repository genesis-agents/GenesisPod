...

{
  ...
  "detailedContent": "完整的维度分析内容（Markdown格式，2000-4000字，包含多个子章节，使用 [n] 格式引用证据，使用 <!-- figure:证据编号:图表序号 --> 格式嵌入图表）",
  "figureReferences": [
    {
      "id": "fig-1",
...
### 图表使用原则
1. **优先原始图表**：如果证据中有相关图表，优先引用原始图表
2. **必须生成图表**：每个维度分析**必须**在 generatedCharts 中提供至少 1-2 个数据可视化图表。从证据中提取关键数据（市场规模、增长率、对比数据等）生成图表。**这是强制要求，不可省略。**
3. **位置合理**：图表应嵌入在相关段落之后，而非集中在文末
4. **图文对应**：每个图表引用前后应有相关的文字说明
...
### generatedCharts 字段说明（必填）
- **每个维度必须至少生成 1 个图表**，展示该维度的关键数据趋势、对比或分布
- 数据必须来源于证据中的具体数据，不要编造
- 常见图表类型：line（趋势）、bar（对比）、pie（占比）、area（增长）
- reason: 必须说明为什么需要生成这个图表
...
**第二部分**（可选）：JSON 格式的图表数据。如果证据中包含可量化数据（数字、百分比、趋势），则生成 1-2 个图表：
...
- figureReferences：只引用证据中实际存在的图片（extractedFigures）
- 没有可量化数据时，不要生成图表，省略 ---CHARTS--- 分隔符`;
