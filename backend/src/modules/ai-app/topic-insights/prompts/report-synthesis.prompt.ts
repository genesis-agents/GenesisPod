...

{
  ...
  "strategicRecommendations": {
    "fullText": "【必填】战略建议完整内容（Markdown格式，分受众建议，600-800字）",
...
**4. 跨维度关联分析【重要】**
- 使用 ### 子标题组织内容结构（如 ### 因果链分析、### 反馈回路、### 维度对比、### 系统性风险）
- 揭示 {{totalDimensions}} 个维度之间的因果关系
- 至少 3 条因果链，每条包含 3 个以上环节，必须用"因素A → 因素B → 结果C"格式
- 每条因果链必须标注时间窗口（如"6-12个月"、"2-3年"）
- 识别至少 1 个反馈回路（自我强化或自我抑制），画出完整循环
- 用**对比表格**呈现维度间的关键差异（如各维度成熟度、风险等级对比）
