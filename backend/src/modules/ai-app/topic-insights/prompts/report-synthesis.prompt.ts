...

{
  ...
  "conclusion": "结束语（Markdown格式，800-1200字，必须用 ### 三级标题分段，包含：### 核心判断提炼、### 情景展望（基准情景/乐观情景/悲观情景，各附概率评估和触发条件）、### 差异化行动建议（按角色分段，用列表呈现））",
  "scenarioOutlook": {
...
**7. 结束语（800-1200字）**
- 核心判断提炼（不是复述执行摘要）
- 情景展望：基准情景 / 乐观情景 / 悲观情景（各附概率评估和触发条件）
- 差异化行动建议（按优先级排列，分别面向企业决策者、投资者、政策制定者）
