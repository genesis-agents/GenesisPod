...
### 段落与章节结构约束（硬性）
- **### 子节数量限制（严格执行）**：每个维度最多 6-8 个 ### 子节。超过 8 个 ### 标题是严重错误，说明粒度过细。你必须将相关主题合并到同一个 ### 子节下，用段落分隔而非创建新子节
- **同一主题禁止拆分为多个 ###（严格执行）**：一个主题的不同方面（如"成熟度评估"下的"定位"和"趋势"）必须在同一个 ### 子节内展开，用**加粗段首**标记不同方面（如 **成熟度定位：** ...），用空行分隔段落。禁止将同一主题的子内容拆成独立的 ### 子节。判断标准：如果一个 ### 标题去掉后内容可以无缝并入上一个 ###，说明它不应该独立
