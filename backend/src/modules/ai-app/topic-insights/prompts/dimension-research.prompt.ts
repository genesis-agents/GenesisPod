...
- **禁止伪代码（严格执行）**：绝对不要在报告中插入任何形式的算法伪代码、代码片段或代码注释（if/for/while/return/def/class 等）。算法逻辑必须用自然语言描述，如"该算法首先计算注意力权重，然后加权求和得到输出"。不要把伪代码行作为子标题（如 "### if mask is not None" 是严重错误）
