...
- **结论独立成段**：总结性内容（如"综上所述..."、"总体来看..."）不能作为列表项出现，必须作为独立段落
...
### 段落与章节结构约束（硬性）
- **### 子节数量限制（严格执行）**：每个维度最多 6-8 个 ### 子节（含"本章要点"引用块）。超过 8 个 ### 标题是严重错误，说明粒度过细。你必须将相关主题合并到同一个 ### 子节下，用段落分隔而非创建新子节
...
- **结论独立成段**：总结性内容（"综上所述..."、"总体来看..."等）不能作为列表项，必须独立成段
...
4. **独立判断**：基于证据给出明确的立场或评价，而非仅列举不同观点。用"数据表明..."、"证据显示..."、"分析表明..."表达判断（每节最多 1 次"我们认为"）。如证据矛盾或不足，应明确说明"当前证据尚不足以得出确定性结论"
5. **量化支撑**：每个核心论点至少关联一个具体数据点，避免"许多"、"大量"等模糊表述
6. **对比表格**：涉及多个方案/产品/方法的对比时，必须用 Markdown 表格呈现，禁止纯文字罗列。表格需含关键维度（性能指标、时间、成本等），每个单元格填写具体数据而非"高/中/低"
7. **引用完整性**：禁止出现悬空引用（如"如图所示"、"展示了这一管线的实践"），每个引用必须指明具体来源编号 [N] 或具体论文/报告名称。引用的图表/数据必须在上下文中实际存在
