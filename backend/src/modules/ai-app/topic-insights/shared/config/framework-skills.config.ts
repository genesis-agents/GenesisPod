 ...
 *
 * 来源：baseline `aaff7b15e`（后续提交至 `38347e2a7`）
 * `src/modules/ai-app/topic-insights/config/framework-skills.config.ts`
 * 完整原文对齐。harness 迁移（bb5fb8b9a）只保留了新的
 * `skills/frameworks/policy.config.ts`（skill-md 扫描数据源），
 * 丢失了 baseline 的**函数接口**：resolveFrameworkSkills /
 * detectEventSubType 以及 **DEBATE_SKILLS_BY_TOPIC_TYPE /
 * RECOMMENDED_DEPTH_BY_TOPIC_TYPE** 两个常量表。
 *
 * Leader Planning 的后处理依赖这些（见 baseline leader-planning.service.ts
 * L423 / L484 / L274）。本文件恢复 baseline 完整契约。
 */
 ...
 * 参考: baseline agent-roles.config.ts 的 ROLE_RECOMMENDATIONS_BY_DEPTH
 */
 ...
 * 当 Leader 分配 DEVIL_ADVOCATE / quality_reviewer 时，这些 skills 会自动注入
 */
