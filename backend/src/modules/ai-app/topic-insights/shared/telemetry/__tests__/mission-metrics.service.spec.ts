/**
 * MissionMetricsService 单元测试（目标架构 v2 · harness 是唯一路径）
 *
...
describe("MissionMetricsService", () => {
  let svc: MissionMetricsService;

  beforeEach(() => {
    svc = new MissionMetricsService();
  });
  ...
  describe("DB persistence (Optional PrismaService)", () => {
    it("recordRun 调用 prisma.harnessRunMetric.create (fire-and-forget)", async () => {
      ...
      const svcDb = new MissionMetricsService(prisma);
