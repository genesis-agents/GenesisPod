 ...
 * Topic Insights 监控端点
 *
 * GET  /topic-insights/health           → HarnessHealthSnapshot
 * GET  /topic-insights/health/history   → DB-aggregated history snapshot
 * POST /topic-insights/reset-rollback   → 手动清除 auto-rollback 告警
 * GET  /topic-insights/capabilities     → TopicInsightsCapabilitySnapshot
 * POST /topic-insights/dispatch         → AG-17-LDP 意图分类
 */
...
} from "@/modules/ai-app/topic-insights/intent/dispatcher.service";
import {
  TopicInsightsCapabilityReconciler,
  type TopicInsightsCapabilitySnapshot,
} from "@/modules/ai-app/topic-insights/agents/capability";
import type { ResearchDepth } from "../../mission/pipeline/types";
...
export class TopicInsightsHealthController {
  constructor(
    private readonly rollout: MissionMetricsService,
