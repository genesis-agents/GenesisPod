 ...
 * - reviewSectionOutput: no model fallback, parse fail fallback, max revisions forced pass, with charts
...
const mockSectionPlan = {
  id: "section-001",
  title: "技术现状分析",
  description: "分析当前技术发展状态",
  keyPoints: ["市场规模", "技术趋势", "主要参与者"],
  targetWords: 1500,
...
describe("ResearchLeaderService (supplemental)", () => {
  ...
  describe("reviewSectionOutput", () => {
    it("should return approved with default score when no reasoning model available", async () => {
      mocks.mockFacade.getAvailableModelsExtended.mockResolvedValue([]);
      mocks.mockFacade.getReasoningModel.mockResolvedValue(null);
      ...
      expect(result.feedback).toContain("无推理模型");
    });

    it("should return approved with default when AI response cannot be parsed", async () => {
      mocks.mockFacade.getAvailableModelsExtended.mockResolvedValue([]);
      mocks.mockFacade.getReasoningModel.mockResolvedValue({
        id: "o3-mini",
        name: "o3-mini",
    ...
    it("should force approval when revision count reaches 2 and AI returns not approved", async () => {
      mocks.mockFacade.getAvailableModelsExtended.mockResolvedValue([]);
      mocks.mockFacade.getReasoningModel.mockResolvedValue({
        id: "o3-mini",
        name: "o3-mini",
      ...
      mocks.mockFacade.chat.mockResolvedValue({
        content: JSON.stringify({
          approved: false,
          score: 55,
          feedback: "内容不够深入，需要更多数据支撑",
          revisionInstructions: "请增加具体数据",
        }),
        isError: false,
      });
      ...
      expect(result.approved).toBe(true);
      expect(result.score).toBeGreaterThanOrEqual(60); // Math.max(55, 60) = 60
      expect(result.feedback).toContain("强制通过");
    });

    it("should return not approved when AI says needs revision and revisionCount < 2", async () => {
      mocks.mockFacade.getAvailableModelsExtended.mockResolvedValue([]);
      mocks.mockFacade.getReasoningModel.mockResolvedValue({
        id: "o3-mini",
        name: "o3-mini",
      ...
      expect(result.revisionInstructions).toBe("请添加具体案例");
    });

    it("should return approved=true when AI returns approved", async () => {
      mocks.mockFacade.getAvailableModelsExtended.mockResolvedValue([]);
      mocks.mockFacade.getReasoningModel.mockResolvedValue({
        id: "o3-mini",
        name: "o3-mini",
    ...
    it("should include charts data in prompt when provided", async () => {
      mocks.mockFacade.getAvailableModelsExtended.mockResolvedValue([]);
      mocks.mockFacade.getReasoningModel.mockResolvedValue({
        id: "o3-mini",
        name: "o3-mini",
      ...
      mocks.mockFacade.chat.mockResolvedValue({
        content: JSON.stringify({ approved: true, score: 90, feedback: "ok" }),
        isError: false,
      });

      await service.reviewSectionOutput(mockSectionPlan, "章节内容", 0, {
        generatedCharts: [
          { id: "chart-001", type: "bar", title: "趋势图" } as never,
        ],
      });
      ...
      expect(mocks.mockFacade.chat).toHaveBeenCalledWith(
        expect.objectContaining({
          messages: expect.arrayContaining([
            expect.objectContaining({
              content: expect.stringContaining("chart-001"),
            }),
          ]),
        }),
      );
    ...
    it("should include previousSections summary in prompt when provided", async () => {
      mocks.mockFacade.getAvailableModelsExtended.mockResolvedValue([]);
      mocks.mockFacade.getReasoningModel.mockResolvedValue({
        id: "o3-mini",
        name: "o3-mini",
      ...
      await service.reviewSectionOutput(
        mockSectionPlan,
        "当前章节",
        0,
        undefined,
        [{ title: "前一章节", content: "前一章节内容" }],
        "zh",
      );
      ...
      expect(mocks.mockFacade.chat).toHaveBeenCalledWith(
        expect.objectContaining({
          messages: expect.arrayContaining([
            expect.objectContaining({
              content: expect.stringContaining("前一章节"),
            }),
          ]),
        }),
      );
