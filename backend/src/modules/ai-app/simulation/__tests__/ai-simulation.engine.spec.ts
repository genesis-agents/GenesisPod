...
describe('AiSimulationEngineService', () => {
  ...
  describe('executeRun', () => {
    ...
    it('should initialize state and complete a 2-round run', async () => {
      ...
      (externalData.getSnapshot as jest.Mock).mockResolvedValue({ snapshot, evidence });

      // First findUnique call - the run itself
      (prisma.simulationRun.findUnique as jest.Mock)
        .mockResolvedValueOnce({ ...mockRun, scenario: mockScenario, turns: [] }) // executeRun initial
        .mockResolvedValueOnce({ ...mockRun, scenario: mockScenario }) // processRound round 1
        .mockResolvedValueOnce({ ...mockRun, currentRound: 1, scenario: mockScenario }) // processRound round 2
        .mockResolvedValueOnce({ // computeDebrief
          ...mockRun,
          currentRound: 2,
          turns: [
            { ...mockTurn, submissions: [] },
          ],
          scenario: mockScenario,
          worldState: {},
        });
    ...
    it('should resume from a specific round when resume=true', async () => {
      ...
      (prisma.simulationRun.findUnique as jest.Mock)
        .mockResolvedValueOnce({ ...resumeRun, scenario: mockScenario, turns: [] })
  ...
  describe('generateAgentDecision (via executeRun)', () => {
    ...
    it('should skip unavailable AI models and try the next one', async () => {
      ...
      const unavailableModel = { id: 'model-bad', name: 'bad-model', isAvailable: false };
      const availableModel = { id: 'model-ok', name: 'ok-model', isAvailable: true };

      (aiFacade.getAvailableModelsExtended as jest.Mock).mockResolvedValue([unavailableModel, availableModel]);
      ...
      const oneRoundRun = { ...mockRun, rounds: 1, params: { humanBreakEvery: 0, irrationalProb: 0, chaosProb: 0 } };
      (prisma.simulationRun.findUnique as jest.Mock)
        .mockResolvedValueOnce({ ...oneRoundRun, scenario: mockScenario, turns: [] })
    ...
    it('should fallback to template when AI returns error', async () => {
      ...
      (aiFacade.getAvailableModelsExtended as jest.Mock).mockResolvedValue([model]);
      (aiFacade.chat as jest.Mock).mockResolvedValue({
        isError: true,
      ...
      (prisma.simulationRun.findUnique as jest.Mock)
        .mockResolvedValueOnce({ ...mockRun, scenario: mockScenario, turns: [] })
    ...
    it('should fallback to template when AI throws quota error', async () => {
      ...
      const model = { id: 'model-1', name: 'model-1', isAvailable: true };
      (aiFacade.getAvailableModelsExtended as jest.Mock).mockResolvedValue([model]);
      (aiFacade.chat as jest.Mock).mockRejectedValue(new Error('quota exceeded 429'));
    ...
    it('should fallback to template when AI throws token limit error', async () => {
      ...
      const model = { id: 'model-1', name: 'model-1', isAvailable: true };
      (aiFacade.getAvailableModelsExtended as jest.Mock).mockResolvedValue([model]);
      (aiFacade.chat as jest.Mock).mockRejectedValue(new Error('MAX_TOKENS exceeded'));
    ...
    it('should fallback to template when AI throws empty response error', async () => {
      ...
      const model = { id: 'model-1', name: 'model-1', isAvailable: true };
      (aiFacade.getAvailableModelsExtended as jest.Mock).mockResolvedValue([model]);
      (aiFacade.chat as jest.Mock).mockRejectedValue(new Error('Empty response from API'));
    ...
    it('should use irrational creativity when irrationalBias is high', async () => {
      ...
      (aiFacade.chat as jest.Mock).mockResolvedValue({
        isError: false,
        content: '{"innerMonologue":"irrational thought","publicAction":"chaos action"}',
      });
      ...
      (prisma.simulationRun.findUnique as jest.Mock)
        .mockResolvedValueOnce({ ...runWithHighIrrational, scenario: mockScenario, turns: [] })
        .mockResolvedValueOnce({ ...runWithHighIrrational, scenario: mockScenario })
        .mockResolvedValueOnce({
          ...mockRun,
          turns: [mockTurn],
          scenario: mockScenario,
  ...
  describe('processRound', () => {
    it('should throw error when run not found during processRound', async () => {
      ...
      await expect(service.executeRun('run-1')).rejects.toThrow('not found when processing round');
    });
    ...
    it('should mark chaosInjected=true for CHAOS team agents when chaos is triggered', async () => {
      ...
      (prisma.simulationRun.findUnique as jest.Mock)
        .mockResolvedValueOnce({ ...runWithChaos, scenario: { ...mockScenario, agents: [chaosAgent] }, turns: [] })
        .mockResolvedValueOnce({ ...runWithChaos, scenario: { ...mockScenario, agents: [chaosAgent] } })
        .mockResolvedValueOnce({
          ...mockRun,
          turns: [mockTurn],
    ...
    it('should set visibility to global for ARBITER team agents', async () => {
      ...
      (prisma.simulationRun.findUnique as jest.Mock)
        .mockResolvedValueOnce({ ...mockRun, scenario: { ...mockScenario, agents: [arbiterAgent] }, turns: [] })
        .mockResolvedValueOnce({ ...mockRun, scenario: { ...mockScenario, agents: [arbiterAgent] } })
        .mockResolvedValueOnce({
          ...mockRun,
          turns: [mockTurn],
          scenario: mockScenario,
      ...
      const createCall = (prisma.simulationTurn.create as jest.Mock).mock.calls[0][0];
      const submissions = createCall.data.submissions as Array<{ team: string; visibility: string }>;
      const arbiterSubmission = submissions.find((s) => s.team === SimulationTeam.ARBITER);
  ...
  describe('simpleAdjudication', () => {
    it('should reject when agent cost exceeds company cash', async () => {
      ...
      (prisma.simulationRun.findUnique as jest.Mock)
        .mockResolvedValueOnce({ ...mockRun, scenario: { ...mockScenario, agents: [expensiveAgent] }, turns: [] })
        .mockResolvedValueOnce({ ...mockRun, scenario: { ...mockScenario, agents: [expensiveAgent] } })
        .mockResolvedValueOnce({
          ...mockRun,
          turns: [mockTurn],
          scenario: mockScenario,
      ...
      const createCall = (prisma.simulationTurn.create as jest.Mock).mock.calls[0][0];
      expect(createCall.data.adjudication.ruling).toBe('rejected_insufficient_funds');
    });
    ...
    it('should mark missing data providers in evidence', async () => {
      ...
      (prisma.simulationRun.findUnique as jest.Mock)
        .mockResolvedValueOnce({ ...runWithNoWorldState, scenario: mockScenario, turns: [] })
        .mockResolvedValueOnce({ ...runWithNoWorldState, scenario: mockScenario })
        .mockResolvedValueOnce({
          ...mockRun,
          turns: [mockTurn],
          scenario: mockScenario,
      ...
      const createCall = (prisma.simulationTurn.create as jest.Mock).mock.calls[0][0];
      const evidenceRefs = createCall.data.adjudication.evidenceRefs as Array<{ provider: string; status: string }>;
    ...
    it('should trigger black_swan ruling when chaosProb=1', async () => {
      ...
      const runWithMaxChaos = {
        ...mockRun,
        worldState: { market: true, finance: true, news: true, regulation: true },
        params: { humanBreakEvery: 0, irrationalProb: 0, chaosProb: 1.0 },
      };

      (prisma.simulationRun.findUnique as jest.Mock)
        .mockResolvedValueOnce({ ...runWithMaxChaos, scenario: mockScenario, turns: [] })
    ...
    it('should proceed with full world state data', async () => {
      ...
      (prisma.simulationRun.findUnique as jest.Mock)
        .mockResolvedValueOnce({ ...runWithFullState, scenario: mockScenario, turns: [] })
  ...
  describe('parseAgentResponse (via AI response)', () => {
    const setupRunWithModel = (chatResponse: string) => {
      ...
      (aiFacade.getAvailableModelsExtended as jest.Mock).mockResolvedValue([model]);
      (aiFacade.chat as jest.Mock).mockResolvedValue({ isError: false, content: chatResponse });
    ...
    it('should parse valid JSON response', async () => {
      const validJson = '{"innerMonologue":"thinking","publicAction":"attacking"}';
      setupRunWithModel(validJson);

      await service.executeRun('run-1');

      const createCall = (prisma.simulationTurn.create as jest.Mock).mock.calls[0][0];
      const submissions = createCall.data.submissions as Array<{ innerMonologue: string; publicAction: string }>;
    ...
    it('should extract fields from partial JSON', async () => {
      const partialJson = '{"innerMonologue":"partial thought","publicAction":"partial action"';
    ...
    it('should use plain text as fallback when no JSON structure present', async () => {
      const plainText = 'I will attack the market by lowering prices significantly';
  ...
  describe('computeDebrief', () => {
    ...
    it('should detect black swan events in debrief', async () => {
      ...
      const turnWithBlackSwan = {
        ...mockTurn,
        adjudication: { ruling: 'black_swan', notes: 'event occurred', blackSwanEvent: { name: 'Market Crash', affectedTeams: ['BLUE'] } },
        worldState: {
          blackSwan: { name: 'Market Crash', description: 'Prices crashed', impact: 'high', affectedTeams: ['BLUE'], probability: 0.1, triggered: true },
          blackSwanHistory: [],
        },
        submissions: [
          { agentId: 'agent-1', team: 'BLUE', role: 'CEO', publicAction: 'defend', innerMonologue: 'worried', irrational: false, companyId: 'company-1', visibility: 'team', timestamp: new Date().toISOString(), chaosInjected: false },
        ],
        createdAt: new Date(),
      };
      ...
      const summary = completionCall?.[0]?.data?.summary as { publicReport?: { keyFindings?: string[] } };
      expect(summary?.publicReport?.keyFindings?.some((f: string) => f.includes('Market Crash'))).toBe(true);
    });
    ...
    it('should detect rejected_insufficient_funds in debrief', async () => {
      ...
      const turnWithRejection = {
        ...mockTurn,
        roundNumber: 1,
        adjudication: { ruling: 'rejected_insufficient_funds', notes: 'Not enough cash' },
        worldState: {},
        submissions: [],
        createdAt: new Date(),
      };
      ...
      const summary = completionCall?.[0]?.data?.summary as { internalReport?: { biasesDetected?: Array<{ type: string }> } };
      expect(
        summary?.internalReport?.biasesDetected?.some((b) => b.type === 'overconfidence'),
      ).toBe(true);
    });
    ...
    it('should detect irrational submissions in debrief', async () => {
      ...
      const summary = completionCall?.[0]?.data?.summary as { internalReport?: { biasesDetected?: Array<{ type: string }> } };
      expect(
        summary?.internalReport?.biasesDetected?.some((b) => b.type === 'irrational_spike'),
      ).toBe(true);
    });
    ...
    it('should report missing data in debrief when world state is empty', async () => {
      ...
      const summary = completionCall?.[0]?.data?.summary as { internalReport?: { blindspots?: Array<{ type: string }> } };
      expect(
  ...
  describe('detectStateChange (tested via debrief)', () => {
    it('should detect new black swan in state change', async () => {
      ...
      const turn2 = {
        ...
        id: 'turn-2',
        roundNumber: 2,
        adjudication: { ruling: 'black_swan', blackSwanEvent: { name: 'Storm', affectedTeams: ['BLUE', 'RED'] } },
      ...
      (prisma.simulationRun.findUnique as jest.Mock)
        .mockResolvedValueOnce({ ...mockRun, rounds: 2, scenario: mockScenario, turns: [] })
        .mockResolvedValueOnce({ ...mockRun, rounds: 2, scenario: mockScenario }) // round 1
        .mockResolvedValueOnce({ ...mockRun, rounds: 2, scenario: mockScenario }) // round 2
        .mockResolvedValueOnce({
          ...mockRun,
          rounds: 2,
          turns: [turn1, turn2],
      ...
      expect(
        summary?.internalReport?.causalChain?.some((c) => c.cause.includes('回合')),
      ).toBe(true);
    });
  });
  ...
  describe('buildAgentSystemPrompt', () => {
    it('should include correct team role descriptions for different teams', async () => {
      const teams = [SimulationTeam.BLUE, SimulationTeam.RED, SimulationTeam.GREEN, SimulationTeam.WHITE, SimulationTeam.CHAOS];
      ...
      for (const team of teams) {
        ...
        (aiFacade.chat as jest.Mock).mockImplementation(({ messages }) => {
          ...
          return Promise.resolve({ isError: false, content: '{"innerMonologue":"t","publicAction":"a"}' });
        });

        (prisma.simulationRun.findUnique as jest.Mock)
          .mockResolvedValueOnce({ ...oneRoundRun, scenario: { ...mockScenario, agents: [teamAgent] }, turns: [] })
          .mockResolvedValueOnce({ ...oneRoundRun, scenario: { ...mockScenario, agents: [teamAgent] } })
          .mockResolvedValueOnce({ ...oneRoundRun, turns: [mockTurn], scenario: mockScenario, worldState: {} });
  ...
  describe('buildAgentUserPrompt', () => {
    it('should include black swan information in user prompt', async () => {
      const worldStateWithBlackSwan = {
        market: true,
        blackSwan: { name: 'Supply Chain Disruption', description: 'Major disruption' },
      };
      ...
      (prisma.simulationRun.findUnique as jest.Mock)
        .mockResolvedValueOnce({ ...oneRoundRun, scenario: mockScenario, turns: [] })
        .mockResolvedValueOnce({ ...oneRoundRun, scenario: mockScenario, worldState: worldStateWithBlackSwan })
        .mockResolvedValueOnce({ ...oneRoundRun, turns: [mockTurn], scenario: mockScenario, worldState: worldStateWithBlackSwan });
    ...
    it('should include public memory when available', async () => {
      ...
      const oneRoundRun = { ...mockRun, rounds: 1, params: { humanBreakEvery: 0, irrationalProb: 0, chaosProb: 0 } };
      (prisma.simulationRun.findUnique as jest.Mock)
        .mockResolvedValueOnce({ ...oneRoundRun, scenario: { ...mockScenario, agents: [agentWithMemory] }, turns: [] })
        .mockResolvedValueOnce({ ...oneRoundRun, scenario: { ...mockScenario, agents: [agentWithMemory] } })
        .mockResolvedValueOnce({ ...oneRoundRun, turns: [mockTurn], scenario: mockScenario, worldState: {} });
