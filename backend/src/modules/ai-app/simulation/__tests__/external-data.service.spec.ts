...
describe('ExternalDataService', () => {
  ...
  describe('fetchFromProvider', () => {
    ...
    it('should handle HTTP error response', async () => {
      ...
      mockAxios.get.mockRejectedValue({ response: { status: 401 }, message: 'Unauthorized' });
    ...
    it('should filter out invalid providers without id or name', async () => {
      const providers = [
        { id: '', name: 'Missing ID', category: 'market', enabled: true, baseUrl: 'https://example.com', apiKey: 'key' },
        { id: 'valid', name: 'Valid', category: 'market', enabled: true, baseUrl: 'https://valid.com', apiKey: 'key' },
      ];
      (prisma.systemSetting.findUnique as jest.Mock).mockResolvedValue({
        key: 'external.providers',
        value: JSON.stringify(providers),
  ...
  describe('testProvider', () => {
    ...
    it('should handle HTTP error from provider test', async () => {
      mockAxios.get.mockRejectedValue({ response: { status: 403 }, message: 'Forbidden' });
