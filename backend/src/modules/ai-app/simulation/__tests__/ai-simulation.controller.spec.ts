...
describe('AiSimulationController', () => {
  ...
  beforeEach(async () => {
    ...
    const mockExternalData = {
      getSnapshot: jest.fn().mockResolvedValue({ data: {}, timestamp: Date.now() }),
  ...
  describe('createScenario', () => {
    ...
    it('should create scenario with companies and agents', async () => {
      const body = {
        ...
        industry: 'retail',
        companies: [
          { name: 'Company A', type: 'leader', market: 'domestic' },
        ],
        agents: [
  ...
  describe('getScenario', () => {
    it('should return scenario by id', async () => {
      ...
      expect(simulationService.getScenarioById).toHaveBeenCalledWith('scenario-1');
      expect(result).toEqual(mockScenario);
    });
  });
  ...
  describe('updateScenario', () => {
    it('should update scenario', async () => {
      ...
      expect(simulationService.updateScenario).toHaveBeenCalledWith('scenario-1', body);
      expect(result).toEqual(mockScenario);
    });
  ...
  describe('deleteScenario', () => {
    it('should delete scenario', async () => {
      ...
      expect(simulationService.deleteScenario).toHaveBeenCalledWith('scenario-1');
  ...
  describe('getRun', () => {
    it('should get run without perspective filter', async () => {
      ...
      expect(simulationService.getRunById).toHaveBeenCalledWith('run-1', undefined);
      expect(result).toEqual(mockRun);
    });
    ...
    it('should get run with BLUE perspective', async () => {
      ...
      expect(simulationService.getRunById).toHaveBeenCalledWith('run-1', 'BLUE');
    });
  ...
  describe('interveneRun', () => {
    it('should intervene in run with message', async () => {
      ...
      expect(simulationService.interveneRun).toHaveBeenCalledWith('run-1', body);
      expect(result).toEqual({ success: true });
    });
