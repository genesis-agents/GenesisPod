...
jest.mock('../../../credits/billing-context', () => ({
  BillingContext: {
    run: jest.fn().mockImplementation((_ctx: unknown, fn: () => unknown) => Promise.resolve(fn())),
  },
}));
...
describe('AiSimulationService', () => {
  ...
  let engine: jest.Mocked<AiSimulationEngineService>;
  ...
  const mockScenario = {
    ...
    updatedAt: new Date(),
    companies: [
      { id: 'company-1', name: 'My Company', type: 'benchmark', scenarioId: 'scenario-1' },
    ],
    agents: [
      { id: 'agent-1', role: 'CEO', team: SimulationTeam.BLUE, scenarioId: 'scenario-1', company: null, companyId: null },
    ],
    runs: [],
  };
  ...
  beforeEach(async () => {
    ...
    engine = module.get(AiSimulationEngineService);
  });
  ...
  describe('createScenario', () => {
    it('should create a scenario with companies and agents', async () => {
      (prisma.simulationScenario.create as jest.Mock).mockResolvedValue(mockScenario);
      (prisma.simulationAgent.createMany as jest.Mock).mockResolvedValue({ count: 1 });
      (prisma.simulationScenario.findUnique as jest.Mock).mockResolvedValue(mockScenario);

      const result = await service.createScenario({
        name: 'Test Scenario',
        industry: 'SaaS',
        companies: [{ name: 'My Company', type: 'benchmark', market: 'Global' }],
        agents: [{ team: SimulationTeam.BLUE, role: 'CEO', companyName: 'My Company' }],
      });
    ...
    it('should create scenario without companies or agents', async () => {
      ...
      (prisma.simulationScenario.create as jest.Mock).mockResolvedValue(simpleScenario);
      (prisma.simulationScenario.findUnique as jest.Mock).mockResolvedValue(simpleScenario);
    ...
    it('should map agent companyName to companyId', async () => {
      const scenarioWithCompany = {
        ...mockScenario,
        companies: [{ id: 'comp-1', name: 'My Company', scenarioId: 'scenario-1' }],
      };
      (prisma.simulationScenario.create as jest.Mock).mockResolvedValue(scenarioWithCompany);
      (prisma.simulationAgent.createMany as jest.Mock).mockResolvedValue({ count: 1 });
      (prisma.simulationScenario.findUnique as jest.Mock).mockResolvedValue(scenarioWithCompany);
      ...
      const createManyCall = (prisma.simulationAgent.createMany as jest.Mock).mock.calls[0][0];
  ...
  describe('getScenarioById', () => {
    ...
    it('should throw NotFoundException when not found', async () => {
      (prisma.simulationScenario.findUnique as jest.Mock).mockResolvedValue(null);

      await expect(service.getScenarioById('not-found')).rejects.toThrow(NotFoundException);
    });
  });
  ...
  describe('deleteScenario', () => {
    it('should delete scenario and related records', async () => {
      (prisma.simulationScenario.findUnique as jest.Mock).mockResolvedValue(mockScenario);
      (prisma.simulationAgent.deleteMany as jest.Mock).mockResolvedValue({ count: 1 });
      (prisma.simulationCompany.deleteMany as jest.Mock).mockResolvedValue({ count: 1 });
      (prisma.simulationScenario.delete as jest.Mock).mockResolvedValue(mockScenario);
      ...
      expect(result).toEqual({ success: true, message: 'Scenario deleted successfully' });
    });

    it('should throw NotFoundException when scenario not found', async () => {
      (prisma.simulationScenario.findUnique as jest.Mock).mockResolvedValue(null);

      await expect(service.deleteScenario('not-found')).rejects.toThrow(NotFoundException);
    });
  });
  ...
  describe('updateScenario', () => {
    it('should update scenario fields', async () => {
      ...
      (prisma.simulationScenario.findUnique as jest.Mock)
        ...
        .mockResolvedValueOnce(existingScenario);
      (prisma.simulationScenario.update as jest.Mock).mockResolvedValue(existingScenario);
    ...
    it('should replace companies when provided', async () => {
      ...
      (prisma.simulationScenario.findUnique as jest.Mock)
        ...
        .mockResolvedValueOnce(existingScenario);
      (prisma.simulationScenario.update as jest.Mock).mockResolvedValue(existingScenario);
      (prisma.simulationCompany.deleteMany as jest.Mock).mockResolvedValue({ count: 0 });
      (prisma.simulationCompany.createMany as jest.Mock).mockResolvedValue({ count: 1 });
    ...
    it('should replace agents when provided', async () => {
      ...
      (prisma.simulationScenario.findUnique as jest.Mock)
        ...
        .mockResolvedValueOnce(existingScenario);
      (prisma.simulationScenario.update as jest.Mock).mockResolvedValue(existingScenario);
      (prisma.simulationAgent.deleteMany as jest.Mock).mockResolvedValue({ count: 0 });
      (prisma.simulationAgent.createMany as jest.Mock).mockResolvedValue({ count: 1 });
  ...
  describe('getRunById', () => {
    ...
    it('should throw NotFoundException when run not found', async () => {
      ...
      await expect(service.getRunById('not-found')).rejects.toThrow(NotFoundException);
    });
  });

  describe('listScenarios', () => {
    it('should list all scenarios ordered by updatedAt', async () => {
      (prisma.simulationScenario.findMany as jest.Mock).mockResolvedValue([mockScenario]);
  ...
  describe('startRun', () => {
    ...
    it('should use default 2 rounds when not specified', async () => {
      ...
      const createCall = (prisma.simulationRun.create as jest.Mock).mock.calls[0][0];
  ...
  describe('pauseRun', () => {
    it('should pause a running simulation', async () => {
      ...
      (prisma.simulationRun.findUnique as jest.Mock)
        ...
        .mockResolvedValueOnce({ ...runningRun, status: SimulationRunStatus.PAUSED });
      (prisma.simulationRun.update as jest.Mock).mockResolvedValue({
        ...runningRun,
        status: SimulationRunStatus.PAUSED,
      });

      const result = await service.pauseRun('run-1');
    ...
    it('should not update if run is not running', async () => {
      ...
      (prisma.simulationRun.findUnique as jest.Mock).mockResolvedValue(pausedRun);
  ...
  describe('getRunById with perspective', () => {
    it('should filter submissions by non-GOD perspective', async () => {
      ...
      (prisma.simulationRun.findUnique as jest.Mock).mockResolvedValue(runWithTurns);
  ...
  describe('filterSubmissionByPerspective (behavior test via getRun)', () => {
    it('should return full data for GOD perspective', async () => {
      ...
      const canViewFull = godPerspective === 'GOD' || submission.team === godPerspective;
