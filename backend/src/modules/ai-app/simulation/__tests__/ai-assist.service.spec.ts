...
describe('AIAssistService', () => {
  ...
  describe('analyzeIndustry', () => {
    it('should return LLM analysis when successful', async () => {
      const mockAnalysis = {
        companies: [
          { name: 'Company A', type: 'competitor', market: 'Global', reason: 'Leader' },
        ],
        agents: [
          { role: 'Regulator', team: 'WHITE', reason: 'Compliance' },
        ],
    ...
    it('should fall back to default when no models available', async () => {
      (aiFacade.getAvailableModels as jest.Mock).mockResolvedValue([]);
    ...
    it('should fall back to default when LLM fails', async () => {
      (aiFacade.chat as jest.Mock).mockRejectedValue(new Error('LLM failed'));

      const result = await service.analyzeIndustry({ industry: 'SaaS' });

      expect(result).toHaveProperty('companies');
      expect(result.insights).toEqual(expect.arrayContaining([expect.stringContaining('AI分析暂时不可用')]));
    });

    it('should fall back when LLM returns invalid JSON', async () => {
      (aiFacade.chat as jest.Mock).mockResolvedValue({ content: 'Not valid JSON' });
    ...
    it('should use region in analysis', async () => {
      ...
      const result = await service.analyzeIndustry({ industry: 'SaaS', region: 'China' });
  ...
  describe('suggestAgents', () => {
    ...
    it('should not duplicate existing agents', async () => {
      ...
      const existingAgents = [{ role: 'CEO', team: 'RED', companyName: 'Competitor' }];
  ...
  describe('generateScenarioSuggestions', () => {
    it('should return scenario suggestions for industry', async () => {
      (aiFacade.chat as jest.Mock).mockResolvedValue({
        content: JSON.stringify({
          companies: [
            { name: 'Company A', type: 'competitor', market: 'Global', reason: 'Big player' },
          ],
          agents: [],
          goals: { targetShare: '20%', risk: 'medium', growth: 'moderate' },
    ...
    it('should recommend more rounds for high-risk industries', async () => {
      ...
      const semiResult = await service.generateScenarioSuggestions({ industry: 'Semiconductor' });
      const saasResult = await service.generateScenarioSuggestions({ industry: 'SaaS' });

      expect(semiResult.recommendedRounds).toBeGreaterThan(saasResult.recommendedRounds);
    });
  });
  ...
  describe('generateCompanyMetrics', () => {
    it('should return metrics for a company', async () => {
      ...
      (aiFacade.getAvailableModels as jest.Mock).mockResolvedValue(mockAvailableModels);
      (aiFacade.chat as jest.Mock).mockResolvedValue({
        content: JSON.stringify({
          metrics: {
            cash: 50000,
  ...
  describe('suggestParams', () => {
    ...
    it('should increase chaos probability for semiconductor industry', async () => {
      const semiResult = await service.suggestParams({ industry: 'Semiconductor' });
    ...
    it('should recommend more rounds for high-volatility industries', async () => {
      const highVolResult = await service.suggestParams({ industry: 'AI Compute Infrastructure' });
    ...
    it('should require more frequent human review for high-regulation industries', async () => {
      const regulatedResult = await service.suggestParams({ industry: 'Fintech' });
      const standardResult = await service.suggestParams({ industry: 'SaaS' });

      expect(regulatedResult.humanBreakEvery).toBeLessThanOrEqual(standardResult.humanBreakEvery);
    });
    ...
    it('should add extra chaos for China region', async () => {
      const chinaResult = await service.suggestParams({ industry: 'SaaS', region: 'China' });
      const globalResult = await service.suggestParams({ industry: 'SaaS', region: 'Global' });
    ...
    it('should enable tech and finance events for high volatility', async () => {
      const result = await service.suggestParams({ industry: 'AI Compute Infrastructure' });
