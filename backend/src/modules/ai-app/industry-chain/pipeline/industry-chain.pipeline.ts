...
输出结构化 JSON：segments（含 order：上游小、下游大）、companies（每家尽量标注 segment = 所属环节名，与 segments 的 name 对应；**美股上市公司务必给出 ticker 股票代码**，如 NVDA / AMD / TSM，用于权威解析 SEC 数据；可含 sourceRefs）、relations。
