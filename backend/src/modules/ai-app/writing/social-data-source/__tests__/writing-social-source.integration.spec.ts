...
const makeChapterListRow = (overrides: Partial<{
  id: string;
  title: string;
  content: string | null;
  wordCount: number;
...
const makeChapterBundleRow = (overrides: Partial<{
  id: string;
  title: string;
  content: string | null;
  wordCount: number;
...
describe('WritingSocialSourceProvider — userId isolation (integration)', () => {
  let provider: WritingSocialSourceProvider;

  beforeEach(() => {
    jest.clearAllMocks();
    provider = new WritingSocialSourceProvider(mockPrisma);
    mockFindMany.mockResolvedValue([]);
  });

  it('uses the real WritingSocialSourceProvider class', () => {
    expect(provider).toBeInstanceOf(WritingSocialSourceProvider);
  ...
  describe('listItems', () => {
    ...
    it('hasMore logic — slices result and sets nextCursor when extra row returned', async () => {
      const rows = Array.from({ length: 21 }, (_, i) =>
        makeChapterListRow({ id: `ch-${i}`, createdAt: new Date(2024, 0, i + 1) }),
      );
