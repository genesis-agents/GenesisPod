...
请直接输出章节内容，以"第${this.numberToChinese(chapter.chapterNumber)}章 ${chapter.title}"开头。`;
  ...
  async cancelMission(missionId: string, userId: string) {
    ...
    // 先更新数据库状态（确保即使 orchestrator 失败也能取消）
    await this.prisma.writingMission.update({
      where: { id: missionId },
      data: {
        status: "FAILED",
    ...
    // 更新项目状态
    const project = await this.prisma.writingProject.findUnique({
      where: { id: mission.project.ownerId ? mission.projectId : "" },
      select: { currentWords: true, id: true },
    });
    ...
    if (project) {
      ...
      this.logger.log(
        `Updated project ${mission.projectId} status to ${newStatus} after cancellation`,
      );
    }

    // 尝试取消 orchestrator 执行（忽略错误，因为任务可能已经不在内存中）
    try {
