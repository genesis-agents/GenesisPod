...
`;
  ...
  private async runMissionInBackground(
    ...
    try {
      ...
      if (generatedContent) {
        totalWordCount = this.textProcessor.countWords(generatedContent);
        this.logger.log(
...
${Array.from(
  { length: totalVolumes },
  (_, i) => `
### 第${this.textProcessor.numberToChinese(i + 1)}卷
- 卷名：
- 核心冲突：
- 主要情节：
- 情感走向：`,
  ...
  "chapters": [
    ...
    while (!outline && retryCount <= maxRetries) {
      ...
      try {
        ...
        // 解析大纲
        const parsedOutline = this.jsonParser.parseOutlineJSON(
...
["标题1", "标题2", ...]`;
    ...
    for (let i = 0; i < outline.chapters.length; i++) {
      ...
      await this.updateMissionProgress(
        missionId,
        chapterProgress,
        `作家团队正在创作第${this.textProcessor.numberToChinese(chapterNumber)}章...`,
      );
      ...
      await this.eventEmitter.emitAgentWorking(input.projectId, {
        ...
        agentRole: "writer",
        status: "working",
        taskDescription: `创作第${this.textProcessor.numberToChinese(chapterNumber)}章 ${chapterInfo.title}`,
      ...
      await this.eventEmitter.emitMissionProgress(
        ...
        missionId,
        chapterProgress,
        `创作第${this.textProcessor.numberToChinese(chapterNumber)}章`,
        ["writer"],
      );
      ...
      const writerPrompt = this.buildChapterWriterPrompt(
        ...
        input.targetWordCount, // ★ 使用用户指定的目标字数
      );
...
- 描写要符合世界观设定`;
      ...
      if (!chapterContent || chapterContent.length < 500) {
        ...
        try {
          // 重试一次 - ★ 包含表达约束防止重复
          const retryPrompt = `请创作"第${this.textProcessor.numberToChinese(chapterNumber)}章 ${chapterInfo.title}"的内容。
...
${avoidancePrompt ? `\n【表达约束 - 禁止使用以下表达】\n${avoidancePrompt}` : ""}`;
          ...
          const retryTargetWords = input.targetWordCount || 3000;
          const retrySystemPrompt = `你是专业的小说作家。请直接创作故事内容，必须达到${retryTargetWords}字以上。
...
${qualityConstraints ? `${qualityConstraints}\n` : ""}`;
          ...
          chapterContent =
            ...
            `第${this.textProcessor.numberToChinese(chapterNumber)}章 ${chapterInfo.title}\n\n（内容生成中...）`;
        } catch (retryError) {
          this.logger.warn(
            `[${missionId}] Chapter ${chapterNumber} retry also failed: ${(retryError as Error).message}`,
          );
          chapterContent = `第${this.textProcessor.numberToChinese(chapterNumber)}章 ${chapterInfo.title}\n\n（内容生成中...）`;
        }
      }
      ...
      await this.eventEmitter.emitAgentWorking(input.projectId, {
        ...
        agentRole: "checker",
        status: "working",
        taskDescription: `校验第${this.textProcessor.numberToChinese(chapterNumber)}章一致性`,
      });

      await this.updateMissionProgress(
        missionId,
        chapterProgress + 1,
        `检查员正在校验第${this.textProcessor.numberToChinese(chapterNumber)}章...`,
      );
...
输出 JSON 格式：
{
  ...
  "issues": [
    { "type": "character/setting/timeline/terminology", "severity": "error/warning", "description": "问题描述", "location": "问题位置", "fix": "修复建议" }
      ...
      // 解析检查结果
      const checkResult = this.jsonParser.parseConsistencyCheckResult(
...
请严格逐条验证每个问题是否已被正确修复，不要放过任何细节。`,
        ...
        // 解析验证结果
        const verificationResult = this.jsonParser.parseVerificationResult(
      ...
      if (chapterNumber % 5 === 0) {
        await this.updateMissionProgress(
          missionId,
          chapterProgress + 2,
          `编辑正在润色第${this.textProcessor.numberToChinese(chapterNumber)}章...`,
        );
...
${editNarrativeConstraints}`;
      ...
      if (
        !chapterContent.includes(
          `第${this.textProcessor.numberToChinese(chapterNumber)}章`,
        )
      ) {
        chapterContent = `第${this.textProcessor.numberToChinese(chapterNumber)}章 ${chapterInfo.title}\n\n${chapterContent}`;
      }
...
- 用"突然"、"忽然"等词汇`,
      ...
      try {
        while (rewriteAttempts < safetyMaxRewriteAttempts) {
          // ★★★ 第一步：强制执行叙事工艺检查（不取决于 QualityGate）
          // 这是解决"AI写作陋习"问题的关键！
...
${narrativeConstraints}`;
          ...
          void this.kernelMemory
            .write({
              ...
              value: {
                ...
                wordCount: this.textProcessor.countWords(chapterContent),
      ...
      await this.eventEmitter.emitAgentWorking(input.projectId, {
        ...
        agentRole: "writer",
        status: "completed",
        taskDescription: `完成第${this.textProcessor.numberToChinese(chapterNumber)}章 (${chapterWordCount}字)`,
      });
    ...
    for (let i = 0; i < allChapters.length; i++) {
      ...
      if (
        ...
        const volumeTitle =
          ...
          `第${this.textProcessor.numberToChinese(currentVolumeIndex + 1)}卷`;
        volumeContents.push(
...
直接输出摘要内容，不要添加额外格式标记。`;
    ...
    // 降级到简单摘要
    return this.textProcessor.generateChapterSummarySimple(content);
  }
  ...
  private async extractChapterContext(
    ...
    await this.eventEmitter.emitAgentWorking(projectId, {
      ...
      agentRole: "keeper",
      status: "working",
      taskDescription: `为第${this.textProcessor.numberToChinese(chapterNumber)}章提取相关设定`,
    });
    ...
    try {
      ...
      if (jsonMatch) {
        ...
        await this.eventEmitter.emitAgentWorking(projectId, {
          ...
          agentRole: "keeper",
          status: "completed",
          taskDescription: `已为第${this.textProcessor.numberToChinese(chapterNumber)}章准备上下文`,
        });
        ...
        messages: [
          {
            ...
            content:
              ...
              "\n\n【关键】确保输出完整的、可解析的 JSON。如果内容较多，优先保证 JSON 结构完整，可适当精简描述。",
          },
      ...
      if (jsonMatch) {
        ...
        if (bible) {
          ...
          if (updates.newCharacters.length > 0) {
            // 先获取所有现有角色用于去重检查
            const allExistingChars =
              await this.prisma.writingCharacter.findMany({
                where: { bibleId: bible.id },
                select: {
                  id: true,
                  name: true,
            ...
            for (const newChar of updates.newCharacters) {
              try {
                // ★ 增强的去重检查：检查名字、别名、以及括号内的别名
                // 解析 "王昭儿（王美人）" 这种格式
                const parsedNames: string[] = [newChar.name];
                const aliasMatch = newChar.name.match(/^(.+?)（(.+?)）$/);
                if (aliasMatch) {
                  parsedNames.push(aliasMatch[1]); // 王昭儿
                  parsedNames.push(aliasMatch[2]); // 王美人
                }

                // 检查是否与现有角色重复
                const existingChar = allExistingChars.find((existing) => {
                  // 检查完全匹配
                  if (existing.name === newChar.name) return true;
                  // 检查解析后的名字是否匹配
                  if (parsedNames.includes(existing.name)) return true;
                  // 检查现有角色的别名
                  ...
                  if (
                    newChar.name.includes(existing.name) ||
                    existing.name.includes(newChar.name)
                  )
                    return true;
                  return false;
                });
      ...
      if (chapter) {
        ...
        const previousContext = previousChapters.reverse().map((prevChap) => {
          ...
          const summary = storedSummary
            ? storedSummary
            : prevChap.content
              ? this.textProcessor.extractSummaryFromContent(prevChap.content)
              : "（无内容）";
...
请输出修改后的完整章节内容：`;
    ...
    if (response.content && response.content.length > 200) {
      // ★★★ GAP-2 修复：@Leader modify_chapter 也必须进行叙事工艺检查 ★★★
      ...
      if (!narrativeReport.passed) {
        ...
        if (hasEndingOrCliche) {
          this.logger.log(
            `[${missionId}] Rewriting to fix ${narrativeReport.issues.length} narrative issues in modified chapter`,
          );
          const rewrittenContent = await this.narrativeCraft.rewriteEnding(
            modifiedContent,
          ...
          if (rewrittenContent && rewrittenContent !== modifiedContent) {
            ...
            this.logger.log(
              `[${missionId}] Successfully fixed narrative issues in modified chapter`,
            );
          }
        }
      }
  ...
  private async processResult(
      ...
      try {
        ...
        if (completionResult.needsContinuation) {
          ...
          writingResult.wordCount =
            this.textProcessor.countWords(extractedContent);
        } else {
          // 使用最终内容
          writingResult.content =
            completionResult.finalContent || extractedContent;
          writingResult.wordCount = this.textProcessor.countWords(
            writingResult.content,
          );
        }
    ...
    if (writingResult.content && writingResult.wordCount) {
      ...
      const fallbackContent = this.textProcessor.extractFallbackContent(
        result,
        input,
      );
      if (fallbackContent) {
        ...
        writingResult.wordCount =
          this.textProcessor.countWords(fallbackContent);
        await this.saveGeneratedContent(
          input,
          fallbackContent,
          writingResult.wordCount,
...
`;
        ...
        await this.prisma.writingChapter.create({
          data: {
            ...
            wordCount: this.textProcessor.countWords(placeholderContent),
            status: "DRAFT",
          },
        });
  ...
  private splitIntoChapters(content: string): string[] {
    ...
    for (const paragraph of paragraphs) {
      const paragraphWordCount = this.textProcessor.countWords(paragraph);
      if (
...
只输出重写后的开篇。`,
      ...
      // ★★★ GAP-1 修复：续写模式也必须进行叙事工艺检查 ★★★
      const narrativeReport =
        this.narrativeCraft.analyzeContent(chapterContent);
      if (!narrativeReport.passed) {
        this.logger.warn(
          `[${missionId}] Chapter ${chapter.chapterNumber} failed narrative craft check in continuation mode (score=${narrativeReport.score})`,
        );
        ...
        if (hasEndingOrCliche) {
          ...
          if (rewrittenContent !== chapterContent) {
            chapterContent = rewrittenContent;
            this.logger.log(
              `[${missionId}] Chapter ${chapter.chapterNumber} narrative issues fixed in continuation mode`,
            );
          }
        }
      }
  ...
  async reExtractChapterTitles(
    ...
    for (const chapter of chapters) {
      ...
      if (isPlaceholder && chapter.content) {
        const newTitle = this.textProcessor.extractChapterTitle(
          chapter.content,
          chapter.chapterNumber,
        );
