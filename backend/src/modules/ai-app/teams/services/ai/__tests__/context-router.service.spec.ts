...
import { ContextRouterService, ContextMessage } from '../context-router.service';
...
describe('ContextRouterService', () => {
  ...
  let aiFacade: { intentDetector: { detectIntent: jest.Mock } | null };
  ...
  describe('routeContext', () => {
    ...
    it('should handle REFERENCE_RECENT strategy', async () => {
      ...
      prisma.topicMessage.findMany.mockResolvedValue([...mockMessages, mockDebateMessage]);

      const result = await service.routeContext('topic-1', 'Summarize this', []);
    ...
    it('should build context from messages correctly - user messages as "user" role', async () => {
      ...
      const result = await service.routeContext('topic-1', 'Hello', []);
      const context = result.context as ContextMessage[];
  ...
  describe('generateSystemPromptAddition', () => {
    ...
    it('should generate GENERATE prompt addition', async () => {
      ...
      const result = await service.routeContext('topic-1', 'Generate image', []);
