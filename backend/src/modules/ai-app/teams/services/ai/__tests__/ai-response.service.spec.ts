...
const mockAiFacade = {
  chat: jest.fn().mockResolvedValue({ content: 'AI response here', tokensUsed: 200 }),
  embed: jest.fn().mockResolvedValue([0.1, 0.2]),
  getModelById: jest.fn().mockResolvedValue({ modelId: 'gpt-4o', name: 'GPT-4o', provider: 'openai' }),
};
...
const mockTeamMemberAgent = {
  ...
  formatMessage: jest.fn().mockReturnValue({ role: 'assistant', content: 'Response' }),
};
...
const mockCreditsService = {
  checkBalance: jest.fn().mockResolvedValue({ sufficient: true, balance: 1000 }),
...
describe('AiResponseService', () => {
  ...
  beforeEach(async () => {
    ...
    mockPrisma.topic.findUnique.mockResolvedValue({ id: 'topic-1', name: 'Test Topic', description: 'A topic' });
  ...
  describe('buildSmartContext', () => {
    ...
    it('should filter out mission system messages', async () => {
      const missionMessage = makeMessage({ content: '[任务分解]\n\n请执行以下任务...' });
      const normalMessage = makeMessage({ content: 'What is AI?', senderId: 'user-1' });
      ...
      expect(result.messages.every((m) => !m.content.includes('[任务分解]'))).toBe(true);
    });

    it('should filter out [工作汇报] messages', async () => {
      const reportMessage = makeMessage({ content: '[工作汇报] 任务完成度 50%' });
      const normalMessage = makeMessage({ content: 'Good progress!', senderId: 'user-1' });

      mockPrisma.topicMessage.findMany.mockResolvedValueOnce([reportMessage, normalMessage]);

      const result = await service.buildSmartContext('topic-1', 'ai-1');

      expect(result.messages.every((m) => !m.content.includes('[工作汇报]'))).toBe(true);
    });
    ...
    it('should always include the latest user message', async () => {
      ...
      const userMsgInContext = result.messages.find((m) => m.id === 'user-msg-1');
    ...
    it('should prioritize debate opponent messages in debate mode', async () => {
      ...
      const result = await service.buildSmartContext('topic-1', 'ai-1', 1, 'opponent-1');
  ...
  describe('extractUrls', () => {
    it('should extract HTTP URLs from text', () => {
      const text = 'Visit http://example.com and https://test.org for more info';

      const result = (service as unknown as { extractUrls: (t: string) => string[] }).extractUrls(text);
    ...
    it('should return empty array when no URLs exist', () => {
      const result = (service as unknown as { extractUrls: (t: string) => string[] }).extractUrls(
        'No URLs here',
      );
    ...
    it('should deduplicate URLs', () => {
      ...
      const exampleCount = result.filter((u) => u === 'https://example.com').length;
  ...
  describe('generateAIResponse', () => {
    ...
    it('should check credit balance before generating response', async () => {
      ...
      expect(mockCreditsService.checkBalance).toHaveBeenCalledWith('user-1', 30);
    });
    ...
    it('should build debate prompt when debateRole is provided', async () => {
      mockPrisma.topicAIMember.findFirst.mockResolvedValueOnce(makeAIMember());
      mockAiFacade.chat.mockResolvedValueOnce({ content: 'Debate response', tokensUsed: 100 });
    ...
    it('should include resource context in AI prompt when resources exist', async () => {
      ...
      mockAiFacade.chat.mockResolvedValueOnce({ content: 'Response with context', tokensUsed: 200 });
      ...
      const allContent = chatCall.messages.map((m: { content: string }) => m.content).join('\n');
    ...
    it('should work when credit service is not available', async () => {
      ...
      const serviceWithoutCredits = module.get<AiResponseService>(AiResponseService);
      mockAiFacade.chat.mockResolvedValueOnce({ content: 'Response', tokensUsed: 100 });

      // Should not throw even without CreditsService
      await expect(
        serviceWithoutCredits.generateAIResponse('topic-1', 'user-1', 'ai-1', []),
  ...
  describe('createAIMessage', () => {
    ...
    it('defaults tokensUsed to 0 when not provided', async () => {
      mockPrisma.topicMessage.create.mockResolvedValueOnce({ id: 'ai-msg-2', content: 'test' });
    ...
    it('updates topic updatedAt after creating message', async () => {
      mockPrisma.topicMessage.create.mockResolvedValueOnce({ id: 'ai-msg-3', content: 'test' });
  ...
  describe('parseAIMentionsFromContent', () => {
    it('returns empty array when no AI members exist', async () => {
      ...
      const result = await service.parseAIMentionsFromContent('topic-1', 'Hello @Bot');
    ...
    it('detects @mention of an AI by display name', async () => {
      ...
      const result = await service.parseAIMentionsFromContent('topic-1', 'Hello @Bot, how are you?');
    ...
    it('excludes the specified AI member from results', async () => {
      ...
      await service.parseAIMentionsFromContent('topic-1', '@OtherBot hello', 'ai-1');
    ...
    it('returns empty when content does not match any AI name', async () => {
      ...
      const result = await service.parseAIMentionsFromContent('topic-1', 'Hello world');
  ...
  describe('shouldSearchForInfo (private)', () => {
    const invoke = (content: string) =>
      (service as unknown as { shouldSearchForInfo: (s: string) => boolean }).shouldSearchForInfo(content);
  ...
  describe('shouldUseTools (private)', () => {
    const invoke = (aiMember: Parameters<AiResponseService['shouldUseTools' & string]>[0]) =>
      (service as unknown as { shouldUseTools: (m: typeof aiMember) => boolean }).shouldUseTools(aiMember);
  ...
  describe('buildMemberConfig (private)', () => {
    const invoke = (aiMember: object) =>
      (service as unknown as { buildMemberConfig: (m: typeof aiMember) => Record<string, unknown> }).buildMemberConfig(aiMember);
  ...
  describe('getDefaultModelId (private)', () => {
    const invoke = (modelId: string) =>
      (service as unknown as { getDefaultModelId: (id: string) => string }).getDefaultModelId(modelId);

    it('returns the model identifier as-is when it contains a dash and known provider', () => {
      expect(invoke('gpt-4-turbo')).toBe('gpt-4-turbo');
      expect(invoke('claude-3-5-sonnet-20241022')).toBe('claude-3-5-sonnet-20241022');
      expect(invoke('grok-3-latest')).toBe('grok-3-latest');
    });

    it('returns mapped default for "grok"', () => {
      expect(invoke('grok')).toBe('grok-3-latest');
    });

    it('returns mapped default for "claude"', () => {
      expect(invoke('claude')).toBe('claude-sonnet-4-20250514');
    });

    it('returns mapped default for "gemini"', () => {
      expect(invoke('gemini')).toBe('gemini-2.0-flash');
    });

    it('returns identifier as-is for unknown model without dashes', () => {
      expect(invoke('unknown-custom-model')).toBe('unknown-custom-model');
    });
  });
  ...
  describe('isRetryableError (private)', () => {
    const invoke = (error: Error) =>
      (service as unknown as { isRetryableError: (e: Error) => boolean }).isRetryableError(error);
  ...
  describe('generateAIResponse - additional branches', () => {
    it('records metrics when metricsService is available', async () => {
      ...
      mockPrisma.topicAIMember.findFirst.mockResolvedValueOnce(makeAIMember());
      mockAiFacade.chat.mockResolvedValueOnce({ content: 'Response', tokensUsed: 500 });
      ...
      expect(metricsMock.recordAIResponseTokens).toHaveBeenCalledWith('gpt-4o', 500);
    });
    ...
    it('calls auditService when available', async () => {
      mockPrisma.topicAIMember.findFirst.mockResolvedValueOnce(makeAIMember());
      mockAiFacade.chat.mockResolvedValueOnce({ content: 'Audited response', tokensUsed: 100 });
    ...
    it('includes AI collaboration prompt when canMentionOtherAI is true and other AIs exist', async () => {
      ...
      mockPrisma.topicAIMember.findFirst.mockResolvedValueOnce(aiMemberWithCollaboration);
      mockPrisma.topicAIMember.findMany.mockResolvedValueOnce([
        { id: 'ai-2', displayName: 'Colleague Bot', roleDescription: 'Helper' },
      ]);
      mockAiFacade.chat.mockResolvedValueOnce({ content: 'Collaborative response', tokensUsed: 100 });
    ...
    it('uses web-search tool when last user message triggers search', async () => {
      ...
      const mockWebSearchTool = {
        execute: jest.fn().mockResolvedValue({
          ...
          data: {
            success: true,
            results: [
              { title: 'Latest AI', url: 'https://ai.com', content: 'AI research content' },
            ],
          },
        }),
      };
      ...
      mockAiFacade.chat.mockResolvedValueOnce({ content: 'Search-enhanced response', tokensUsed: 200 });
    ...
    it('handles web fetch tool failure gracefully', async () => {
      ...
      mockToolRegistry.tryGet.mockReturnValueOnce(mockWebFetchTool);
      mockAiFacade.chat.mockResolvedValueOnce({ content: 'Response without fetch', tokensUsed: 100 });
    ...
    it('truncates very long context messages to MAX_SINGLE_MESSAGE_LENGTH', async () => {
      ...
      expect(mockAiFacade.chat).toHaveBeenCalled();
      const chatCall = mockAiFacade.chat.mock.calls[0][0];
      const userMsgs = chatCall.messages.filter((m: { role: string }) => m.role === 'user');
    ...
    it('calls contextRetrievalService when available and query is long enough', async () => {
      ...
      const longUserMessage = makeMessage({
        content: 'This is a long user question about AI research and its impact on society today',
        senderId: 'user-1',
        aiMemberId: null,
      });
    ...
    it('handles isReasoning model with extended output length', async () => {
      mockPrisma.topicAIMember.findFirst.mockResolvedValueOnce(makeAIMember({ aiModel: 'o1-mini' }));
      ...
      mockAiFacade.chat.mockResolvedValueOnce({ content: 'Deep reasoning response', tokensUsed: 2000 });
    ...
    it('handles large model (gpt-4) with long output length', async () => {
      mockPrisma.topicAIMember.findFirst.mockResolvedValueOnce(makeAIMember({ aiModel: 'gpt-4-turbo' }));
      mockAiFacade.getModelById.mockResolvedValueOnce({
        modelId: 'gpt-4-turbo',
        name: 'GPT-4 Turbo',
        provider: 'openai',
      ...
      mockAiFacade.chat.mockResolvedValueOnce({ content: 'GPT-4 response', tokensUsed: 500 });
    ...
    it('handles medium output length for non-large models', async () => {
      ...
      mockAiFacade.chat.mockResolvedValueOnce({ content: 'Llama response', tokensUsed: 100 });
    ...
    it('handles REFERENCE_RECENT context strategy from contextRouter', async () => {
      const { ContextStrategy } = require('@nestjs/common'); // fallback
      ...
      const module = await Test.createTestingModule({
        providers: [
          ...
          { provide: AIEngineFacade, useValue: mockAiFacade },
          { provide: ToolRegistry, useValue: mockToolRegistry },
          { provide: ContextRouterService, useValue: mockContextRouterWithRefRecent },
          { provide: TeamMemberAgent, useValue: mockTeamMemberAgent },
          { provide: TopicEventEmitterService, useValue: mockTopicEventEmitter },
          { provide: TopicContextRetrievalService, useValue: null },
          { provide: CreditsService, useValue: null },
      ...
      const svc = module.get<AiResponseService>(AiResponseService);
      mockPrisma.topicAIMember.findFirst.mockResolvedValueOnce(makeAIMember());
      mockAiFacade.chat.mockResolvedValueOnce({ content: 'Context-aware response', tokensUsed: 100 });
  ...
  describe('mission message filtering', () => {
    ...
    missionPatterns.forEach((pattern) => {
      it(`should filter out message starting with: "${pattern.substring(0, 30)}..."`, async () => {
        ...
        const normalMsg = makeMessage({ content: 'Normal conversation', senderId: 'user-1' });
        mockPrisma.topicMessage.findMany.mockResolvedValueOnce([missionMsg, normalMsg]);
