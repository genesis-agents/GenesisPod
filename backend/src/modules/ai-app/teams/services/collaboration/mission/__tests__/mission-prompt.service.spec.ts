...
describe('MissionPromptService', () => {
  ...
  describe('buildLeaderPlanningPrompt', () => {
    it('should build a planning prompt with all fields', () => {
      ...
      const result = service.buildLeaderPlanningPrompt(mission, leader, mockTeamMembers);
    ...
    it('should call missionContextService.buildContextPackagePromptSection', () => {
      ...
      expect(missionContextService.buildContextPackagePromptSection).toHaveBeenCalledWith(
  ...
  describe('buildScopeGuidance', () => {
    it('should return empty string for non-large-content task', () => {
      const result = service.buildScopeGuidance({ title: 'Write a report', description: 'Simple report' });
      expect(result).toBe('');
    });
  ...
  describe('detectLargeContentTask', () => {
    it('should detect novel with chapter structure', () => {
      expect(service.detectLargeContentTask('写一部玄幻小说，共8卷')).toBe(true);
    });

    it('should detect content keyword + structure keyword', () => {
      expect(service.detectLargeContentTask('这是一部连载故事，分5篇')).toBe(true);
    });
  ...
  describe('buildTaskExecutionPrompt', () => {
    it('should build execution prompt with search context', () => {
      const result = service.buildTaskExecutionPrompt(
        ...
        { title: 'Chapter 1', description: 'Write chapter', taskType: 'writing' },
        'Search results here',
      );
  ...
  describe('needsWebSearch', () => {
    ...
    it('should return false for regular task', () => {
      expect(service.needsWebSearch('Write a story', '', 'Chapter 1', 'Write introduction')).toBe(false);
    });

    it('should return true for research keyword', () => {
      expect(service.needsWebSearch('', '', '行业调研报告', '调研最新数据')).toBe(true);
    });
  });
  ...
  describe('buildSearchQuery', () => {
    it('should build search query from task title', () => {
      const result = service.buildSearchQuery('AI Trends', 'Latest Models', 'Compare models');
  ...
  describe('buildLeaderReviewPrompt', () => {
    it('should build review prompt with task result', () => {
      ...
      const result = service.buildLeaderReviewPrompt(mission, task, 'Chapter 2 content');
  ...
  describe('buildTaskRevisionPrompt', () => {
    it('should build revision prompt', () => {
      ...
      const result = service.buildTaskRevisionPrompt(task, 'Please improve the beginning');
  ...
  describe('buildFinalReportWithFullContent', () => {
    it('should build final report from completed tasks', () => {
      ...
      const { fullContent, summaryPrompt } = service.buildFinalReportWithFullContent(mission);
...
| 1 | Task 1 | @Alice | reason | 关键 | 无 |`;
  ...
  describe('parseReviewResult', () => {
    ...
    it('should return false when no clear result', () => {
      expect(service.parseReviewResult('内容写得很好，有一些小问题')).toBe(false);
    });
  ...
  describe('buildSummarizeForReviewPrompt', () => {
    it('should build summarize prompt', () => {
      const result = service.buildSummarizeForReviewPrompt('Long content here', 'Chapter 1');
