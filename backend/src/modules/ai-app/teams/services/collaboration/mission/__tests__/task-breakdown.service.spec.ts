...
describe('TaskBreakdownService', () => {
      ...
      $transaction: jest.fn().mockImplementation(async (fn) => {
        return fn({
          agentTask: {
            findMany: jest.fn().mockResolvedValue([]),
            create: jest.fn().mockResolvedValue(mockCreatedTask),
            createManyAndReturn: jest.fn().mockResolvedValue([{ id: 'task-created-1' }]),
          },
        });
      }),
    };
  ...
  describe('rebalanceTaskAssignments', () => {
    it('should do nothing when no tasks', () => {
      const breakdown = { tasks: [], understanding: '', executionPlan: '', risks: '' };
      service.rebalanceTaskAssignments(breakdown, mockTeamMembers);
      expect(mockTaskDecomposer.rebalanceTaskAssignments).not.toHaveBeenCalled();
    });
    ...
    it('should delegate to taskDecomposer and update assignments', () => {
      const rebalancedTasks = [
        { ...mockBreakdownTasks[0], assigneeId: 'member-2', assigneeName: 'Reviewer' },
        { ...mockBreakdownTasks[1], assigneeId: 'member-1', assigneeName: 'Writer' },
      ];
      mockTaskDecomposer.rebalanceTaskAssignments.mockReturnValue(rebalancedTasks);
  ...
  describe('createTasksFromBreakdown', () => {
    ...
    it('should skip tasks whose assignee is not found in teamMembers', async () => {
      ...
      mockTaskDecomposer.rebalanceTaskAssignments.mockReturnValue(breakdown.tasks);
