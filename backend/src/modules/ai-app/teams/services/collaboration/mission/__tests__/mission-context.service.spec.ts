...
const makeContextPackage = (overrides: Partial<MissionContextPackage> = {}): MissionContextPackage => ({
...
Now let's divide the tasks.
      ...
      const result = service.extractContextFromLeaderOutput(leaderOutput, 'leader-1');
...
\`\`\`
      ...
      const result = service.extractContextFromLeaderOutput(leaderOutput, 'my-leader-id');
...
|钟叔|人物|哑巴管家|
    ...
    it('should return null when natural language extraction finds nothing meaningful', () => {
      const leaderOutput = 'This is just a plain text without any structured content.';
...
\`\`\`
  ...
  describe('buildAgentSystemPromptWithContext', () => {
    it('should return minimal prompt when context and background are null', () => {
      ...
      const result = service.buildAgentSystemPromptWithContext(agent, task, null);
    ...
    it('should include hard constraints block when MUST constraints exist', () => {
      const context = makeContextPackage({
        hardConstraints: [
          { id: 'HC-001', rule: 'Characters must remain consistent', severity: 'MUST' },
        ],
      });

      const result = service.buildAgentSystemPromptWithContext(makeAgent(), makeTask(), context);
    ...
    it('should include quality standards block', () => {
      const context = makeContextPackage({
        qualityStandards: [
          { dimension: 'Consistency', requirement: 'All facts must align', metric: '< 5 contradictions' },
        ],
      });
    ...
    it('should include task description when provided', () => {
      ...
      const result = service.buildAgentSystemPromptWithContext(makeAgent(), task, null);
    ...
    it('should include task description in full prompt when context exists', () => {
      ...
      const result = service.buildAgentSystemPromptWithContext(makeAgent(), task, context);
    ...
    it('should use displayName when agentName is not set', () => {
      ...
      const result = service.buildAgentSystemPromptWithContext(agent, makeTask(), null);
...
`;
  ...
  describe('buildContextPackagePromptSection', () => {
    ...
    it('should list all member names', () => {
      const result = service.buildContextPackagePromptSection(['Alice', 'Bob', 'Charlie']);
  ...
  describe('validateOutputAgainstContext', () => {
    ...
    it('should return valid result structure', () => {
      const context = makeContextPackage();
      const result = service.validateOutputAgainstContext('Output text', context);
    ...
    it('should handle entity without attributes without throwing', () => {
      const context = makeContextPackage({
        entities: [
          { name: 'Gandalf', type: '人物', definition: 'A wizard' },
        ],
      });
      const output = 'Gandalf appeared and cast a spell.';

      expect(() => service.validateOutputAgainstContext(output, context)).not.toThrow();
    });

    it('should handle output with no prohibition keywords', () => {
      const context = makeContextPackage({
        prohibitions: [{ description: '非常长的一个禁止条款，包含了很多不相关的词语' }],
      });
...
\`\`\``,
    ...
    it('should truncate very long output to 6000 chars', async () => {
      ...
      await service.extractEstablishedFacts('task-1', 'Task', veryLongOutput, null, mockAiCaller);
      ...
      const userMessage = callArg.find((m: { role: string }) => m.role === 'user');
    ...
    it('should include existing entities in the prompt to avoid duplication', async () => {
      ...
      const existingContext = makeContextPackage({
        ...
        establishedFacts: [{ id: 'EF-1', sourceTaskId: 't1', sourceTaskTitle: 'T1', establishedAt: '', statement: 'Existing fact', category: 'decision', importance: 'high' }],
      });
      mockAiCaller.mockResolvedValue({ content: '[]' });

      await service.extractEstablishedFacts('task-1', 'Task', longOutput, existingContext, mockAiCaller);
    ...
    it('should try to parse JSON without code block delimiters', async () => {
      const longOutput = 'x'.repeat(201);
      mockAiCaller.mockResolvedValue({
        content: '[{"statement": "Direct JSON fact without code block", "category": "decision", "importance": "high"}]',
      });
...
\`\`\``,
  ...
  describe('mergeEstablishedFacts', () => {
    ...
    it('should not add duplicate facts (case-insensitive)', () => {
      ...
      const result = service.mergeEstablishedFacts(existingContext, duplicateFacts);
    ...
    it('should preserve existing context fields when merging', () => {
      const existingContext = makeContextPackage({
        hardConstraints: [{ id: 'HC-1', rule: 'Important rule', severity: 'MUST' }],
        entities: [{ name: 'Hero', type: '人物', definition: 'Main character' }],
      });
