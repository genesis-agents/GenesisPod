...
import { AgentTaskStatus, MissionStatus, MissionLogType, MessageContentType, TaskType } from '@prisma/client';
...
const mockCallbacks = {
  ...
  getTeamMembers: jest.fn().mockResolvedValue({
    leader: { id: 'leader-1', agentName: 'Leader', displayName: 'Leader Agent', aiModel: 'gpt-4', isLeader: true },
    members: [{ id: 'member-1', agentName: 'Alice', displayName: 'Alice Agent', aiModel: 'gemini-pro', isLeader: false }],
    all: [],
  }),
...
describe('MissionExecutionService', () => {
  ...
  const mockAiFacade = {
    chat: jest.fn().mockResolvedValue({ content: 'AI response content', tokensUsed: 200 }),
    getModelById: jest.fn().mockResolvedValue({ id: 'gpt-4', modelId: 'gpt-4', name: 'GPT-4' }),
    circuitBreaker: {
    ...
    capabilityResolver: {
      resolveCapabilities: jest.fn().mockResolvedValue({ tools: [], skills: [] }),
    },
  };
  ...
  describe('setCallbacks', () => {
    ...
    it('should throw when calling methods without callbacks set', async () => {
      ...
      await expect(
        freshService.executeNextTasks('mission-1'),
  ...
  describe('callAIWithConfig', () => {
    it('should call aiFacade.chat with messages and system prompt', async () => {
      ...
      const result = await service.callAIWithConfig('gpt-4', messages, 'System prompt');
    ...
    it('should track tokens when missionId is provided', async () => {
      ...
      await service.callAIWithConfig('gpt-4', [], 'System', { missionId: 'mission-1' });
  ...
  describe('callAIWithRetry', () => {
    const taskContext = { taskId: 'task-1', taskTitle: 'Test Task', missionId: 'mission-1' };
    ...
    it('should retry on retryable error', async () => {
      mockAiFacade.chat
        ...
        .mockResolvedValueOnce({ content: 'Success on retry', tokensUsed: 100 });
    ...
    it('should emit heartbeat events when heartbeat context provided', async () => {
      ...
      await service.callAIWithRetry('gpt-4', [], 'System', {}, taskContext, heartbeatContext);
  ...
  describe('executeNextTasks', () => {
    ...
    it('should skip when mission execution is already in progress', async () => {
      (stateManager.startMissionExecution as jest.Mock).mockReturnValueOnce(false);
    ...
    it('should execute pending tasks that have no pending dependencies', async () => {
      const missionWithTasks = {
        ...buildMockMission(),
        tasks: [
          { ...buildMockTask(), id: 'task-1', status: AgentTaskStatus.PENDING, dependsOnIds: [] },
        ],
      };
  ...
  describe('findAlternativeAgent', () => {
    ...
    it('should find alternative agent from team members', async () => {
      ...
      const result = await service.findAlternativeAgent(mission as any, [], task as any);
    ...
    it('should return null when only 1 member in team', async () => {
      mockCallbacks.getTeamMembers.mockResolvedValueOnce({
        leader: { id: 'leader-1', agentName: 'Leader', displayName: 'Leader Agent', aiModel: 'gpt-4', isLeader: true },
        members: [],
        all: [{ id: 'leader-1', agentName: 'Leader', displayName: 'Leader Agent', aiModel: 'gpt-4', isLeader: true }],
      });
    ...
    it('should return non-leader candidate when one is available and exclude failedAgentIds', async () => {
      mockCallbacks.getTeamMembers.mockResolvedValueOnce({
        leader: { id: 'leader-1', agentName: 'Leader', displayName: 'Leader Agent', aiModel: 'gpt-4', isLeader: true },
        members: [
          { id: 'member-1', agentName: 'Alice', displayName: 'Alice Agent', aiModel: 'gemini', isLeader: false },
          { id: 'member-2', agentName: 'Bob', displayName: 'Bob Agent', aiModel: 'claude', isLeader: false },
        ],
        all: [
          { id: 'leader-1', agentName: 'Leader', displayName: 'Leader Agent', aiModel: 'gpt-4', isLeader: true },
          { id: 'member-1', agentName: 'Alice', displayName: 'Alice Agent', aiModel: 'gemini', isLeader: false },
          { id: 'member-2', agentName: 'Bob', displayName: 'Bob Agent', aiModel: 'claude', isLeader: false },
        ],
      });
      ...
      const result = await service.findAlternativeAgent(mission as any, ['member-1'], task as any);
  ...
  describe('findAlternativeAgentWithCircuitBreaker', () => {
    beforeEach(() => {
      service.setCallbacks(mockCallbacks as any);
      mockAiFacade.circuitBreaker.canExecute = jest.fn().mockReturnValue(true);
      mockAiFacade.circuitBreaker.selectBest = jest.fn().mockReturnValue('member-1');
    });

    it('should return null when only 1 member', async () => {
      mockCallbacks.getTeamMembers.mockResolvedValueOnce({
        leader: { id: 'leader-1', displayName: 'Leader', aiModel: 'gpt-4', isLeader: true },
        members: [],
        all: [{ id: 'leader-1', displayName: 'Leader', aiModel: 'gpt-4', isLeader: true }],
      });

      const task = buildMockTask();
      const mission = buildMockMission();

      const result = await service.findAlternativeAgentWithCircuitBreaker(mission as any, [], task as any);
    ...
    it('should select best agent via circuit breaker', async () => {
      mockCallbacks.getTeamMembers.mockResolvedValueOnce({
        leader: { id: 'leader-1', displayName: 'Leader', aiModel: 'gpt-4', isLeader: true },
        members: [
          { id: 'member-1', displayName: 'Alice', aiModel: 'gemini', isLeader: false },
          { id: 'member-2', displayName: 'Bob', aiModel: 'claude', isLeader: false },
        ],
        all: [
          { id: 'leader-1', displayName: 'Leader', aiModel: 'gpt-4', isLeader: true },
          { id: 'member-1', displayName: 'Alice', aiModel: 'gemini', isLeader: false },
          { id: 'member-2', displayName: 'Bob', aiModel: 'claude', isLeader: false },
        ],
      });
      ...
      const result = await service.findAlternativeAgentWithCircuitBreaker(mission as any, ['leader-1'], task as any);

      expect(result).not.toBeNull();
    });

    it('should exclude agents with open circuit breakers', async () => {
      mockAiFacade.circuitBreaker.canExecute = jest.fn().mockImplementation((id: string) => {
        return id !== 'member-1'; // member-1 is blocked
      });
      mockAiFacade.circuitBreaker.selectBest = jest.fn().mockReturnValue('member-2');
    ...
    it('should handle errors and return null', async () => {
      mockCallbacks.getTeamMembers.mockRejectedValueOnce(new Error('Service error'));
  ...
  describe('temperature/maxTokens mapping via callAIWithConfig', () => {
    beforeEach(() => {
      ...
      mockAiFacade.chat.mockResolvedValue({ content: 'AI response content', tokensUsed: 200 });
    });
    ...
    it('should map temperature 0.2 to deterministic', async () => {
      await service.callAIWithConfig('gpt-4', [], 'System', { temperature: 0.2 });
    ...
    it('should map temperature 0.3 to low', async () => {
      await service.callAIWithConfig('gpt-4', [], 'System', { temperature: 0.3 });
    ...
    it('should map temperature 0.7 to medium', async () => {
      await service.callAIWithConfig('gpt-4', [], 'System', { temperature: 0.7 });
    ...
    it('should map temperature 0.9 to high', async () => {
      await service.callAIWithConfig('gpt-4', [], 'System', { temperature: 0.9 });
    ...
    it('should map maxTokens 1500 to short outputLength', async () => {
      await service.callAIWithConfig('gpt-4', [], 'System', { maxTokens: 1500 });
    ...
    it('should map maxTokens 3000 to medium outputLength', async () => {
      await service.callAIWithConfig('gpt-4', [], 'System', { maxTokens: 3000 });
    ...
    it('should map maxTokens 5000 to standard outputLength', async () => {
      await service.callAIWithConfig('gpt-4', [], 'System', { maxTokens: 5000 });
    ...
    it('should map maxTokens 7000 to long outputLength', async () => {
      await service.callAIWithConfig('gpt-4', [], 'System', { maxTokens: 7000 });
    ...
    it('should map maxTokens 10000 to extended outputLength', async () => {
      await service.callAIWithConfig('gpt-4', [], 'System', { maxTokens: 10000 });
  ...
  describe('callAIWithRetry error handling', () => {
    ...
    it('should stop immediately on rate limit error without retrying', async () => {
      ...
      mockAiFacade.chat.mockRejectedValue(new Error('rate_limit_exceeded: too many requests'));
    ...
    it('should handle empty response content gracefully', async () => {
      ...
      const result = await service.callAIWithRetry('gpt-4', [], 'System', {}, taskContext);
    ...
    it('should emit heartbeat events when heartbeatContext is provided', async () => {
      ...
      const heartbeatContext = { topicId: 'topic-1', agentId: 'agent-1', agentName: 'Alice' };

      // Make AI call take long enough for heartbeat to fire
      let resolve: (value: any) => void;
      const pendingCall = new Promise((r) => { resolve = r; });
      mockAiFacade.chat.mockReturnValueOnce(pendingCall as any);

      const callPromise = service.callAIWithRetry('gpt-4', [], 'System', {}, taskContext, heartbeatContext);
  ...
  describe('autoRetryBlockedTasks edge cases', () => {
    beforeEach(() => {
      service.setCallbacks(mockCallbacks as any);
      mockAiFacade.circuitBreaker.canExecute = jest.fn().mockReturnValue(true);
      mockAiFacade.circuitBreaker.getCooldownRemaining = jest.fn().mockReturnValue(5000);
    });
  ...
  describe('forceCompleteStuckTasks edge cases', () => {
    ...
    it('should process multiple stuck tasks', async () => {
      ...
      const stuckTasks = [
        { ...buildMockTask(), id: 'task-1', status: AgentTaskStatus.REVISION_NEEDED, updatedAt: stuckAt, result: null },
        { ...buildMockTask(), id: 'task-2', status: AgentTaskStatus.AWAITING_REVIEW, updatedAt: stuckAt, result: null },
      ];
