...
describe('TopicJoinRequestService', () => {
  ...
  const mockPrisma = {
    ...
    topicJoinRequest: {
      ...
      update: jest.fn().mockResolvedValue({ ...mockJoinRequest, status: JoinRequestStatus.APPROVED }),
  ...
  describe('createJoinRequest', () => {
    it('should create join request successfully', async () => {
      ...
      const result = await service.createJoinRequest('topic-1', 'user-2', 'Please join');
    ...
    it('should throw BadRequestException when already a member', async () => {
      mockPrisma.topicMember.findUnique.mockResolvedValueOnce({ id: 'existing' });
    ...
    it('should handle P2002 duplicate request error', async () => {
      mockPrisma.topicMember.findUnique.mockResolvedValueOnce(null);
      mockPrisma.topicJoinRequest.create.mockRejectedValueOnce({ code: 'P2002' });
    ...
    it('should re-throw non-P2002 errors', async () => {
      mockPrisma.topicMember.findUnique.mockResolvedValueOnce(null);
      mockPrisma.topicJoinRequest.create.mockRejectedValueOnce(new Error('DB error'));
    ...
    it('should not notify when no admins exist', async () => {
      mockPrisma.topic.findUnique.mockResolvedValueOnce({ ...mockTopic, members: [] });
      mockPrisma.topicMember.findUnique.mockResolvedValueOnce(null);
  ...
  describe('getJoinRequests', () => {
    ...
    it('should apply pagination options', async () => {
      await service.getJoinRequests('topic-1', 'admin-1', { page: 2, limit: 10 });
    ...
    it('should filter by status when provided', async () => {
      await service.getJoinRequests('topic-1', 'admin-1', { status: JoinRequestStatus.PENDING });
  ...
  describe('approveJoinRequest', () => {
    it('should approve join request and add member', async () => {
      const result = await service.approveJoinRequest('topic-1', 'req-1', 'admin-1');
    ...
    it('should assign specified role when approving', async () => {
      await service.approveJoinRequest('topic-1', 'req-1', 'admin-1', TopicRole.ADMIN);
  ...
  describe('rejectJoinRequest', () => {
    it('should reject join request', async () => {
      const result = await service.rejectJoinRequest('topic-1', 'req-1', 'admin-1', 'Not qualified');
