...
const mockPublicTopic = {
  ...
  members: [],
  joinRequests: [],
  createdBy: { id: 'user-1', username: 'creator', fullName: 'Creator', avatarUrl: null },
...
describe('TopicPublicService', () => {
  ...
  const mockPrisma = {
    ...
    topicJoinRequest: {
      ...
      findUnique: jest.fn().mockResolvedValue({
        ...mockJoinRequest,
        topic: { id: 'topic-1', name: 'Public Team' },
        user: { id: 'user-2', email: 'user2@example.com', username: 'user2', fullName: 'User Two' },
      }),
      update: jest.fn().mockResolvedValue({ ...mockJoinRequest, status: 'APPROVED' }),
    },
        ...
        expect.objectContaining({
          where: expect.objectContaining({
            OR: expect.arrayContaining([
              expect.objectContaining({ name: expect.objectContaining({ contains: 'AI' }) }),
            ]),
          }),
          take: 10,
        }),
  ...
  describe('requestToJoinTopic', () => {
    it('should create join request for public topic', async () => {
      const result = await service.requestToJoinTopic('topic-1', 'user-2', 'Please join');
  ...
  describe('getJoinRequests', () => {
    it('should return pending join requests for admin', async () => {
      ...
      expect(prisma.topicJoinRequest.findMany).toHaveBeenCalledWith(
        expect.objectContaining({
          where: expect.objectContaining({ topicId: 'topic-1', status: 'PENDING' }),
        }),
      );
  ...
  describe('reviewJoinRequest', () => {
    ...
    it('should reject join request', async () => {
      await service.reviewJoinRequest('req-1', 'admin-1', false, 'Not qualified');
