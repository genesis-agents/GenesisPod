...
const mockTopic = {
  ...
  aiMembers: [
    { id: 'ai-1', displayName: 'AI Assistant' },
  ],
};
...
describe('TopicSummariesService', () => {
  ...
  beforeEach(async () => {
    ...
    aiFacade = {
      chat: jest.fn().mockResolvedValue({ content: 'Generated summary', tokensUsed: 200 }),
    };
  ...
  describe('generateSummary', () => {
    ...
    it('should filter messages from fromMessageId', async () => {
      ...
      prisma.topicMessage.findUnique.mockResolvedValueOnce({ createdAt: fromMsgDate });
    ...
    it('should filter messages to toMessageId', async () => {
      ...
      prisma.topicMessage.findUnique.mockResolvedValueOnce({ createdAt: toMsgDate });
      prisma.topicMessage.findMany.mockResolvedValue(mockMessages);
    ...
    it('should use specified aiModel', async () => {
      await service.generateSummary('topic-1', 'user-1', { aiModel: 'gemini-pro' });
  ...
  describe('deleteSummary', () => {
    ...
    it('should allow OWNER/ADMIN to delete others\' summaries', async () => {
      ...
      topicCrudService.checkTopicPermission.mockResolvedValue({ role: 'OWNER' });
