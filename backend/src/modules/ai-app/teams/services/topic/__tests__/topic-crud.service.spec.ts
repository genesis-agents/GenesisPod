...
const mockTopic = {
  ...
  createdAt: new Date(),
  updatedAt: new Date(),
  createdBy: { id: 'user-1', username: 'user1', fullName: 'User One', avatarUrl: null },
  members: [
    {
      id: 'membership-1',
      topicId: 'topic-1',
      ...
      lastReadAt: null,
      user: { id: 'user-1', username: 'user1', fullName: 'User One', avatarUrl: null },
    },
  ],
  aiMembers: [],
...
describe('TopicCrudService', () => {
    ...
    prisma = {
      ...
      $transaction: jest.fn().mockImplementation(async (fn) => {
        const tx = {
          ...
          topicMessageReaction: { deleteMany: jest.fn().mockResolvedValue({ count: 0 }) },
          topicMessageMention: { deleteMany: jest.fn().mockResolvedValue({ count: 0 }) },
          topicMessageAttachment: { deleteMany: jest.fn().mockResolvedValue({ count: 0 }) },
          topicMessage: {
            findMany: jest.fn().mockResolvedValue([]),
            deleteMany: jest.fn().mockResolvedValue({ count: 0 }),
          },
          topicMessageBookmark: { deleteMany: jest.fn().mockResolvedValue({ count: 0 }) },
          topicMessageForward: { deleteMany: jest.fn().mockResolvedValue({ count: 0 }) },
          topicSummary: { deleteMany: jest.fn().mockResolvedValue({ count: 0 }) },
          topicResource: { deleteMany: jest.fn().mockResolvedValue({ count: 0 }) },
        };
        return fn(tx);
      }),
  ...
  describe('getTopicById', () => {
    ...
    it('should throw NotFoundException when topic not found', async () => {
      ...
      await expect(service.getTopicById('nonexistent', 'user-1')).rejects.toThrow(
        NotFoundException,
      );
    });
