...
import { RoleRegistry } from '../../../../../ai-engine/facade';
...
const mockTeamInfo = {
  ...
  memberRoles: [{ roleId: 'analyst', minCount: 1, maxCount: 3, required: true }],
};
...
const mockTeamConfig = {
  ...
  availableTools: [],
  constraintProfile: {
    cost: { budget: 10, modelPreference: 'balanced' as const, allowOverBudget: false, warningThreshold: 0.8 },
    quality: { depth: 'standard' as const, accuracy: 'prefer_evidence' as const, reviewRequired: true, minReviewScore: 7, maxReworks: 2 },
    efficiency: { maxDuration: 300000, priority: 'normal' as const, allowParallel: true, maxParallelism: 3 },
  },
...
describe('AiTeamsIntegrationService', () => {
  ...
  describe('createCustomTeam', () => {
    ...
    it('should create a custom team', () => {
      const result = service.createCustomTeam(createDto as any);
    ...
    it('should throw when teams service unavailable', () => {
      ...
      expect(() => serviceWithoutFacade.createCustomTeam(createDto as any)).toThrow(
      ...
      expect(teamRegistry.registerConfig).toHaveBeenCalledWith(
        expect.objectContaining({
          constraintProfile: expect.objectContaining({
            cost: expect.objectContaining({ budget: 50, modelPreference: 'premium' }),
            quality: expect.objectContaining({ depth: 'comprehensive', maxReworks: 5 }),
          }),
        }),
      );
    });
  ...
  describe('updateCustomTeam', () => {
    ...
    it('should update custom team', () => {
      const result = service.updateCustomTeam('custom-123', updateDto as any);
    ...
    it('should throw when teams service unavailable', () => {
      ...
      expect(() => serviceWithoutTeams.updateCustomTeam('custom-123', updateDto as any)).toThrow(
        'TeamsService not available',
      );
    });

    it('should throw when trying to update predefined team', () => {
      expect(() => service.updateCustomTeam('predefined-team', updateDto as any)).toThrow(
        'Cannot update predefined teams',
      );
    });
    ...
    it('should throw when team not found', () => {
      ...
      expect(() => service.updateCustomTeam('custom-nonexistent', updateDto as any)).toThrow(
  ...
  describe('deleteCustomTeam', () => {
    ...
    it('should throw when teams registry unavailable', () => {
      ...
      expect(() => serviceWithoutRegistry.deleteCustomTeam('custom-123')).toThrow(
        'TeamsService not available',
      );
    });
