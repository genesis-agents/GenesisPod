...
import { TeamCollaborationAgent, TeamTaskType, VotingStrategy } from '../team-collaboration.agent';
import { BUILTIN_AGENTS } from '../../../../ai-engine/agents/base/plan-based-agent';
import { BUILTIN_TOOLS } from '../../../../ai-engine/core/types/agent.types';
import type { AgentInput, AgentPlan } from '../../../../ai-engine/agents/base/plan-based-agent';
...
describe('TeamCollaborationAgent', () => {
  ...
  describe('getTemplates', () => {
    ...
    it('should include team-brainstorm template', () => {
      ...
      expect(brainstorm?.defaultOptions?.taskType).toBe(TeamTaskType.TEAM_BRAINSTORM);
    });
    ...
    it('should include task-breakdown template', () => {
      ...
      expect(breakdown?.defaultOptions?.taskType).toBe(TeamTaskType.TASK_DELEGATION);
    });
    ...
    it('should include consensus-decision template', () => {
      ...
      expect(consensus?.defaultOptions?.votingStrategy).toBe(VotingStrategy.MAJORITY);
    });
    ...
    it('should include red-blue-debate template', () => {
      ...
      expect(debate?.defaultOptions?.taskType).toBe(TeamTaskType.DEBATE_SESSION);
    });
    ...
    it('should include mission-planning template', () => {
      ...
      expect(mission?.defaultOptions?.taskType).toBe(TeamTaskType.MISSION_EXECUTION);
    });
  });
  ...
  describe('plan() - via options.taskType', () => {
    ...
    it('should carry topicId in plan metadata when provided', async () => {
      const input = buildInput({
        options: { taskType: TeamTaskType.TEAM_BRAINSTORM, topicId: 'my-topic' },
      });
    ...
    it('should calculate estimatedTime as sum of step durations', async () => {
      ...
      const plan = await agent.plan(input);
      const sumDurations = plan.steps.reduce((sum, s) => sum + s.estimatedDuration, 0);
  ...
  describe('execute() - no input', () => {
    it('should yield error event when plan has no input', async () => {
      const plan: AgentPlan = {
        ...
        steps: [{ id: 'step-1', name: 'Test', description: '', toolId: BUILTIN_TOOLS.TEXT_GENERATION, dependencies: [], estimatedDuration: 1000 }],
        estimatedTime: 1000,
        toolsRequired: [],
        modelsRequired: [],
        metadata: {},
  ...
  describe('execute() - TEAM_BRAINSTORM full flow', () => {
    ...
    it('should include artifact with correct type in complete event', async () => {
      ...
      expect(completeEvent.result.artifacts[0].mimeType).toBe('application/json');
    });
  ...
  describe('execute() - DEBATE_SESSION full flow', () => {
    it('should complete successfully with judge', async () => {
      const input = buildInput({
        options: { taskType: TeamTaskType.DEBATE_SESSION, maxRounds: 1, includeJudge: true },
      });
    ...
    it('should complete successfully without judge', async () => {
      const input = buildInput({
        options: { taskType: TeamTaskType.DEBATE_SESSION, maxRounds: 1, includeJudge: false },
      });
  ...
  describe('execute() - metadata and topicId', () => {
    it('should pass topicId in artifact metadata', async () => {
      const input = buildInput({
        options: { taskType: TeamTaskType.MISSION_EXECUTION, topicId: 'topic-xyz' },
      });
      ...
      expect(completeEvent.result.artifacts[0].metadata.topicId).toBe('topic-xyz');
    });
    ...
    it('should track member contributions count', async () => {
      ...
      expect(completeEvent.result.artifacts[0].metadata.memberCount).toBeGreaterThan(0);
    });
  });
  ...
  describe('execute() - step event structure', () => {
    it('should emit step_start events for each step', async () => {
      ...
      const events = await collectEvents(agent.execute(plan));
      const stepStartEvents = events.filter((e: any) => e.type === 'step_start');
    ...
    it('should emit step_complete events for each step', async () => {
      ...
      const events = await collectEvents(agent.execute(plan));
      const stepCompleteEvents = events.filter((e: any) => e.type === 'step_complete');
    ...
    it('should emit step_progress events at 30% and 100%', async () => {
      ...
      const events = await collectEvents(agent.execute(plan));
      const progressEvents = events.filter((e: any) => e.type === 'step_progress') as any[];

      const thirtyPercentEvents = progressEvents.filter((e) => e.progress === 30);
      const hundredPercentEvents = progressEvents.filter((e) => e.progress === 100);
