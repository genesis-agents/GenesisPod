 ...
 *   - ai-engine/skills/runtime/engine-skill-provider.adapter.ts → ai-harness/agents/abstractions
...
/** 把文件路径按 modules/ 切片得到所属层（ai-app / ai-harness / ai-engine / ai-infra / open-api / common / 其他） */
...
function importLayer(spec: string): string | null {
  ...
  if (m1) return m1[1];
  // 同目录或子目录相对路径不指向 modules/
  if (spec.startsWith(".") && !/modules\//.test(spec)) return null;
  // 第三方 / @nestjs / 等不计
...
describe("Layer Boundaries (CLAUDE.md L4→L3→L2.5→L2→L1)", () => {
  describe("Single-direction dependency", () => {
    it("ai-engine 不得 import ai-harness（除合法 adapter）", () => {
      ...
      const allowlist = [
        "modules/ai-engine/skills/runtime/engine-skill-provider.adapter.ts",
      ];
