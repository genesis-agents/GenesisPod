...
function execCommand(command, options = {}) {
  try {
    ...
    if (options.returnError) {
      return { error: true, code: error.status, output: error.stdout || error.stderr || '' };
    }
    return null;
  }
}
...
function countAnyTypes(directory, excludePatterns = []) {
  const defaultExcludes = ['node_modules', 'dist', '.next', '*.spec.ts', '*.test.ts'];
...
function main() {
  ...
  const frontendAnyCount = countAnyTypesManual(FRONTEND_SRC, ['node_modules', '.next']);
  ...
  const metrics = {
    ...
    overall: {
      ...
      totalEslintErrors: (backendEslintErrors >= 0 ? backendEslintErrors : 0) +
  ...
  console.log(`  Overall:  ${metrics.overall.allTypeChecksPassed ? '✓ PASS' : '✗ FAIL'}`);

  console.log('\n⚠️  ESLint Errors:');
  console.log(`  Backend:  ${backendEslintErrors >= 0 ? backendEslintErrors : 'N/A'}`);
  console.log(`  Frontend: ${frontendEslintErrors >= 0 ? frontendEslintErrors : 'N/A'}`);
  console.log(`  Total:    ${metrics.overall.totalEslintErrors >= 0 ? metrics.overall.totalEslintErrors : 'N/A'}`);
