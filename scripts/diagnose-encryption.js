...
async function main() {
  ...
  const key = (process.env.SETTINGS_ENCRYPTION_KEY || 'deepdive-default-encryption-key!').padEnd(32, '0').substring(0, 32);

  console.log('Encryption key (first 10 chars):', key.substring(0, 10) + '...');

  const settings = await prisma.systemSetting.findMany({ where: { encrypted: true } });
