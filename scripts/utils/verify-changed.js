...
function main() {
  ...
  const hasPrismaChanges = changedFiles.some(f => f.includes('prisma/schema.prisma'));
  const hasAiServiceChanges = changedFiles.some(f => f.startsWith('ai-service/'));
  const hasRootChanges = changedFiles.some(f => !f.includes('/') || f.startsWith('scripts/'));
  ...
  if (hasRootChanges && !hasFrontendChanges && !hasBackendChanges) {
    log('\nRoot configuration changes detected. Consider running full validation.', 'yellow');
  }
  ...
  log(`\n${passedCount}/${totalCount} tasks passed`, allPassed ? 'green' : 'red');
