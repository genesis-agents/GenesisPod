...
export default function MissionDetailPage() {
  ...
  const view = useMemo(() => {
    ...
    // （W8 follow-up：24 处 view.X 改吃 missionView 直接形状后此 shim 可删）。
    // W1 cutover 后 persisted 本身就是 missionView 的派生 alias，所以原先的
    // synthesis fallback (events.length===0 && persisted / persisted-merge override)
    // 已成为纯 dead code —— canonical view 自身已含 stages/agents/status/cost
    // 终态全部信息，由 backend projector + ArtifactComposerService 投影。
    return viewToDerivedShim(missionView, events);
  }, [missionView, events]);
