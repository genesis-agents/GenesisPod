...
// ★ W8 cutover (B5-2): canonical mission truth 单轨；shim 文件已删除，DerivedView
//   shape 投影逻辑下沉为本文件私有 helper（buildLegacyDerivedView，行尾）。
//   truth 唯一来源：useMissionDetailView。
...
// ============================================================================
// W8 inline shim — buildLegacyDerivedView
// ============================================================================
//
// 落地依据：thinning plan §B5-2 / §3.4 / §7.2
//
// W8 cutover (2026-05-26)：原 view-to-derived.shim.ts 文件已删除；shim 投影逻辑
// 下沉为本文件私有 helper。page.tsx 76 处 view.X 访问的 shape 适配在此局部完成，
// 不再有跨文件 bridge。canonical view → DerivedView 形状适配规则：
//
//   • mission.status → DerivedView legacy timestamp 字段 (completedAt/failedAt/
//     cancelledAt/rejectedAt) 由 finishedAt 派生
//   • 14 backend canonical stage → 5 frontend StageId (STAGE_STEPS 聚合)
//   • agents trace 由 raw events 解析 (§7.2 raw event timeline display)
...
function buildLegacyDerivedView(
  view:
    | import('@/services/agent-playground/api').MissionDetailView
    | null
    | undefined,
...
function dvProjectMission(
  view: import('@/services/agent-playground/api').MissionDetailView
): MissionState {
