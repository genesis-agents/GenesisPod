...
import type { ReportChart } from '@/types/topic-research';
 ...
 * 生成描述性的 alt 文本
 */
function generateAltText(chart: ReportChart): string {
  const parts: string[] = [];
  ...
  if (chart.type) {
    ...
    parts.push(typeNames[chart.type] || chart.type);
  }

  if (chart.source) {
    parts.push(`来源: ${chart.source}`);
  }

  return parts.length > 0 ? parts.join(' - ') : '图表';
}
...
function ReferenceFigureRenderer({
  ...
  const altText = generateAltText(chart);
    ...
    <>
      <div className="relative">
        ...
        <div
          ...
          aria-label={allowZoom ? `点击放大查看: ${altText}` : undefined}
        >
          <Image
            src={chart.imageUrl}
            alt={altText}
            ...
            <div className="mt-2 flex items-center gap-2">
              {chart.evidenceCitationIndex && (
                <button
                  ...
                  aria-label={`查看引用来源 ${chart.evidenceCitationIndex}`}
                >
                  [{chart.evidenceCitationIndex}]
                </button>
              )}
...
function IncompleteChartPlaceholder({
  ...
  return (
    <div className="flex flex-col items-center justify-center rounded-lg border border-dashed border-amber-300 bg-amber-50 p-8">
      ...
      <p className="mt-2 text-sm text-amber-700">图表数据不完整</p>
      {chart.title && (
        <p className="mt-1 text-xs text-amber-600">{chart.title}</p>
      )}
      <p className="mt-2 max-w-xs text-center text-xs text-amber-500">
        {!chart.type && '缺少图表类型'}
        {!chart.type && (!chart.data || chart.data.length === 0) && '、'}
        {(!chart.data || chart.data.length === 0) && '缺少图表数据'}
      </p>
      {onRetry && (
        <button
          onClick={onRetry}
...
function UnknownChartPlaceholder({ chart }: { chart: ReportChart }) {
  return (
    <div className="flex flex-col items-center justify-center rounded-lg border border-dashed border-gray-300 bg-gray-50 p-8">
      <ImageOff className="h-12 w-12 text-gray-400" aria-hidden="true" />
      <p className="mt-2 text-sm text-gray-500">无法渲染图表</p>
      {chart.title && (
        <p className="mt-1 text-xs text-gray-400">{chart.title}</p>
      )}
      ...
      <div className={`flex items-center justify-center py-12 ${className}`}>
        <div className="text-center">
          <Loader2
            ...
            aria-label="加载中"
          />
          <p className="mt-2 text-sm text-gray-500">正在加载图表...</p>
        </div>
      </div>
    );
  }
